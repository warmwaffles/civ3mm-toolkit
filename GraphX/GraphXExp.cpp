...
L_INT RedirectClose(L_INT /*hf*/, L_VOID L_FAR* /*pUserData*/)
{
    return TRUE;
}

L_UINT RedirectWrite(L_INT /*FD*/, L_CHAR L_FAR* /*pBuf*/, L_INT uCount, L_VOID L_FAR* /*pUserData*/)
{
    // not implemented
    return (L_UINT)uCount;
}

L_INT RedirectOpen(L_CHAR* /*pFileName*/, L_INT /*nMode*/, L_INT /*nShare*/, L_VOID L_FAR* /*pUserData*/)
{
...
L_INT32 RedirectSeek(L_INT /*fd*/, L_INT32 lnPos, L_INT nOrigin, L_VOID L_FAR* /*pUserData*/)
{
    switch (nOrigin)
    {
    case SEEK_SET:
...
L_UINT RedirectRead(L_INT /*fd*/, L_CHAR* pBuf, L_INT nCount, L_VOID L_FAR* /*pUserData*/)
{
...
HDC GRAPHX::CreateGraphXDC(pBITMAPHANDLE pBitmap)
{
...
L_BOOL GRAPHX::DeleteGraphXDC(HDC hDC)
{
...
L_VOID GRAPHX::AccessBitmap (pBITMAPHANDLE pBitmap)
{
    GraphXnamespace::L_AccessBitmap(pBitmap);
}

L_INT GRAPHX::AddBitmapNoise(pBITMAPHANDLE	pBitmap,
							 L_UINT			uRange,
							 L_UINT			uChannel)
{
...
L_INT GRAPHX::AllocateBitmap (pBITMAPHANDLE	pBitmap,
							  L_INT			nMemoryType)
{
...
L_INT GRAPHX::AverageFilterBitmap(pBITMAPHANDLE	pBitmap,
								  L_UINT		uDim)
{	
...
L_INT GRAPHX::BinaryFilterBitmap(pBITMAPHANDLE	pBitmap,
								 LPBINARYFLT	pFilter)
{	
...
L_INT GRAPHX::ChangeBitmapContrast(pBITMAPHANDLE	pBitmap,
								   L_INT			nChange)
{
...
L_INT GRAPHX::ChangeBitmapHeight(pBITMAPHANDLE	pBitmap,
								 L_INT			nHeight)
{
...
L_INT GRAPHX::ChangeBitmapHue(pBITMAPHANDLE	pBitmap,
							  L_INT			nAngle)
{
...
L_INT GRAPHX::ChangeBitmapIntensity(pBITMAPHANDLE	pBitmap,
									L_INT			nChange)
{
...
L_INT GRAPHX::ChangeBitmapSaturation(pBITMAPHANDLE	pBitmap,
									 L_INT			nChange)
{
...
L_INT GRAPHX::ChangeFromDDB(HDC             hDC,
							pBITMAPHANDLE	pBitmap,
							HBITMAP         hBitmap,
							HPALETTE        hPalette)
{
    L_INT res = GraphXnamespace::L_ChangeFromDDB(
        hDC,
        pBitmap,
        hBitmap,
        hPalette);
...
L_INT GRAPHX::ChangeFromDIB(pBITMAPHANDLE	pBitmap,
							HANDLE			hDIB)
{
...
HANDLE GRAPHX::ChangeToDIB(pBITMAPHANDLE pBitmap)
{
...
L_INT GRAPHX::ClearBitmap(pBITMAPHANDLE pBitmap)
{
...
L_INT GRAPHX::ColorMergeBitmap(pBITMAPHANDLE	pBitmap,
...
L_INT GRAPHX::ColorResBitmap(pBITMAPHANDLE		pBitmapSrc,
							 ...
							 L_VOID*			pUserData)
{
	L_INT res = GraphXnamespace::L_ColorResBitmap(pBitmapSrc,
		pBitmapDst,
		nBitsPerPixel,
		uFlags,
		pPalette,
...
L_INT GRAPHX::ColorSeparateBitmap(pBITMAPHANDLE		pBitmap,
								  pBITMAPHANDLE*	ppBitmap,
								  L_UINT32			uFlags)
{
...
L_INT GRAPHX::CombineBitmap(pBITMAPHANDLE	pBitmapDst,
							L_INT			nXDst,
							L_INT			nYDst,
							L_INT			nWidth,
							L_INT			nHeight,
							...
							L_UINT32		uFlags)
{
	L_INT res = GraphXnamespace::L_CombineBitmap(pBitmapDst,
		nXDst,
		nYDst,
		nWidth,
		nHeight,
...
{
	L_INT res = GraphXnamespace::L_ConvertBuffer(pBuffer,
		nWidth, nBitsPerPixelSrc, nBitsPerPixelDst,
		nOrderSrc, nOrderDst, pPaletteSrc, pPaletteDst);
    return CheckSuccess(res, _T("ConvertBuffer"));
}

L_INT GRAPHX::ConvertFromDDB(HDC hDC,
							 pBITMAPHANDLE pBitmap,
							 HBITMAP hBitmap,
							 HPALETTE hPalette)
{
    L_INT res = GraphXnamespace::L_ConvertFromDDB(hDC,
        pBitmap,
        hBitmap,
        hPalette);
...
HBITMAP GRAPHX::ConvertToDDB(HDC hDC, pBITMAPHANDLE pBitmap)
{
...
HANDLE GRAPHX::ConvertToDIB(pBITMAPHANDLE pBitmap)
{
...
L_INT GRAPHX::CopyBitmap(pBITMAPHANDLE pBitmapDst,
						 pBITMAPHANDLE pBitmapSrc)
{
...
L_INT GRAPHX::CopyBitmapData(pBITMAPHANDLE pBitmapDst,
							 pBITMAPHANDLE pBitmapSrc)
{
...
L_VOID GRAPHX::CopyBitmapHandle(pBITMAPHANDLE pBitmapDst,
								pBITMAPHANDLE pBitmapSrc)
{
    GraphXnamespace::L_CopyBitmapHandle(pBitmapDst, pBitmapSrc);
}

L_INT GRAPHX::CopyBitmapRect(pBITMAPHANDLE pBitmapDst,
							 pBITMAPHANDLE pBitmapSrc,
							 L_UINT uCol,
							 L_UINT uRow,
							 L_UINT uWidth,
...
L_BOOL GRAPHX::CopyFromClipboard(HWND hWnd,
								 pBITMAPHANDLE pBitmap)
{
...
L_BOOL GRAPHX::CopyToClipboard (HWND hWnd,
								pBITMAPHANDLE pBitmap)
{
...
L_INT GRAPHX::CopyToClipboardExt(HWND hWnd,
								 pBITMAPHANDLE pBitmap,
								 L_UINT uFlags)
{
...
L_INT GRAPHX::CreateBitmap(pBITMAPHANDLE pBitmap,
						   L_UINT uMemory,
						   L_UINT uWidth,
						   L_UINT uHeight,
						   ...
						   L_UINT32 dwSize)
{
    L_INT res = GraphXnamespace::L_CreateBitmap(pBitmap,
        uMemory,
        uWidth,
        uHeight,
        uBitsPerPixel,
...
HPALETTE GRAPHX::CreatePaintPalette(HDC hDC,
									pBITMAPHANDLE pBitmap)
{
...
L_INT GRAPHX::DeskewBitmap(pBITMAPHANDLE pBitmap, L_INT32 L_FAR* pnAngle)
{
    return CheckSuccess(GraphXnamespace::L_DeskewBitmap(pBitmap, pnAngle), _T("DeskewBitmap"));
}

L_INT GRAPHX::DespeckleBitmap(pBITMAPHANDLE pBitmap)
{
    return CheckSuccess(GraphXnamespace::L_DespeckleBitmap(pBitmap), _T("DespeckleBitmap"));
}

HPALETTE GRAPHX::DupPalette(HPALETTE hPalette)
{
	return GraphXnamespace::L_DupPalette(hPalette);
}

L_INT GRAPHX::FileConvert(
						  const L_CHAR* pszFileSrc,
						  ...
						  pFILEINFO pFileInfo)
{
    L_INT res = GraphXnamespace::L_FileConvert((LPSTR)pszFileSrc, (LPSTR)pszFileDst,
        nType, nWidth, nHeight, nBitsPerPixel, nQFactor, pLoadOptions, pSaveOptions,
		pFileInfo);
    return CheckSuccess(res, _T("FileConvert"));
}

L_INT GRAPHX::FileInfo(const L_CHAR*     pszFile,
					   pFILEINFO           pFileInfo, 
					   L_UINT              flags,
					   pLOADFILEOPTION	   pLoadOptions)
{
    L_INT res = GraphXnamespace::L_FileInfo((LPSTR)pszFile, pFileInfo, flags,
		pLoadOptions);
    return CheckSuccess(res, _T("FileInfo"));
}

L_INT GRAPHX::FileInfo( int         hFile, 
					   long        nStart, 
					   long        nLength, 
					   pFILEINFO   pFileInfo,
					   L_UINT      flags)
...
L_INT GRAPHX::FileInfoMemory(L_CHAR* pBuffer,
							 pFILEINFO pFileInfo,
							 L_INT32 nBufferSize, 
							 L_UINT flags,
							 pLOADFILEOPTION pLoadOptions)
...
L_INT GRAPHX::FillBitmap(pBITMAPHANDLE pBitmap,
						 COLORREF crFill)
{
...
L_VOID GRAPHX::FreeBitmap(pBITMAPHANDLE pBitmap)
{
    GraphXnamespace::L_FreeBitmap(pBitmap);
}

L_INT GRAPHX::GetBitmapColors(pBITMAPHANDLE pBitmap,
							  L_INT nIndex,
							  L_INT nCount,
							  LPRGBQUAD pPalette)
{
...
L_INT32 GRAPHX::GetBitmapRowCol(pBITMAPHANDLE pBitmap,
								L_UCHAR* pBuffer,
								L_INT nRow,
								L_INT nCol,
								L_UINT32 uBytes)
{
    L_INT32 res;
    res = GraphXnamespace::L_GetBitmapRowCol(
		pBitmap,
		pBuffer,
		nRow,
		nCol,
...
COLORREF GRAPHX::GetPixelColor(pBITMAPHANDLE pBitmap,
							   L_INT nRow,
							   L_INT nCol)
{
...
L_INT GRAPHX::GrayScaleBitmap(pBITMAPHANDLE pBitmap, L_INT nBitsPerPixel)
{
    L_INT res = GraphXnamespace::L_GrayScaleBitmap (pBitmap, nBitsPerPixel);
    return CheckSuccess(res, _T("GrayScaleBitmap"));
}

L_INT GRAPHX::IsGrayScaleBitmap(pBITMAPHANDLE pBitmap)
{
    return GraphXnamespace::L_IsGrayScaleBitmap(pBitmap);
}

L_INT GRAPHX::HalfToneBitmap(pBITMAPHANDLE pBitmap,
							 L_UINT uType)
{
...
L_INT GRAPHX::HalfToneBitmapExt(pBITMAPHANDLE pBitmap,
								L_UINT uType,
								L_INT32 nAngle)
{
...
L_INT GRAPHX::InitBitmap(pBITMAPHANDLE pBitmap,
						 L_INT nWidth,
						 L_INT nHeight,
						 L_INT nBitsPerPixel)
{
...
L_INT GRAPHX::LoadBitmap(const L_CHAR* pszFile,
						 pBITMAPHANDLE pBitmap,
						 L_INT nBitsPerPixel,
						 L_INT nOrder,
						 pLOADFILEOPTION pLoadOptions,
...
L_INT GRAPHX::LoadBitmapMemory(L_CHAR* pBuffer,
							   pBITMAPHANDLE pBitmap,
							   L_INT nBitsPerPixel,
							   L_INT nOrder,
							   L_INT32 nBufferSize,
...
L_INT GRAPHX::LoadFile(L_CHAR* pszFile,
					   pBITMAPHANDLE pBitmap,
					   L_INT nBitsPerPixel,
					   L_INT nOrder,
					   L_UINT uFlags,
...
L_INT GRAPHX::LoadFileOffset(L_INT fd,
...
L_INT GRAPHX::LoadMemory(L_CHAR* pBuffer,
						 pBITMAPHANDLE pBitmap,
						 L_INT nBitsPerPixel,
						 L_INT nOrder,
						 L_UINT uFlags,
						 ...
						 pFILEINFO pFileInfo)
{
	...
	if (fi.Format == FILE_GIF)
	{
		...
		res = GraphXnamespace::L_LoadMemory (pBuffer, pBitmap, nBitsPerPixel, nOrder, uFlags, pfnCallback, pUserData, nBufferSize, pLoadOptions, pFileInfo);
    return CheckSuccess(res, _T("LoadMemory"));
}

L_INT GRAPHX::PaintDC(HDC hDC,
					  pBITMAPHANDLE pBitmap,
					  LPRECT pSrc,
					  LPRECT pClipSrc,
					  LPRECT pDst,
...
L_INT GRAPHX::PaintDCEffect(HDC hDC,
							pBITMAPHANDLE pBitmap,
							LPRECT pSrc,
							LPRECT pClipSrc,
							LPRECT pDst,
...
L_INT GRAPHX::PaintRgnDC(HDC hDC,
						 pBITMAPHANDLE pBitmap,
						 LPRECT pSrc,
						 LPRECT pClipSrc,
						 LPRECT pDst,
...
HDC GRAPHX::PrintBitmapExt(HDC hDC,
						   pBITMAPHANDLE pBitmap,
						   L_INT nX,
						   L_INT nY,
						   L_INT nWidth,
						   ...
						   L_BOOL fEndDoc)
{
    HDC res = GraphXnamespace::L_PrintBitmapExt(
        hDC,
        pBitmap,
        nX,
        nY,
...
L_INT GRAPHX::PutBitmapColors(pBITMAPHANDLE pBitmap,
							  L_INT nIndex,
							  L_INT nCount,
							  LPRGBQUAD pPalette)
{
...
L_INT32 GRAPHX::PutBitmapRow(pBITMAPHANDLE pBitmap,
							 L_UCHAR* pBuffer,
							 L_INT nRow,
							 L_UINT32 uBytes);

L_INT32 GRAPHX::PutBitmapRowCol(pBITMAPHANDLE pBitmap,
								L_UCHAR* pBuffer,
								L_INT nRow,
								L_INT nCol,
								L_UINT32 uBytes)
{
    L_INT32 res;
    res = GraphXnamespace::L_PutBitmapRowCol(
        pBitmap,
        pBuffer,
        nRow,
        nCol,
...
L_INT GRAPHX::PutPixelColor(pBITMAPHANDLE pBitmap,
							L_INT nRow,
							L_INT nCol,
							COLORREF crColor)
{
...
L_VOID GRAPHX::ReleaseBitmap(pBITMAPHANDLE pBitmap)
{
    GraphXnamespace::L_ReleaseBitmap(pBitmap);
}

L_INT GRAPHX::ResizeBitmap(pBITMAPHANDLE pBitmapSrc, pBITMAPHANDLE pBitmapDst)
{
    L_INT res = GraphXnamespace::L_ResizeBitmap (pBitmapSrc, pBitmapDst);
    return CheckSuccess(res, _T("ResizeBitmap"));
}

L_INT GRAPHX::RotateBitmap(pBITMAPHANDLE pBitmap,
						   L_INT nAngle,
						   L_UINT uFlags,
						   COLORREF crFill)
{
...
L_INT GRAPHX::SaveBitmap(const L_CHAR* pszFile,
						 pBITMAPHANDLE pBitmap,
						 L_INT nFormat,
						 L_INT nBitsPerPixel,
						 L_INT nQFactor,
						 pSAVEFILEOPTION pSaveOptions)
{
    L_INT res = GraphXnamespace::L_SaveBitmap((L_CHAR*) pszFile, 
        pBitmap, 
        nFormat, 
        nBitsPerPixel, 
        nQFactor,
...
L_INT GRAPHX::SaveBitmapMemory(HANDLE  *hHandle,
							   pBITMAPHANDLE pBitmap,
							   L_INT nFormat,
							   L_UINT32  *uSize,
							   L_INT nBitsPerPixel,
							   ...
							   pSAVEFILEOPTION pSaveOptions)
{
    L_INT res = GraphXnamespace::L_SaveBitmapMemory(hHandle,
        pBitmap, 
        nFormat, 
        nBitsPerPixel, 
        nQFactor,
...
L_INT GRAPHX::SaveFile(L_CHAR* pszFile,
					   pBITMAPHANDLE pBitmap,
					   L_INT nFormat,
					   L_INT nBitsPerPixel,
					   L_INT nQFactor,
					   ...
					   pSAVEFILEOPTION pSaveOptions)
{
    L_INT res = GraphXnamespace::L_SaveFile (pszFile, pBitmap, nFormat, nBitsPerPixel, nQFactor, uFlags, pfnCallback, pUserData, pSaveOptions);
    return CheckSuccess(res, _T("SaveFile"));
}

L_INT GRAPHX::SaveFileOffset(L_INT fd,
							 L_INT32 nOffsetBegin,
...
L_UINT32 GRAPHX::SetDisplayMode(L_UINT32 uFlagPos,
								L_UINT32 uFlagSet)
{
    return GraphXnamespace::L_SetDisplayMode(uFlagPos, uFlagSet);
}

STATUSCALLBACK GRAPHX::SetStatusCallBack(STATUSCALLBACK pfnCallback, L_VOID L_FAR * pUserData)
{
...
L_INT GRAPHX::SizeBitmap(pBITMAPHANDLE pBitmap,
						 L_INT nWidth,
						 L_INT nHeight)
{
...
L_INT GRAPHX::SizeBitmapExt(pBITMAPHANDLE pBitmap, L_INT nWidth, L_INT nHeight, L_UINT32 uFlags)
{
	L_INT res = GraphXnamespace::L_SizeBitmap (pBitmap, nWidth, nHeight, uFlags);
	return CheckSuccess(res, _T("SizeBitmapExt"));
}

L_BOOL GRAPHX::IsTwainAvailable(HWND hWnd)
{
	return GraphXnamespace::L_IsTwainAvailable (hWnd);
}

L_INT GRAPHX::TwainAcquire(HWND hWnd,
						   pBITMAPHANDLE pBitmap)
{
    return CheckSuccess(GraphXnamespace::L_TwainAcquire(hWnd, pBitmap), _T("TwainAcquire"));
}

L_INT GRAPHX::TwainAcquireExt(HWND hWnd,
							  pBITMAPHANDLE pBitmap,
							  L_UINT uFlags,
							  TWAINCALLBACK pfnCallback,
							  L_VOID* pUserData)
{
    return CheckSuccess(GraphXnamespace::L_TwainAcquireExt (hWnd, pBitmap, uFlags, pfnCallback, pUserData), 
        _T("TwainAcquireExt"));
}

L_INT GRAPHX::TwainEnumSources(HWND hWnd,
							   ENUMSOURCECALLBACK pFunction,
							   L_VOID* pUserData)
{
	return CheckSuccess(GraphXnamespace::L_TwainEnumSources(hWnd, pFunction, pUserData), _T("TwainEnumSources"));
}

L_INT GRAPHX::TwainSelect(HWND hWnd)
{
	return CheckSuccess(GraphXnamespace::L_TwainSelect(hWnd), _T("TwainSelect"));
}

L_INT GRAPHX::TwainSetProps(HWND hWnd, pTWAINPROPERTIES pTwain, L_UINT32 uFlags)
{   
...
L_INT GRAPHX::TwainGetProps(pTWAINPROPERTIES pTwain)
{   
...
void GRAPHX::SetOnErrorThrow(bool onErrorThrow)
{
    staticOnErrorThrow = onErrorThrow;
}

bool GRAPHX::GetOnErrorThrow()
{
    return staticOnErrorThrow;
}

L_INT GRAPHX::CheckSuccess(L_INT res, const char * function)
{
...
CGraphXException::CGraphXException(L_INT       res, 
								   LPCTSTR     function):CException(TRUE)
{
    m_result = res;
...
CString CGraphXException::GetErrorMessage(L_INT error)
{
	...
	case ERROR_OCR_BADPLGN:
		msg = _T("Error #231. Bad Polygon used to define region extents; probably beyond the dimensions of the source image or bad vertices.");
		break;
...
BOOL CGraphXException::GetErrorMessage(LPTSTR      lpszError, 
									   UINT        nMaxError, 
									   PUINT       pnHelpContext)
{
    CString msg;
...
L_INT GRAPHX::SaveBitmapList(LPSTR lpszFile, HBITMAPLIST hList, L_INT nFormat, L_INT nBits, L_INT nQFactor, pSAVEFILEOPTION pSaveOptions)
{
...
HWND GRAPHX::CreateImageListControl(L_UINT32 dwStyle, L_INT x, L_INT y, L_INT nWidth, L_INT nHeight, HWND hWndParent, L_INT nID, COLORREF crBack)
{
	return GraphXnamespace::L_CreateImageListControl(dwStyle, x, y, nWidth, nHeight, hWndParent, nID, crBack);
}

L_INT GRAPHX::ImgListSetSelOptions(HWND hWnd, LILSELOPTION* Opt)
{
...
L_INT GRAPHX::ImgListGetSelOptions(HWND hWnd, LILSELOPTION* Opt)
{
...
L_INT GRAPHX::ImgListClear(HWND hWnd)
{
...
L_INT GRAPHX::ImgListGetItemCount(HWND hWnd)
{
...
L_INT GRAPHX::ImgListInsert(HWND hWnd, LILITEM* pItem)
{
...
L_INT GRAPHX::ImgListInsertAt(HWND hWnd, int nIndex, LILITEM* pItem)
{
...
L_INT GRAPHX::ImgListGetItem(HWND hWnd, L_UINT nIndex, LILITEM* pItem)
{
...
L_INT GRAPHX::ImgListSetItem(HWND hWnd, L_UINT nIndex, LILITEM* pItem)
{
...
L_INT GRAPHX::ImgListGetItemOptions(HWND hWnd, LILITEMOPTION* pOpt)
{
...
L_INT GRAPHX::ImgListSetItemOptions(HWND hWnd, LILITEMOPTION* pOpt)
{
...
L_INT GRAPHX::ImgListRemove(HWND hWnd, int nItem)
{
...
L_INT GRAPHX::ImgListGetSelItems(HWND hWnd, LILITEM* pItems)
{
...
L_INT GRAPHX::ImgListGetSelCount(HWND hWnd)
{
...
L_INT GRAPHX::ImgListEnsureVisible(HWND hWnd, int nItem)
{
	return SendMessage(hWnd, L_ILM_ENSUREVISIBLE, (WPARAM)nItem, (LPARAM)0);
}

L_INT GRAPHX::ImgListGetPage(HWND hWnd)
{
...
L_INT GRAPHX::ImgListGetPagesCount(HWND hWnd)
{
...
L_INT GRAPHX::ImgListHitTest(HWND hWnd, POINT* pt)
{
...
L_INT GRAPHX::SetBitmapRgnHandle(pBITMAPHANDLE pBitmap, pRGNXFORM pXForm, HRGN hRgn, L_UINT uCombineMode)
{
...
L_INT GRAPHX::SetBitmapRgnColor(pBITMAPHANDLE pBitmap, COLORREF crColor, L_UINT uCombineMode)
{
	L_INT res = GraphXnamespace::L_SetBitmapRgnColor(pBitmap, crColor, uCombineMode);
	return CheckSuccess(res, _T("SetBitmapRgnColor"));
}

L_INT GRAPHX::SetBitmapRgnEllipse(pBITMAPHANDLE pBitmap, pRGNXFORM pXForm, RECT L_FAR* pRect, L_UINT uCombineMode)
{
...
L_INT GRAPHX::SetBitmapRgnPolygon(pBITMAPHANDLE pBitmap, pRGNXFORM pXForm, POINT L_FAR* pPoints, L_UINT uPoints, L_UINT uFillMode, L_UINT uCombineMode)
{
...
L_INT GRAPHX::SetBitmapRgnRect(pBITMAPHANDLE pBitmap, pRGNXFORM pXForm, RECT L_FAR* pRect, L_UINT uCombineMode)
{
...
L_INT GRAPHX::SetBitmapRgnRoundRect(pBITMAPHANDLE pBitmap, pRGNXFORM pXForm, RECT L_FAR* pRect, L_INT nWidthEllipse, L_INT nHeightEllipse, L_UINT uCombineMode)
{
...
L_INT GRAPHX::SetBitmapRgnFromMask(pBITMAPHANDLE pBitmap, pRGNXFORM pXForm, pBITMAPHANDLE pMask, L_UINT uCombineMode)
{
...
L_INT GRAPHX::DlgFileSave(HWND hWnd, OPENFILENAME* pOpenFileName, FILESAVEPARM L_FAR* pFileSaveParm, L_UINT32 uFlags, LPFILESAVEFORMAT pSaveFormats, L_INT nSaveFormats, LTCOMMDLGHELPCB pfnCallback, L_VOID L_FAR * pUserData)
{
	L_INT res = GraphXnamespace::L_DlgFileSave(hWnd, pOpenFileName, pFileSaveParm, uFlags, pSaveFormats, nSaveFormats, pfnCallback, pUserData);
	return CheckSuccess(res, _T("DlgFileSave"));
}

L_INT GRAPHX::SetPaintIntensity(pBITMAPHANDLE pBitmap, L_INT nValue)
{
	L_INT res = GraphXnamespace::L_SetPaintIntensity(pBitmap, nValue);
	return CheckSuccess(res, _T("SetPaintIntensity"));
}

L_INT GRAPHX::SetPaintContrast(pBITMAPHANDLE pBitmap, L_INT nValue)
{
	L_INT res = GraphXnamespace::L_SetPaintContrast(pBitmap, nValue);
	return CheckSuccess(res, _T("SetPaintContrast"));
}

L_INT GRAPHX::GetPaintIntensity(pBITMAPHANDLE pBitmap)
{
	return GraphXnamespace::L_GetPaintIntensity(pBitmap);
}

L_INT GRAPHX::GetPaintContrast(pBITMAPHANDLE pBitmap)
{
	return GraphXnamespace::L_GetPaintContrast(pBitmap);
}

L_INT GRAPHX::TrimBitmap(pBITMAPHANDLE pBitmap, L_INT nCol, L_INT nRow, L_UINT uWidth, L_UINT uHeight)
{
	L_INT res = GraphXnamespace::L_TrimBitmap(pBitmap, nCol, nRow, uWidth, uHeight);
	return CheckSuccess(res, _T("TrimBitmap"));
}

L_INT GRAPHX::ChangeBitmapViewPerspective(pBITMAPHANDLE pDstBitmap, pBITMAPHANDLE pSrcBitmap, L_INT ViewPerspective)
{
...
//											  OldSize*dScale.
//
BOOL GRAPHX::SmartResize(BITMAPHANDLE* pBmpSrc, BITMAPHANDLE* pBmpDst, double dScale)
{
	ASSERT(pBmpSrc);
	ASSERT(pBmpDst);
	...
	GRAPHX::CreateBitmap(pBmpDst, TYPE_CONV,
		w, h,											// new sizes
		8, 0, grayscalePal, TOP_LEFT, NULL, 0);
    ...
    for (cy = 0; cy < pBmpDst->Height; cy++)	// for every line of the resulting bitmap
	{
		...
		for (cx = 0; cx < pBmpDst->Width; cx++)	// for every pixel of the line of the resulting bitmap
		{
			...
			*/
			...
			fZeroIsWhite	?	*pDstBits = (BYTE)(255 * (total- whitePixCount)/total)
				:	*pDstBits = (BYTE)(255 * whitePixCount / total);
...
//
L_INT GRAPHX::ConvertFile(LPCTSTR pszSrcFile, LPCTSTR pszDestFile, L_INT nType, L_INT nPage, L_INT nBitsPerPixel, L_INT nQFactor/*=2*/, L_INT nPasses/*=0*/)
{
	if (nPage != -1)
	{
		BITMAPHANDLE hBmp;
...
// Argument         : L_INT nPasses			- quantity of passes (usually in JPEG). Pass 0 to save w/o passes
//
L_INT GRAPHX::ConvertFileMemory(BYTE* pSrcBuf, int nBufSize, LPCTSTR pszDestFile, L_INT nType, L_INT nPage, L_INT nBitsPerPixel, L_INT nQFactor, L_INT nPasses)
{
	if (nPage == -1)
	{
...
//
L_INT GRAPHX::CreateMultipageTIFF(LPCTSTR* pszFilesList, int nSrcFiles, LPCTSTR szDestFile)
{
...
// Argument         : LPCTSTR szDestFile	- name of destination file
//
L_INT GRAPHX::CreateMultipageTIFFMem(BYTE** pSrcBufs, int* pBufSizes, int nSrcFiles, LPCTSTR szDestFile)
{
	BITMAPHANDLE hBmp;
...
L_INT GRAPHX::CaptureArea(pBITMAPHANDLE pBitmap)
{
...
L_INT GRAPHX::ConvertFromDIB(pBITMAPHANDLE pBitmap,
							 BITMAPINFO* pInfo,
							 L_UCHAR* pBits);

L_UINT* GRAPHX::CreateUserMatchTable(LPRGBQUAD pPalette,
									 L_UINT uColors);


L_INT GRAPHX::DecompressBitmapMemory(L_CHAR* pBuffer,
									 pBITMAPHANDLE pBitmap,
									 L_INT nBitsPerPixel,
									 L_INT32 nBufferSize);
...
L_INT GRAPHX::DitherLine(pBITMAPHANDLE pBitmap,
						 L_UCHAR* pBufferSrc,
						 L_UCHAR* pBufferDst);


L_INT GRAPHX::EmbossBitmap(pBITMAPHANDLE pBitmap,
						   L_UINT uDirection,
						   L_UINT uDepth);
...
L_INT GRAPHX::GammaCorrectBitmap(pBITMAPHANDLE pBitmap,
								 L_UINT uGamma);


L_INT GRAPHX::GetBitmapHistogram(pBITMAPHANDLE pBitmap,
								 L_UINT32* pHistogram,
								 L_UINT uChannel);

L_INT32 GRAPHX::GetBitmapRow(pBITMAPHANDLE pBitmap,
							 L_UCHAR* pBuffer,
							 L_INT nRow,
							 L_UINT32 uBytes);


L_INT GRAPHX::GetComment(L_UINT uType,
						 L_UCHAR* pComment,
						 L_UINT uLength);

L_INT GRAPHX::GetCompressFileStamp(L_CHAR* pszFile,
								   pBITMAPHANDLE pBitmap);

L_UINT32 GRAPHX::GetDisplayMode(L_VOID);

L_INT GRAPHX::GetFixedPalette(LPRGBQUAD pPalette,
							  L_INT nBitsPerPixel);

L_INT GRAPHX::GetPCDResolution(L_CHAR* pszFile,
							   pPCDINFO pPCDInfo);


L_INT GRAPHX::HistoContrastBitmap(pBITMAPHANDLE pBitmap,
								  L_INT nChange);

L_INT GRAPHX::HistoEqualizeBitmap(pBITMAPHANDLE pBitmap);


L_INT GRAPHX::IntensityDetectBitmap(pBITMAPHANDLE pBitmap,
									L_UINT uLow,
									L_UINT uHigh);
...
L_INT GRAPHX::MedianFilterBitmap(pBITMAPHANDLE pBitmap,
								 L_UINT uDim);

L_INT GRAPHX::MosaicBitmap(pBITMAPHANDLE pBitmap,
						   L_UINT uDim);
...
L_INT GRAPHX::PosterizeBitmap(pBITMAPHANDLE pBitmap,
							  L_UINT uLevels);

HDC GRAPHX::PrintBitmap(pBITMAPHANDLE pBitmap,
						L_INT nX,
						L_INT nY,
						L_INT nWidth,
						L_INT nHeight,
...
HDC GRAPHX::PrintBitmapFast(HDC hDC,
							pBITMAPHANDLE pBitmap,
							L_INT nXPos, 
							L_INT nYPos,
							L_INT nWidth, 
...
L_INT GRAPHX::ReadFileComment(L_CHAR* pszFile,
							  L_UINT uType,
							  L_UCHAR* pComment,
							  L_UINT uLength);


L_INT GRAPHX::RemapBitmapIntensity(pBITMAPHANDLE pBitmap,
								   L_UCHAR* pLUT,
								   L_UINT uChannel);

L_INT GRAPHX::ResampleBitmap(pBITMAPHANDLE pBitmap,
							 L_INT nWidth,
							 L_INT nHeight);

L_INT GRAPHX::Resize(L_UCHAR* pBuffer,
					 L_INT nRow,
					 L_INT nBitsPerPixel,
					 L_INT* pXSize,
					 L_INT* pYSize,
...
L_INT GRAPHX::ScreenCaptureBitmap(HDC hDC,
								  pBITMAPHANDLE pBitmap,
								  LPRECT pRect);

L_INT GRAPHX::SetComment(L_UINT uType,
						 L_UCHAR* pComment,
						 L_UINT uLength);
...
L_INT GRAPHX::SharpenBitmap(pBITMAPHANDLE pBitmap,
							L_INT nSharpness);

L_INT GRAPHX::ShearBitmap(pBITMAPHANDLE pBitmapSrc,
						  L_INT nAngle,
						  L_BOOL fHorizontal,
						  COLORREF crFill);


L_INT GRAPHX::SpatialFilterBitmap(pBITMAPHANDLE pBitmap,
...
L_INT GRAPHX::StartDithering(pBITMAPHANDLE pBitmap,
							 LPRGBQUAD pPalette,
							 L_UINT uColors);

L_INT GRAPHX::StartResize (L_INT nOldWidth,
						   L_INT nOldHeight,
						   L_INT nNewWidth,
						   L_INT nNewHeight,
						   L_VOID** ppResizeData);
...
L_INT GRAPHX::UnderlayBitmap(pBITMAPHANDLE pBitmapDst,
