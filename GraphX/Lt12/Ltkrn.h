/*[]=====================================================================[]*/
/*[]     GraphX Run Time Library - Version 9                          []*/
/*[]                                                                     []*/
/*[]                                                                     []*/
...
#define ISROTATED(pBitmap)                                                                                                                 \
    ((pBitmap)->ViewPerspective == TOP_LEFT90 || (pBitmap)->ViewPerspective == TOP_LEFT270 ||                                              \
     (pBitmap)->ViewPerspective == BOTTOM_LEFT90 || (pBitmap)->ViewPerspective == BOTTOM_LEFT270)
#define ISFLIPPED(pBitmap)                                                                                                                 \
    ((pBitmap)->ViewPerspective == BOTTOM_LEFT || (pBitmap)->ViewPerspective == BOTTOM_LEFT90 ||                                           \
     (pBitmap)->ViewPerspective == BOTTOM_LEFT180 || (pBitmap)->ViewPerspective == BOTTOM_LEFT270)
...
L_INT EXT_FUNCTION L_AllocateBitmap(pBITMAPHANDLE pBitmap, L_INT nMemoryType);

L_INT EXT_FUNCTION L_ChangeBitmapHeight(pBITMAPHANDLE pBitmap, L_INT nHeight);

L_INT EXT_FUNCTION L_ChangeBitmapViewPerspective(pBITMAPHANDLE pDstBitmap, pBITMAPHANDLE pSrcBitmap, L_INT ViewPerspective);

L_INT EXT_FUNCTION L_ChangeFromDIB(pBITMAPHANDLE pBitmap, HGLOBAL hDIB);
...
L_INT EXT_FUNCTION
L_ColorResBitmapList(HBITMAPLIST hList, L_INT nBitsPerPixel, L_UINT32 uFlags, LPRGBQUAD pPalette, HPALETTE hPalette, L_UINT uColors);
...
L_INT EXT_FUNCTION L_CompressRow(L_UINT16 L_HUGE* pRunBuffer, L_UCHAR L_HUGE* pBuffer, L_INT nCol, L_INT nWidth);

L_INT EXT_FUNCTION L_CompressRows(L_UINT16 L_HUGE* pRunBuffer, L_UCHAR L_HUGE* pBuffer, L_UINT nWidth, L_UINT nRows);
...
L_INT EXT_FUNCTION L_ConvertFromDIB(pBITMAPHANDLE pBitmap, BITMAPINFO L_FAR* pInfo, L_UCHAR L_FAR* pBits);
...
L_INT EXT_FUNCTION L_CopyBitmap(pBITMAPHANDLE pBitmapDst, pBITMAPHANDLE pBitmapSrc);

L_INT EXT_FUNCTION L_CopyBitmapData(pBITMAPHANDLE pBitmapDst, pBITMAPHANDLE pBitmapSrc);

L_VOID EXT_FUNCTION L_CopyBitmapHandle(pBITMAPHANDLE pBitmapDst, pBITMAPHANDLE pBitmapSrc);

L_INT EXT_FUNCTION L_CopyBitmapListItems(pHBITMAPLIST phList, HBITMAPLIST hList, L_UINT uIndex, L_UINT uCount);

L_INT EXT_FUNCTION
L_CopyBitmapRect(pBITMAPHANDLE pBitmapDst, pBITMAPHANDLE pBitmapSrc, L_INT nCol, L_INT nRow, L_UINT uWidth, L_UINT uHeight);
...
L_UINT L_FAR* EXT_FUNCTION L_CreateUserMatchTable(LPRGBQUAD pPalette, L_UINT uColors);

L_INT EXT_FUNCTION L_DefaultDithering(L_UINT uMethod);

L_INT EXT_FUNCTION L_DeleteBitmapListItems(HBITMAPLIST hList, L_UINT uIndex, L_UINT uCount);

L_INT EXT_FUNCTION L_DestroyBitmapList(HBITMAPLIST hList);

L_BOOL EXT_FUNCTION L_DeleteLeadDC(HDC hDC);

L_INT EXT_FUNCTION L_DitherLine(pBITMAPHANDLE pBitmap, L_UCHAR L_FAR* pBufferSrc, L_UCHAR L_FAR* pBufferDst);

HPALETTE EXT_FUNCTION L_DupPalette(HPALETTE hPalette);

L_INT EXT_FUNCTION L_ExpandRow(L_UINT16 L_HUGE* pRunBuffer, L_UCHAR L_HUGE* pBuffer, L_UINT nCol, L_INT nWidth);

L_INT EXT_FUNCTION L_ExpandRows(L_UINT16 L_HUGE* pRun, L_UCHAR L_HUGE* pBuffer, L_UINT nWidth, L_UINT nRows);

L_INT EXT_FUNCTION L_FillBitmap(pBITMAPHANDLE pBitmap, COLORREF crFill);
...
L_INT EXT_FUNCTION L_GetBitmapAlpha(pBITMAPHANDLE pBitmap, pBITMAPHANDLE pAlpha);

L_INT EXT_FUNCTION L_GetBitmapColors(pBITMAPHANDLE pBitmap, L_INT nIndex, L_INT nCount, LPRGBQUAD pPalette);

L_INT EXT_FUNCTION L_GetBitmapListCount(HBITMAPLIST hList, L_UINT L_FAR* puCount);

L_INT EXT_FUNCTION L_GetBitmapListItem(HBITMAPLIST hList, L_UINT uIndex, pBITMAPHANDLE pBitmap);

L_INT32 EXT_FUNCTION L_GetBitmapRow(pBITMAPHANDLE pBitmap, L_UCHAR L_FAR* pBuffer, L_INT nRow, L_UINT32 uBytes);

L_INT32 EXT_FUNCTION L_GetBitmapRowCol(pBITMAPHANDLE pBitmap, L_UCHAR L_FAR* pBuffer, L_INT nRow, L_INT nCol, L_UINT32 uBytes);
...
L_INT EXT_FUNCTION
L_GetBitmapRowCompressed(pBITMAPHANDLE pBitmap, L_UCHAR L_FAR* pWorkBuffer, L_UINT16 L_FAR* pRunBuffer, L_UINT nRow, L_UINT nLines);

L_INT EXT_FUNCTION L_GetFixedPalette(LPRGBQUAD pPalette, L_INT nBitsPerPixel);

COLORREF EXT_FUNCTION L_GetPixelColor(pBITMAPHANDLE pBitmap, L_INT nRow, L_INT nCol);

STATUSCALLBACK EXT_FUNCTION L_GetStatusCallBack(L_VOID L_FAR* L_FAR* ppUserData);

L_INT EXT_FUNCTION L_GrayScaleBitmap(pBITMAPHANDLE pBitmap, L_INT nBitsPerPixel);

L_INT EXT_FUNCTION L_InitBitmap(pBITMAPHANDLE pBitmap, L_INT nWidth, L_INT nHeight, L_INT nBitsPerPixel);

L_INT EXT_FUNCTION L_InsertBitmapListItem(HBITMAPLIST hList, L_UINT uIndex, pBITMAPHANDLE pBitmap);

L_INT EXT_FUNCTION L_IsGrayScaleBitmap(pBITMAPHANDLE pBitmap);

L_BOOL EXT_FUNCTION L_IsSupportLocked(L_UINT uType);

L_INT EXT_FUNCTION L_PointFromBitmap(pBITMAPHANDLE pBitmap, L_INT ViewPerspective, L_INT L_FAR* px, L_INT L_FAR* py);

L_INT EXT_FUNCTION L_PointToBitmap(pBITMAPHANDLE pBitmap, L_INT ViewPerspective, L_INT L_FAR* px, L_INT L_FAR* py);

L_INT EXT_FUNCTION L_PutBitmapColors(pBITMAPHANDLE pBitmap, L_INT nIndex, L_INT nCount, LPRGBQUAD pPalette);

L_INT32 EXT_FUNCTION L_PutBitmapRow(pBITMAPHANDLE pBitmap, L_UCHAR L_FAR* pBuffer, L_INT nRow, L_UINT32 uBytes);

L_INT32 EXT_FUNCTION L_PutBitmapRowCol(pBITMAPHANDLE pBitmap, L_UCHAR L_FAR* pBuffer, L_INT nRow, L_INT nCol, L_UINT32 uBytes);
...
L_INT EXT_FUNCTION
L_PutBitmapRowCompressed(pBITMAPHANDLE pBitmap, L_UCHAR L_FAR* pWorkBuffer, L_UINT16 L_FAR* pRunBuffer, L_UINT nRow, L_UINT nLines);

L_INT EXT_FUNCTION L_PutPixelColor(pBITMAPHANDLE pBitmap, L_INT nRow, L_INT nCol, COLORREF crColor);

L_INT EXT_FUNCTION L_RectFromBitmap(pBITMAPHANDLE pBitmap, L_INT ViewPerspective, LPRECT lprc);

L_INT EXT_FUNCTION L_RectToBitmap(pBITMAPHANDLE pBitmap, L_INT nViewPerspective, LPRECT lprc);
...
L_INT EXT_FUNCTION L_RemoveBitmapListItem(HBITMAPLIST hList, L_UINT uIndex, pBITMAPHANDLE pBitmap);

L_INT EXT_FUNCTION
L_Resize(L_UCHAR L_FAR* pBuffer, L_INT nRow, L_INT nBitsPerPixel, L_INT L_FAR* pXSize, L_INT L_FAR* pYSize, L_VOID L_FAR* pResizeData);

L_INT EXT_FUNCTION L_ResizeBitmap(pBITMAPHANDLE pBitmapSrc, pBITMAPHANDLE pBitmapDst);

L_INT EXT_FUNCTION L_ReverseBitmap(pBITMAPHANDLE pBitmap);

L_INT EXT_FUNCTION L_RotateBitmap(pBITMAPHANDLE pBitmap, L_INT32 nAngle, L_UINT uFlags, COLORREF crFill);

L_INT EXT_FUNCTION L_RotateBitmapViewPerspective(pBITMAPHANDLE pBitmap, L_INT nAngle);

L_INT EXT_FUNCTION L_SetBitmapAlpha(pBITMAPHANDLE pBitmap, pBITMAPHANDLE pAlpha);

L_INT EXT_FUNCTION L_SetBitmapDataPointer(pBITMAPHANDLE pBitmap, L_UCHAR L_FAR* pData, L_UINT32 dwSize);

L_INT EXT_FUNCTION L_SetBitmapListItem(HBITMAPLIST hList, L_UINT uIndex, pBITMAPHANDLE pBitmap);

L_INT EXT_FUNCTION L_SetHotKeyCallback(L_INT nHotKey, L_UINT uHotKeyModifiers, HOTKEYCALLBACK pfnHotKeyCallback, L_VOID L_FAR* pUserData);

STATUSCALLBACK EXT_FUNCTION L_SetStatusCallBack(STATUSCALLBACK pfnCallback, L_VOID L_FAR* pUserData);

L_UINT L_FAR* EXT_FUNCTION L_SetUserMatchTable(L_UINT L_FAR* pTable);

L_INT EXT_FUNCTION L_ShearBitmap(pBITMAPHANDLE pBitmap, L_INT nAngle, L_BOOL fHorizontal, COLORREF crFill);

L_INT EXT_FUNCTION L_SizeBitmap(pBITMAPHANDLE pBitmap, L_INT nWidth, L_INT nHeight, L_UINT32 uFlags);

L_INT EXT_FUNCTION L_StartDithering(pBITMAPHANDLE pBitmap, LPRGBQUAD pPalette, L_UINT uColors);

L_INT EXT_FUNCTION L_StartResize(L_INT nOldWidth, L_INT nOldHeight, L_INT nNewWidth, L_INT nNewHeight, L_VOID L_FAR* L_FAR* ppResizeData);

L_INT EXT_FUNCTION L_StopDithering(pBITMAPHANDLE pBitmap);

L_INT EXT_FUNCTION L_StopResize(L_VOID L_FAR* pResizeData);

COLORREF EXT_FUNCTION L_TranslateBitmapColor(pBITMAPHANDLE pBitmapDst, pBITMAPHANDLE pBitmapSrc, COLORREF crColor);

L_INT EXT_FUNCTION L_ToggleBitmapCompression(pBITMAPHANDLE pBitmap);

L_INT EXT_FUNCTION L_TrimBitmap(pBITMAPHANDLE pBitmap, L_INT nCol, L_INT nRow, L_UINT uWidth, L_UINT uHeight);
...
L_INT EXT_FUNCTION L_GetResizedRowCol(L_VOID L_FAR* pResizeData, L_UCHAR L_FAR* pBuffer, L_INT nRow, L_INT nCol, L_INT32 uBytes);

L_INT EXT_FUNCTION L_StopResizeBitmap(L_VOID L_FAR* pResizeData);

L_INT EXT_FUNCTION L_MoveBitmapListItems(pHBITMAPLIST phList, HBITMAPLIST hList, L_UINT uIndex, L_UINT uCount);

L_BOOL EXT_FUNCTION L_InitGRAPHX(HINSTANCE hInstance);
L_VOID EXT_FUNCTION L_QuitGRAPHX();
