...
SHORT
CGifCoder::Decoder(SHORT linewidth, INT& bad_code_count)
{
...
//
int
CGifCoder::OutLine(byte* pixels, int LineLen)
{
    int pos;
    if (m_IsInterlaced) {
...
//
int
CGifCoder::OpenPicture(LPCTSTR fname)
{
...
//
int
CGifCoder::OpenPictureMemory(BYTE* buf, DWORD len)
{
    m_BufPos     = 0;
    m_Buf        = buf;
    m_FileLength = len;
...
void
CGifCoder::GetTotalSize(int& cx, int& cy)
{
    cx = dscgif.scrwidth;
    cy = dscgif.scrheight;
}
...
//
L_INT
CGifCoder::LoadBitmap(L_CHAR L_FAR* pszFile, pBITMAPHANDLE pBitmap, L_INT nBitsPerPixel, L_INT /*nOrder*/, pLOADFILEOPTION pLoadOptions)
{
    try {
        int nErr;
...
//
L_INT
CGifCoder::LoadBitmapMemory(L_CHAR L_FAR* pBuffer,
...
//
L_INT
CGifCoder::LoadFile(L_CHAR* pszFile,
                    pBITMAPHANDLE pBitmap,
                    L_INT nBitsPerPixel,
                    L_INT /*nOrder*/,
                    L_UINT /*uFlags*/,
...
//
L_INT
CGifCoder::LoadMemory(L_CHAR L_FAR* pBuffer,
                      pBITMAPHANDLE pBitmap,
                      L_INT nBitsPerPixel,
                      L_INT /*nOrder*/,
                      L_UINT /*uFlags*/,
...
//
void
CGifCoder::CreateLTBitmap(pBITMAPHANDLE pBitmap, L_INT nBitsPerPixel)
{
    int nErr;
    ...
    nErr = GraphXnamespace::L_CreateBitmap(pBitmap,
                                           TYPE_CONV,
                                           images[nPage].cx,
                                           images[nPage].cy,
                                           images[nPage].nRes,
                                           ORDER_BGR,
                                           (LPRGBQUAD)images[nPage].ColTable,
                                           TOP_LEFT,
                                           NULL,
                                           0);
    if (nErr != SUCCESS)
...
*/
    ...
    if (nBitsPerPixel != 0 && nBitsPerPixel != nRes) {
        nErr = GraphXnamespace::L_ColorResBitmap(pBitmap, pBitmap, nBitsPerPixel, CRF_OPTIMIZEDPALETTE, NULL, NULL, NULL, NULL, NULL);
        if (nErr != SUCCESS)
            throw nErr;
    }
}
