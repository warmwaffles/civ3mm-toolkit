...
public:
	...
	static HWND CreateImageListControl(L_UINT32 dwStyle, L_INT x, L_INT y, L_INT nWidth, L_INT nHeight, HWND hWndParent, L_INT nID, COLORREF crBack);
	...
	static L_INT LoadBitmapList(LPSTR lpszFile, HBITMAPLIST* phList, L_INT nBitsTo, L_INT nColorOrder, pLOADFILEOPTION pLoadOptions, pFILEINFO pFileInfo);
	static L_INT SaveBitmapList(LPSTR lpszFile, HBITMAPLIST hList, L_INT nFormat, L_INT nBits = 0, L_INT nQFactor = 2, pSAVEFILEOPTION pSaveOptions = NULL);
	...
	static L_INT ColorSeparateBitmap(pBITMAPHANDLE pBitmap,
	...
	static L_INT ConvertColorSpace(L_UCHAR  *pBufferSrc,
		L_UCHAR  *pBufferDst,
		L_INT nWidth,
		L_INT nFormatSrc,
		L_INT nFormatDst);
	
	static L_INT ConvertFromDDB(HDC hDC,
		pBITMAPHANDLE pBitmap,
		HBITMAP hBitmap,
		HPALETTE hPalette);
	
	static L_INT ConvertFromDIB(pBITMAPHANDLE pBitmap,
		BITMAPINFO* pInfo,
		L_UCHAR* pBits);
	
	static HBITMAP ConvertToDDB(HDC hDC,
		pBITMAPHANDLE pBitmap);
	
	static HANDLE ConvertToDIB(pBITMAPHANDLE pBitmap);
	
	static L_INT CopyBitmap(pBITMAPHANDLE pBitmapDst,
		pBITMAPHANDLE pBitmapSrc);
	
	static L_INT CopyBitmapData(pBITMAPHANDLE pBitmapDst,
		pBITMAPHANDLE pBitmapSrc);
	
	static L_VOID CopyBitmapHandle(pBITMAPHANDLE pBitmapDst,
		pBITMAPHANDLE pBitmapSrc);
	...
	static L_BOOL CopyFromClipboard(HWND hWnd,
		pBITMAPHANDLE pBitmap);
	
	static L_BOOL CopyToClipboard(HWND hWnd,
		pBITMAPHANDLE pBitmap);
	
	static L_INT CopyToClipboardExt(HWND hWnd,
		pBITMAPHANDLE pBitmap,
		L_UINT uFlags);
	...
	static L_UINT* CreateUserMatchTable(LPRGBQUAD pPalette,
		L_UINT uColors);
	
	static HPALETTE CreatePaintPalette(HDC hDC,
		pBITMAPHANDLE pBitmap);
	
	static L_INT DecompressBitmapMemory(L_CHAR* pBuffer,
		pBITMAPHANDLE pBitmap,
		L_INT nBitsPerPixel,
		L_INT32 nBufferSize);
	...
	static L_INT DitherLine(pBITMAPHANDLE pBitmap,
		L_UCHAR  *pBufferSrc,
		L_UCHAR  *pBufferDst);
	...
	static L_INT FileInfo(const L_CHAR* pszFile,
		pFILEINFO		pFileInfo,
		L_UINT			flags,
		pLOADFILEOPTION	pLoadOptions);
	
	static L_INT FileInfo(  int         hFile, 
		long        nStart, 
		long        nLength, 
		pFILEINFO   pFileInfo,
		L_UINT      flags);
	
	static L_INT FileInfoMemory(
        L_CHAR  *		pBuffer,
        pFILEINFO		pFileInfo,
        L_INT32			nBufferSize,
        L_UINT			flags,
	...
	static L_INT FillBitmap(pBITMAPHANDLE pBitmap,
		COLORREF crFill);
	...
	static L_INT GammaCorrectBitmap(pBITMAPHANDLE pBitmap,
		L_UINT uGamma);
	
	static L_INT GetBitmapColors(pBITMAPHANDLE pBitmap,
		L_INT nIndex,
		L_INT nCount,
		LPRGBQUAD pPalette);
	
	static L_INT GetBitmapHistogram(pBITMAPHANDLE pBitmap,
		L_UINT32* pHistogram,
		L_UINT uChannel);
	
	static L_INT32 GetBitmapRow(pBITMAPHANDLE pBitmap,
		L_UCHAR* pBuffer,
		L_INT nRow,
		L_UINT32 uBytes);
	
	static L_INT32 GetBitmapRowCol(pBITMAPHANDLE pBitmap,
		L_UCHAR* pBuffer,
		L_INT nRow,
		L_INT nCol,
		L_UINT32 uBytes);
	
	static L_INT GetComment(L_UINT uType,
		L_UCHAR* pComment,
		L_UINT uLength);
	
	static L_INT GetCompressFileStamp(L_CHAR* pszFile,
		pBITMAPHANDLE pBitmap);
	
	static L_UINT32 GetDisplayMode(L_VOID);
	
	static L_INT GetFixedPalette(LPRGBQUAD pPalette,
		L_INT nBitsPerPixel);
	
	static L_INT GetPCDResolution(L_CHAR* pszFile,
		pPCDINFO pPCDInfo);
	
	static COLORREF GetPixelColor(pBITMAPHANDLE pBitmap,
		L_INT nRow,
		L_INT nCol);
	...
	static L_INT HalfToneBitmap(pBITMAPHANDLE pBitmap,
		L_UINT uType);
	
	static L_INT HalfToneBitmapExt(pBITMAPHANDLE pBitmap,
		L_UINT uType,
		L_INT32 nAngle);
	
	static L_INT HistoContrastBitmap(pBITMAPHANDLE pBitmap,
		L_INT nChange);
	
	static L_INT HistoEqualizeBitmap(pBITMAPHANDLE pBitmap);
	
	static L_INT InitBitmap(pBITMAPHANDLE pBitmap,
		L_INT nWidth,
		L_INT nHeight,
		L_INT nBitsPerPixel);
	
	static L_INT IntensityDetectBitmap(pBITMAPHANDLE pBitmap,
		L_UINT uLow,
		L_UINT uHigh);
	...
	static L_INT MedianFilterBitmap(pBITMAPHANDLE pBitmap,
		L_UINT uDim);
	
	static L_INT MosaicBitmap(pBITMAPHANDLE pBitmap,
		L_UINT uDim);
	
	static L_INT PaintDC( HDC hDC,
		pBITMAPHANDLE pBitmap,
		LPRECT pSrc,
		LPRECT pClipSrc,
		LPRECT pDst,
	...
	static L_INT PaintRgnDC(HDC hDC,
		pBITMAPHANDLE pBitmap,
		LPRECT pSrc,
		LPRECT pClipSrc,
		LPRECT pDst,
	...
	static L_INT PosterizeBitmap(pBITMAPHANDLE pBitmap,
		L_UINT uLevels);
	
	static HDC PrintBitmap(pBITMAPHANDLE pBitmap,
		L_INT nX,
		L_INT nY,
		L_INT nWidth,
		L_INT nHeight,
	...
	static HDC PrintBitmapExt(HDC hDC,
		pBITMAPHANDLE pBitmap,
		L_INT nX,
		L_INT nY,
		L_INT nWidth,
	...
	static HDC PrintBitmapFast(HDC hDC,
		pBITMAPHANDLE pBitmap,
		L_INT nXPos, 
		L_INT nYPos,
		L_INT nWidth, 
	...
	static L_INT PutBitmapColors(pBITMAPHANDLE pBitmap,
		L_INT nIndex,
		L_INT nCount,
		LPRGBQUAD pPalette);
	
	static L_INT32 PutBitmapRow(pBITMAPHANDLE pBitmap,
		L_UCHAR* pBuffer,
		L_INT nRow,
		L_UINT32 uBytes);
	
	static L_INT32 PutBitmapRowCol(pBITMAPHANDLE pBitmap,
		L_UCHAR* pBuffer,
		L_INT nRow,
		L_INT nCol,
		L_UINT32 uBytes);
	
	static L_INT PutPixelColor(pBITMAPHANDLE pBitmap,
		L_INT nRow,
		L_INT nCol,
		COLORREF crColor);
	
	static L_INT ReadFileComment(L_CHAR* pszFile,
		L_UINT uType,
		L_UCHAR* pComment,
		L_UINT uLength);
	...
	static L_INT RemapBitmapIntensity(pBITMAPHANDLE pBitmap,
		L_UCHAR* pLUT,
		L_UINT uChannel);
	
	static L_INT ResampleBitmap(pBITMAPHANDLE pBitmap,
		L_INT nWidth,
		L_INT nHeight);
	
	static L_INT Resize(L_UCHAR* pBuffer,
		L_INT nRow,
		L_INT nBitsPerPixel,
		L_INT* pXSize,
		L_INT* pYSize,
	...
	static L_INT ResizeBitmap(pBITMAPHANDLE pBitmapSrc,
		pBITMAPHANDLE pBitmapDst);
	
	static L_INT ReverseBitmap(pBITMAPHANDLE pBitmap);
	
	static L_INT RotateBitmap(pBITMAPHANDLE pBitmap,
		L_INT nAngle,
		L_UINT uFlags,
		COLORREF crFill);
	...
	static L_INT ScreenCaptureBitmap(HDC hDC,
		pBITMAPHANDLE pBitmap,
		LPRECT pRect);
	
	static L_INT SetComment(L_UINT uType,
		L_UCHAR* pComment,
		L_UINT uLength);
	
	static L_UINT32 SetDisplayMode(L_UINT32 uFlagPos,
		L_UINT32 uFlagSet);
	...
	static L_INT SharpenBitmap(pBITMAPHANDLE pBitmap,
		L_INT nSharpness);
	
	static L_INT ShearBitmap(pBITMAPHANDLE pBitmapSrc,
		L_INT         nAngle,
		L_BOOL        fHorizontal,
		COLORREF      crFill);
	
	static L_INT SizeBitmap(pBITMAPHANDLE pBitmap,
		L_INT nWidth,
		L_INT nHeight);
	
	static L_INT SizeBitmapExt(pBITMAPHANDLE pBitmap,
		L_INT nWidth,
		L_INT nHeight,
		L_UINT32 uFlags);
	
	static L_INT SpatialFilterBitmap(pBITMAPHANDLE pBitmap,
	...
	static L_INT StartDithering(pBITMAPHANDLE pBitmap,
		LPRGBQUAD pPalette,
		L_UINT uColors);
	
	static L_INT StartResize(L_INT nOldWidth,
		L_INT nOldHeight,
		L_INT nNewWidth,
		L_INT nNewHeight,
		L_VOID** ppResizeData);
	...
	static L_INT TwainAcquire(HWND hWnd,
		pBITMAPHANDLE pBitmap);
	
	static L_INT TwainAcquireExt(HWND hWnd,
		pBITMAPHANDLE pBitmap,
		L_UINT uFlags,
		TWAINCALLBACK pfnCallback,
		L_VOID* pUserData);
	
	static L_INT TwainEnumSources(HWND hWnd,
	...
	static L_INT UnderlayBitmap(pBITMAPHANDLE pBitmapDst,
	...
	static L_INT SetBitmapRgnPolygon(pBITMAPHANDLE pBitmap, pRGNXFORM pXForm, POINT L_FAR* pPoints, L_UINT uPoints, L_UINT uFillMode, L_UINT uCombineMode);
	static L_INT SetBitmapRgnRect(pBITMAPHANDLE pBitmap, pRGNXFORM pXForm, RECT L_FAR* pRect, L_UINT uCombineMode);
	static L_INT SetBitmapRgnRoundRect(pBITMAPHANDLE pBitmap, pRGNXFORM pXForm, RECT L_FAR* pRect, L_INT nWidthEllipse, L_INT nHeightEllipse, L_UINT uCombineMode);
	...
	static L_INT DlgFileOpen(HWND hWnd, OPENFILENAME* pOpenFileName, FILEOPENPARM L_FAR *pFileOpenParm, L_UINT32 uFlags, LTCOMMDLGHELPCB pfnCallback, L_VOID L_FAR* pUserData);
	static L_INT DlgFileSave(HWND hWnd, OPENFILENAME* pOpenFileName, FILESAVEPARM L_FAR *pFileSaveParm, L_UINT32 uFlags, LPFILESAVEFORMAT pSaveFormats, L_INT nSaveFormats, LTCOMMDLGHELPCB pfnCallback, L_VOID L_FAR * pUserData);
	...
	static L_INT ConvertFile(LPCTSTR pszSrcFile, LPCTSTR pszDestFile, L_INT nType, L_INT nPage, L_INT nBitsPerPixel= 0, L_INT nQFactor= 2, L_INT nPasses= 0);
	static L_INT ConvertFileMemory(BYTE* pSrcBuf, int nBufSize, LPCTSTR pszDestFile, L_INT nType, L_INT nPage, L_INT nBitsPerPixel= 0, L_INT nQFactor= 2, L_INT nPasses= 0);
...
public: 
    ...
    // get private members
    L_INT   GetLastError()      const { return m_result; }
    LPCTSTR GetErrorFunction()  const { return m_function; }
