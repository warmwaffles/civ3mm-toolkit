...
void CFramePosition::SavePosition(CWnd* pWnd)
{
	WINDOWPLACEMENT wp;
	wp.length = sizeof(wp);
...
void CFramePosition::LoadPosition(CWnd* pWnd, BOOL bMax)
{
	ASSERT(pWnd);
	ASSERT(AfxGetApp());
	...
	if ((wp.rcNormalPosition.left  != 0) || (wp.rcNormalPosition.top    != 0) ||
		(wp.rcNormalPosition.right != 0) || (wp.rcNormalPosition.bottom != 0))
		pWnd->SetWindowPlacement(&wp);
}

LPCTSTR CFramePosition::RegKey()
{
	return "";
}
...
void CNoSizePosition::SavePosition(CWnd* pWnd)
{
	ASSERT(pWnd);
	ASSERT(AfxGetApp());
...
void CNoSizePosition::LoadPosition(CWnd* pWnd)
{
	ASSERT(pWnd);
	ASSERT(AfxGetApp());
...
LPCTSTR CNoSizePosition::RegKey()
{
	return "";
}