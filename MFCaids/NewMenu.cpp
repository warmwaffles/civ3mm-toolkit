...
This new menu works under Windows 95/98/Me and Windows NT4.0/2000/XP.
...
The easiest way is to replace all CDialog, CFrameWnd, CMDIFrameWnd, CMDIChildWnd, CMiniFrameWnd and CMultiDocTemplate with the new classes CNewDialog, CNewFrameWnd, CNewMDIFrameWnd, CNewMDIChildWnd, CNewMiniFrameWnd and CNewMultiDocTemplate in your project. 
In the application derived from CWinApp's implementation, set the menustyle just before you create the frame or a dialog window in the function InitInstance. 

  // Set drawing style of all menus to XP-Mode or STYLE_ORIGINAL
  CNewMenu::SetMenuDrawMode(CNewMenu::STYLE_XP);

When you have a multi document template, add the loading code for menu icons just after creating the template. 
...
Insert at the end of OnCreate function of the MainFrame the loading code for the icons. For example: 
  
  m_DefaultNewMenu.LoadToolBar(IDR_MAINFRAME);

Finally, don't forget to include the definition from the NewMenu.h in stdafx.h and add NewMenu.cpp to the project. 
...
In the application set the menustyle before you create the dialog in the function InitInstance. 
  
  // Set drawing style of all menus to XP-Mode
  CNewMenu::SetMenuDrawMode(CNewMenu::STYLE_XP);

Change all occurances of CDialog to CNewDialog in your Project. 
For loading bitmaps in to the menu the best place is in OnInitDialog function after calling the base class function. 
...
Don't forget to include the the definition from the NewMenu.h in stdafx.h and add NewMenu.cpp to the project. 
...
Normally you should overwrite measureitem, menuchar and initmenupopup. For this purpose I use the CNewFrame template class. So you can replace CYourSpecialBase with CNewFrame<CYourSpecialBase> in your project. 
Don't change it in IMPLEMENT_DYNAMIC, IMPLEMENT_SERIAL or IMPLEMENT_DYNACREATE to sub-class the new one. 
...
For loading bitmaps in to the menu, you can add the code in OnCreate function. 
Finally don't forget to include the the definition from the NewMenu.h in stdafx.h and add NewMenu.cpp to the project. 
How to Use the CNewMenu in an MDI Application
Change in the the application's base class in the BOOL InitInstance() function from CMultiDocTemplate to CNewMultiDocTemplate: 
...
Now insert the following code for loading additional bitmaps from a toolbar into the menus. 
...
Change the base class from the main frame class (CMainFrame) from CMDIFrameWnd to CNewMDIFrame in the definition and implementation file. 
...
Add at the end of int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) for loading bitmap in the default menu: 

  m_DefaultNewMenu.LoadToolBar(IDR_MAINFRAME);

Finally, change the base class of the child frame from CMDIChildWnd to CNewMDIChildWnd. Do the same for all derived classes. 
Don't forget to include the the definition from the NewMenu.h in stdafx.h and add NewMenu.cpp to the project. 
...
Change all occurances of CDialog to CNewDialog in your Project. 
Change the base class from the main frame class (CMainFrame) from CFrameWnd to CNewFrameWnd in definition and implementation file. 
Add at the end of int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) for loading bitmap in the default menu: 
...
static LOGFONT MENU_USER_FONT = { 20,
                                  0,
                                  0,
                                  0,
                                  FW_NORMAL,
...
// Helpers for casting
__inline HMENU
UIntToHMenu(const unsigned int ui)
{
    return ((HMENU)(UINT_PTR)ui);
}

__inline HMENU
HWndToHMenu(const HWND hWnd)
{
    return ((HMENU)hWnd);
}

__inline HWND
HMenuToHWnd(const HMENU hMenu)
{
    return ((HWND)hMenu);
}

__inline UINT
HWndToUInt(const HWND hWnd)
{
    return ((UINT)(UINT_PTR)hWnd);
}

__inline HWND
UIntToHWnd(const UINT hWnd)
{
    return ((HWND)(UINT_PTR)hWnd);
}

__inline UINT
HMenuToUInt(const HMENU hMenu)
{
    return ((UINT)(UINT_PTR)hMenu);
}

UINT
GetSafeTimerID(HWND hWnd, const UINT uiElapse)
{
...
HBITMAP
LoadColorBitmap(LPCTSTR lpszResourceName, HMODULE hInst, int* pNumcol)
{
    if (hInst == 0)
...
BOOL
GetMenuInfo(HMENU hMenu, LPMENUINFO pInfo)
{
...
BOOL
SetMenuInfo(HMENU hMenu, LPCMENUINFO pInfo)
{
...
BOOL
DrawMenubarItem(CWnd* pWnd, CMenu* pMenu, UINT nItemIndex, UINT nState)
{
    CRect itemRect;
...
GetXpHighlightColor()
{
    ...
    if (NumScreenColors() > 256)
        //return DarkenColor(SCALE_DARK, MidColor(GetSysColor(COLOR_WINDOW), GetSysColor(COLOR_HIGHLIGHT)));
...
void
DrawGradient(CDC* pDC, CRect& Rect, COLORREF StartColor, COLORREF EndColor, BOOL bHorizontal, BOOL bUseSolid = FALSE)
{
  ...
  public:
    class CMenuHookData
    {
      public:
        ...
        {
            m_hMenu = hMenu;
            if (!CNewMenu::GetNewMenuBorderAllMenu() && !DYNAMIC_DOWNCAST(CNewMenu, CMenu::FromHandlePermanent(hMenu)))
                m_bDoSubclass = FALSE;
            else
                m_bDoSubclass = TRUE;
...
class CNewMenuIconLock
{
  ...
  public:
    CNewMenuIconLock(CNewMenuIcons* pIcons)
      : m_pIcons(pIcons)
    {
        m_pIcons->AddRef();
    }
...
// Diagnostic Support
void
CNewMenuIcons::AssertValid() const
{
    CObject::AssertValid();
}

void
CNewMenuIcons::Dump(CDumpContext& dc) const
{
    CObject::Dump(dc);
    dc << _T("NewMenuIcons: ")
       << _T("\n");
...
BOOL
CNewMenuIcons::DoMatch(LPCTSTR lpszResourceName, HMODULE hInst)
{
...
BOOL
CNewMenuIcons::DoMatch(WORD* pIconInfo, COLORREF crTransparent)
{
...
int
CNewMenuIcons::FindIndex(UINT nID)
{
...
BOOL
CNewMenuIcons::GetIconSize(int* cx, int* cy)
{
    return ::ImageList_GetIconSize(m_IconsList, cx, cy);
}

CSize
CNewMenuIcons::GetIconSize()
{
    int cx = 0;
    int cy = 0;
...
void
CNewMenuIcons::OnSysColorChange()
{
...
BOOL
CNewMenuIcons::LoadBitmap(int nWidth, int nHeight, LPCTSTR lpszResourceName, HMODULE hInst)
{
    m_nColors       = 0;
...
BOOL
CNewMenuIcons::LoadToolBar(WORD* pIconInfo, COLORREF crTransparent)
{
    BOOL bResult            = FALSE;
...
void
CNewMenuIcons::SetResourceName(LPCTSTR lpszResourceName)
{
    ASSERT_VALID(this);
...
BOOL
CNewMenuIcons::LoadToolBar(LPCTSTR lpszResourceName, HMODULE hInst)
{
    ASSERT_VALID(this);
    ...
    if (hRsrc == NULL) { // Special purpose when you try to load it from a dll 30.05.2002
...
int
CNewMenuIcons::AddGloomIcon(HICON hIcon, int nIndex)
{
    ICONINFO iconInfo;
...
int
CNewMenuIcons::AddGrayIcon(HICON hIcon, int nIndex)
{
    ICONINFO iconInfo;
    ZeroMemory(&iconInfo, sizeof(iconInfo));
    if (!GetIconInfo(hIcon, &iconInfo))
    ...
    for (int i = 0; i < size.cx; ++i) {
        for (j = 0; j < size.cy; ++j) {
            crPixel = myDC.GetPixel(i, j);
            myDC.SetPixel(i, j, MixedColor(LightenColor(SCALE_LIGHT, GrayColor(crPixel)), crMenu));
        }
    }
...
CNewMenuIcons::MakeImages()
{
    ...
    CSize size = GetIconSize();
    CImageList ilTemp, ilTemp1;
    ilTemp.Attach(m_IconsList.Detach());

    // I use the code below because MFC's function CImageList::Create(...)
    // may cause an error.
    HIMAGELIST imglist = ImageList_Create(size.cx, size.cy, ILC_COLORDDB | ILC_MASK, 0, 10);
    ASSERT(imglist);
    m_IconsList.m_hImageList = imglist;
    //	m_IconsList.Create(size.cx, size.cy, ILC_COLORDDB | ILC_MASK, 0, 10);

    HICON hIcon;
    for (int nIndex = 0; nIndex < nCount; nIndex++) {
        hIcon = ilTemp.ExtractIcon(nIndex);
...
int
CNewMenuBitmaps::Add(UINT nID, COLORREF crTransparent)
{
    int nIndex = (int)m_IDs.GetSize();
    while (nIndex--) {
        if (m_IDs.GetAt(nIndex) == nID)
    ...
    if (hBitmap != NULL) {
        ...
        if (m_IconsList.GetSafeHandle() == NULL) {
            // I use the code below because MFC's function CImageList::Create(...)
            // may cause an error.
            HIMAGELIST imglist = ImageList_Create(bitmap.bmWidth, bitmap.bmHeight, ILC_COLORDDB | ILC_MASK, 0, 10);
            ASSERT(imglist);
            m_IconsList.m_hImageList = imglist;
...
void
CNewMenuBitmaps::OnSysColorChange()
{
    int nCount = (int)m_IDs.GetSize();
...
int
CNewMenuBitmaps::Add(HICON hIcon, UINT nID)
{
    ICONINFO iconInfo;
    ZeroMemory(&iconInfo, sizeof(iconInfo));
    if (!GetIconInfo(hIcon, &iconInfo))
...
int
CNewMenuBitmaps::Add(CBitmap* pBitmap, COLORREF crTransparent)
{
    ASSERT(pBitmap);
...
LPCTSTR
CNewMenuItemData::GetString()
{
    return m_szMenuText;
}

void
CNewMenuItemData::SetString(LPCTSTR szMenuText)
{
...
// Diagnostic Support
void
CNewMenuItemData::AssertValid() const
{
    CObject::AssertValid();
}

void
CNewMenuItemData::Dump(CDumpContext& dc) const
{
    CObject::Dump(dc);
...
LOGFONT CNewMenu::m_MenuTitleFont = {
    16, 0, 0, 0, FW_BOLD, 0, 0, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, _T("Arial")
};

void
CNewMenu::SetMenuTitleFont(CFont* pFont)
{
    ASSERT(pFont);
...
void
CNewMenu::SetMenuTitleFont(LOGFONT* pLogFont)
{
    ASSERT(pLogFont);
...
COLORREF
CNewMenu::GetMenuBarColor()
{
    if (IsShellType() == WinXP) {
        BOOL bFlatMenu = FALSE;
        if ((SystemParametersInfo(SPI_GETFLATMENU, 0, &bFlatMenu, 0) && bFlatMenu == TRUE) || (pIsThemeActive && pIsThemeActive())) {
            return GetSysColor(COLOR_MENUBAR);
        }
    }
    return GetSysColor(COLOR_MENU);
...
void
CNewMenu::SetLastMenuRect(HDC hDC, LPRECT pRect)
{
    if (!m_bIsPopupMenu) {
...
BOOL
CNewMenu::OnMeasureItem(const MSG* pMsg)
{
...
CMenu*
CNewMenu::FindPopupMenuFromID(HMENU hMenu, UINT nID)
{
...
CMenu*
CNewMenu::FindPopupMenuFromIDData(HMENU hMenu, UINT nID, ULONG_PTR pData)
{
    // check for a valid menu-handle
    if (::IsMenu(hMenu)) {
        CMenu* pMenu = CMenu::FromHandlePermanent(hMenu);
...
CMenu*
CNewMenu::FindPopupMenuFromIDData(CMenu* pMenu, UINT nID, ULONG_PTR pData)
{
...
CMenu*
CNewMenu::FindPopupMenuFromID(CMenu* pMenu, UINT nID)
{
    if (!pMenu || !IsMenu(pMenu->m_hMenu))
        return NULL;
    ...
    for (int iItem = 0; iItem < (int)nItems; iItem++) {
        ...
        if (pPopup != NULL) {
            ...
            return pMenu; // it is a normal item inside our popup
    }
    // not found
    return NULL;
}
...
CNewMenu::DestroyMenu()
{
    ...
    if (m_pSharedMenuIcons) {
        delete m_pSharedMenuIcons;
        m_pSharedMenuIcons = NULL;
    }
...
UINT
CNewMenu::SetMenuDrawMode(UINT mode)
{
...
BOOL
CNewMenu::SetPopup(BOOL bIsPopup)
{
...
BOOL
CNewMenu::SetSelectDisableMode(BOOL mode)
{
...
BOOL
CNewMenu::GetSelectDisableMode()
{
    return m_bSelectDisable;
}

BOOL
CNewMenu::SetXpBlendig(BOOL bEnable)
{
...
BOOL
CNewMenu::SetNewMenuBorderAllMenu(BOOL bEnable /* =TRUE*/)
{
...
BOOL
CNewMenu::GetNewMenuBorderAllMenu()
{
...
void
CNewMenu::MeasureItem(LPMEASUREITEMSTRUCT lpMIS)
{
    ASSERT(m_pActMenuDrawing);
...
void
CNewMenu::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
    ASSERT(m_pActMenuDrawing);
...
BOOL
CNewMenu::EraseBkgnd(HWND hWnd, HDC hDC)
{
...
void
CNewMenu::DrawTitle(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
    ASSERT(m_pActMenuDrawing);
...
void
CNewMenu::DrawMenuTitle(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
...
void
CNewMenu::DrawItem_WinXP(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
    ASSERT(lpDIS != NULL);
    ...
    COLORREF colorMenu    = MixedColor(colorWindow, colorMenuBar);
    COLORREF colorBitmap  = (NumScreenColors() <= 256) ? GetSysColor(COLOR_BTNFACE) : MixedColor(GetMenuBarColor(), colorWindow);
    COLORREF colorSel     = GetXpHighlightColor();
    COLORREF colorBorder  = bHighContrast ? GetSysColor(COLOR_BTNTEXT) : GetSysColor(COLOR_HIGHLIGHT);
            ...
            if (!(lpDIS->itemState & ODS_HOTLIGHT)) {
                ...
                if (NumScreenColors() <= 256) {
                    ...
                    for (X = 1; X <= 4; X++) {
                        ...
                        for (Y = 4; Y < 8; Y++)
                            pDC->SetPixel(rect.right - X, Y + rect.top, DarkenColor(6 * X * (Y - 3), colorMenuBar));

                        for (Y = 8; Y <= (winH - 1); Y++)
                            pDC->SetPixel(rect.right - X, Y + rect.top, DarkenColor(SCALE_DARK * X, colorMenuBar));
                    }
                }
            }
        }
    ...
    if (state & MF_SEPARATOR) {
        if (pMenuData->m_nTitleFlags & MFT_TITLE) {
            TRACE0("Draw the separator\n");
            HDC hOldDC = lpDIS->hDC;
            lpDIS->hDC = *pDC;
            DrawTitle(lpDIS, bIsMenuBar);
            lpDIS->hDC = hOldDC;
        } else {
            TRACE0("Draw the separator1\n");
            // Draw only the separator
            CRect rect;
            ...
            if (pMenuData->m_nMenuIconOffset < 0 && state & ODS_CHECKED && !checkflag) {
                ...
                if (state & ODS_CHECKED || info.hbmpUnchecked) {
                    ...
                    if (state & ODS_CHECKED) {
                        ...
                        if (!info.hbmpChecked)
                            DrawSpecialCharStyle(pDC, rect, 98, state); // Checkmark
                        else
                            DrawSpecialCharStyle(pDC, rect, 105, state); // Bullet
                    } else {
                        if (state & ODS_DISABLED)
                            pDC->DrawState(rect.TopLeft(), rect.Size(), info.hbmpUnchecked, DSS_DISABLED, (HBRUSH)NULL);
                        else
                            pDC->DrawState(rect.TopLeft(), rect.Size(), info.hbmpUnchecked, DSS_NORMAL, (HBRUSH)NULL);
                    }
                }
...
void
CNewMenu::DrawItem_OldStyle(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
    ASSERT(lpDIS != NULL);
    //CNewMemDC memDC(&lpDIS->rcItem, lpDIS->hDC);
    //CDC* pDC = &memDC;
    ...
    if (state & MF_SEPARATOR) {
        ...
        if (!strText.IsEmpty()) {
            ...
            // Default selection?
            if (state & ODS_DEFAULT)
                logFontMenu.lfWeight = FW_BOLD; // Make the font bold
...
BOOL
CNewMenu::IsMenuBar(HMENU hMenu)
{
...
//
void
CNewMenu::MeasureItem_OldStyle(LPMEASUREITEMSTRUCT lpMIS, BOOL bIsMenuBar)
{
...
void
CNewMenu::MeasureItem_WinXP(LPMEASUREITEMSTRUCT lpMIS, BOOL bIsMenuBar)
{
    ASSERT(lpMIS->itemData);
    CNewMenuItemData* pMenuData = (CNewMenuItemData*)(lpMIS->itemData);
...
void
CNewMenu::SetIconSize(int width, int height)
{
    m_iconX = width;
    m_iconY = height;
}
...
BOOL
CNewMenu::AppendODMenu(LPCTSTR lpstrText, UINT nFlags, UINT nID, int nIconNormal)
{
    return CNewMenu::AppendODMenu(lpstrText, nFlags, nID, (CImageList*)NULL, nIconNormal);
}

BOOL
CNewMenu::AppendODMenu(LPCTSTR lpstrText, UINT nFlags, UINT nID, CBitmap* pBmp)
{
    int nIndex = -1;
...
BOOL
CNewMenu::AppendODMenu(LPCTSTR lpstrText, UINT nFlags, UINT nID, CImageList* pil, int xoffset)
{
    int nIndex = 0;
...
BOOL
CNewMenu::AppendODMenu(LPCTSTR lpstrText, UINT nFlags, UINT nID, CNewMenuIcons* pIcons, int nIndex)
{
...
BOOL
CNewMenu::InsertODMenu(UINT nPosition, LPCTSTR lpstrText, UINT nFlags, UINT nID, int nIconNormal)
{
    int nIndex            = -1;
    ...
    if (nIconNormal >= 0) {
        ...
        } else // the nIconNormal is a bitmap
...
BOOL
CNewMenu::InsertODMenu(UINT nPosition, LPCTSTR lpstrText, UINT nFlags, UINT nID, CBitmap* pBmp)
{
    int nIndex = -1;
    CNewMenuIconLock iconLock(GetMenuIcon(nIndex, pBmp));
    return InsertODMenu(nPosition, lpstrText, nFlags, nID, iconLock, nIndex);
...
BOOL
CNewMenu::InsertODMenu(UINT nPosition, LPCTSTR lpstrText, UINT nFlags, UINT nID, CImageList* pil, int xoffset)
{
    int nIndex = -1;
    CNewMenuIconLock iconLock(GetMenuIcon(nIndex, nID, pil, xoffset));
    return InsertODMenu(nPosition, lpstrText, nFlags, nID, iconLock, nIndex);
...
BOOL
CNewMenu::InsertODMenu(UINT nPosition, LPCTSTR lpstrText, UINT nFlags, UINT nID, CNewMenuIcons* pIcons, int nIndex)
{
    if (!(nFlags & MF_BYPOSITION)) {
        int iPosition   = 0;
...
BOOL
CNewMenu::ModifyODMenu(LPCTSTR lpstrText, UINT nID, int nIconNormal)
{
    int nLoc;
...
BOOL
CNewMenu::ModifyODMenu(LPCTSTR lpstrText, UINT nID, CImageList* pil, int xoffset)
{
    int nIndex            = 0;
...
BOOL
CNewMenu::ModifyODMenu(LPCTSTR lpstrText, UINT nID, CNewMenuIcons* pIcons, int xoffset)
{
    ASSERT(pIcons);
    int nLoc;
    CNewMenuItemData* pItemData;
...
BOOL
CNewMenu::ModifyODMenu(LPCTSTR lpstrText, UINT nID, CBitmap* bmp)
{
    if (bmp) {
        CImageList temp;
...
BOOL
CNewMenu::ModifyODMenu(LPCTSTR lpstrText, UINT nID, COLORREF fill, COLORREF border, int hatchstyle)
{
    // Get device context
    CClientDC DC(0);
...
BOOL
CNewMenu::ModifyODMenu(LPCTSTR lpstrText, LPCTSTR OptionText, int nIconNormal)
{
    int nIndex            = 0;
...
CNewMenuItemData*
CNewMenu::NewODMenu(UINT pos, UINT nFlags, UINT nID, LPCTSTR string)
{
    CNewMenuItemData* pItemData;
...
BOOL
CNewMenu::LoadToolBars(const UINT* arID, int n, HMODULE hInst)
{
    ASSERT(arID);
...
DWORD
CNewMenu::SetMenuIcons(CNewMenuIcons* pMenuIcons)
{
...
CNewMenuIcons*
CNewMenu::GetMenuIcon(int& nIndex, UINT nID, CImageList* pil, int xoffset)
{
...
CNewMenuIcons*
CNewMenu::GetMenuIcon(int& nIndex, int nID)
{
    if (m_pSharedMenuIcons != NULL) {
        POSITION pos = m_pSharedMenuIcons->GetHeadPosition();
        while (pos) {
...
CNewMenuIcons*
CNewMenu::GetMenuIcon(int& nIndex, CBitmap* pBmp)
{
    if (pBmp == NULL) {
        nIndex = -1;
        return NULL;
...
CNewMenuIcons*
CNewMenu::GetToolbarIcons(UINT nToolBar, HMODULE hInst)
{
    ASSERT_VALID(this);
...
BOOL
CNewMenu::LoadToolBar(LPCTSTR lpszResourceName, HMODULE hInst)
{
...
BOOL
CNewMenu::LoadToolBar(WORD* pToolInfo, COLORREF crTransparent)
{
    ASSERT_VALID(this);
    ASSERT(pToolInfo);
...
BOOL
CNewMenu::LoadToolBar(UINT nToolBar, HMODULE hInst)
{
    return LoadToolBar((LPCTSTR)(UINT_PTR)nToolBar, hInst);
}

BOOL
CNewMenu::LoadFromToolBar(UINT nID, UINT nToolBar, int& xoffset)
{
    int xset, offset;
    UINT nStyle;
...
// O.S.
CNewMenuItemData*
CNewMenu::FindMenuItem(UINT nID)
{
...
BOOL
CNewMenu::LoadMenu(int nResource)
{
    return (CNewMenu::LoadMenu(MAKEINTRESOURCE(nResource)));
}

BOOL
CNewMenu::LoadMenu(LPCTSTR lpszResourceName)
{
...
BOOL
CNewMenu::SetItemData(UINT uiId, DWORD_PTR dwItemData, BOOL fByPos)
{
    MENUITEMINFO MenuItemInfo;
    ZeroMemory(&MenuItemInfo, sizeof(MenuItemInfo));
    MenuItemInfo.cbSize = sizeof(MenuItemInfo);
...
BOOL
CNewMenu::SetItemDataPtr(UINT uiId, void* pItemData, BOOL fByPos)
{
    return SetItemData(uiId, (DWORD_PTR)pItemData, fByPos);
}

DWORD_PTR
CNewMenu::GetItemData(UINT uiId, BOOL fByPos) const
{
    MENUITEMINFO MenuItemInfo;
    ZeroMemory(&MenuItemInfo, sizeof(MenuItemInfo));
    MenuItemInfo.cbSize = sizeof(MenuItemInfo);
...
void*
CNewMenu::GetItemDataPtr(UINT uiId, BOOL fByPos) const
{
    return (void*)GetItemData(uiId, fByPos);
}

BOOL
CNewMenu::SetMenuData(DWORD_PTR dwMenuData)
{
...
BOOL
CNewMenu::SetMenuDataPtr(void* pMenuData)
{
...
DWORD_PTR
CNewMenu::GetMenuData() const
{
    return (DWORD_PTR)m_pData;
}

void*
CNewMenu::GetMenuDataPtr() const
{
    return m_pData;
}

void
CNewMenu::LoadCheckmarkBitmap(int unselect, int select)
{
...
BOOL
CNewMenu::GetMenuText(UINT id, CString& string, UINT nFlags /*= MF_BYPOSITION*/)
{
    BOOL returnflag = FALSE;
...
CNewMenuItemData*
CNewMenu::CheckMenuItemData(ULONG_PTR nItemData) const
{
...
CNewMenuItemData*
CNewMenu::FindMenuList(UINT nID)
{
    for (int i = 0; i <= m_MenuItemList.GetUpperBound(); ++i) {
...
void
CNewMenu::InitializeMenuList(int value)
{
    for (int i                         = 0; i <= m_MenuItemList.GetUpperBound(); ++i)
    ...
    for (j = 0; j < GetMenuItemCount(); ++j) {
        ...
        if (state & MF_BITMAP) {
            ...
            if (pSubMenu && pSubMenu->m_hParentMenu != m_hMenu)
                pSubMenu->m_hParentMenu = m_hMenu; // Sets again the parent to this one
...
void
CNewMenu::OnInitMenuPopup(HWND hWnd, CMenu* pPopupMenu, UINT nIndex, BOOL bSysMenu)
{
...
BOOL
CNewMenu::Replace(UINT nID, UINT nNewID)
{
    int nLoc            = 0;
...
LRESULT
CNewMenu::FindKeyboardShortcut(UINT nChar, UINT nFlags, CMenu* pMenu)
{
...
BOOL
CNewMenu::AddBitmapToImageList(CImageList* bmplist, UINT nResourceID)
{
    // O.S.
    if (m_bDynIcons) {
    ...
    if (hbmp)
        mybmp.Attach(hbmp); // Object will be destroyd by destructor of CBitmap
    else
...
COLORREF
CNewMenu::SetBitmapBackground(COLORREF newColor)
{
...
BOOL
CNewMenu::Draw3DCheckmark(CDC* pDC, const CRect& rc, HBITMAP hbmCheck, DWORD dwStyle)
{
    CRect rect(rc);
...
HBITMAP
CNewMenu::LoadSysColorBitmap(int nResourceId)
{
    ...
    return LoadSysColorBitmapSize(hInst, hRsrc, CSize(lpBitmap->biWidth, lpBitmap->biHeight));
}

// sPos means Separator's position, since we have no way to find the
// separator's position in the menu we have to specify them when we call the
// RemoveMenu to make sure the unused separators are removed;
// sPos  = None no separator removal;
//       = Head  separator in front of this menu item;
//       = Tail  separator right after this menu item;
//       = Both  separators at both ends;
// remove the menu item based on their text, return -1 if not found, otherwise
// return the menu position;
int
CNewMenu::RemoveMenu(LPCTSTR pText, ESeparator sPos)
{
...
BOOL
CNewMenu::RemoveMenu(UINT uiId, UINT nFlags)
{
    if (MF_BYPOSITION & nFlags) {
...
BOOL
CNewMenu::DeleteMenu(UINT uiId, UINT nFlags)
{
    if (MF_BYPOSITION & nFlags) {
        UINT nItemState = GetMenuState(uiId, MF_BYPOSITION);
        if ((nItemState & MF_SEPARATOR) && !(nItemState & MF_POPUP)) {
            ...
            if (NULL == pSubMenu) {
                ...
                if (bCanDelete)
                    delete pSubMenu; // Oh yes so we can destroy it
            }
        }
    } else {
        int iPosition   = 0;
...
BOOL
CNewMenu::AppendMenu(UINT nFlags, UINT nIDNewItem, LPCTSTR lpszNewItem, CBitmap* bmp)
{
    return AppendODMenu(lpszNewItem, nFlags, nIDNewItem, bmp);
}

BOOL
CNewMenu::InsertMenu(UINT nPosition, UINT nFlags, UINT nIDNewItem, LPCTSTR lpszNewItem, int nIconNormal)
{
    return InsertODMenu(nPosition, lpszNewItem, nFlags, nIDNewItem, nIconNormal);
}

BOOL
CNewMenu::InsertMenu(UINT nPosition, UINT nFlags, UINT nIDNewItem, LPCTSTR lpszNewItem, CImageList* il, int xoffset)
{
    return InsertODMenu(nPosition, lpszNewItem, nFlags, nIDNewItem, il, xoffset);
}

BOOL
CNewMenu::InsertMenu(UINT nPosition, UINT nFlags, UINT nIDNewItem, LPCTSTR lpszNewItem, CBitmap* bmp)
{
...
CMenu*
CNewMenu::GetSubMenu(int nPos) const
{
    return CMenu::GetSubMenu(nPos);
}

CMenu*
CNewMenu::GetSubMenu(LPCTSTR lpszSubMenuName) const
{
...
// Tongzhe Cui, Functions to remove a popup menu based on its name. Separators
// before and after the popup menu can also be removed if they exist.
int
CNewMenu::GetMenuPosition(LPCTSTR pText)
{
    int j;
    for (int i = 0; i < (int)(GetMenuItemCount()); ++i) {
        if (!GetSubMenu(i)) {
            for (j = 0; j <= m_MenuItemList.GetUpperBound(); ++j) {
...
BOOL
CNewMenu::RemoveMenuTitle()
{
    int numMenuItems = (int)m_MenuItemList.GetSize();

    // We need a separator at the beginning of the menu
...
BOOL
CNewMenu::SetMenuTitle(LPCTSTR pTitle, UINT nTitleFlags)
{
...
BOOL
CNewMenu::SetMenuText(UINT id, CString string, UINT nFlags /*= MF_BYPOSITION*/)
{
    BOOL returnflag = FALSE;
...
void
CNewMenu::ColorBitmap(CDC* pDC, CBitmap& bmp, CSize size, COLORREF fill, COLORREF border, int hatchstyle)
{
    // Create a memory DC
    CDC MemDC;
...
void
CNewMenu::DrawSpecial_OldStyle(CDC* pDC, LPCRECT pRect, UINT nID, DWORD dwStyle)
{
    COLORREF oldColor;
...
void
CNewMenu::DrawSpecial_WinXP(CDC* pDC, LPCRECT pRect, UINT nID, DWORD dwStyle)
{
    TCHAR cSign = 0;
    switch (nID & 0xfff0) {
        case SC_MINIMIZE:
...
CRect
CNewMenu::GetLastActiveMenuRect()
{
    return m_LastActiveMenuRect;
}

void
CNewMenu::DrawSpecialCharStyle(CDC* pDC, LPCRECT pRect, TCHAR Sign, DWORD dwStyle)
{
    COLORREF oldColor;
    if ((dwStyle & ODS_GRAYED) || (dwStyle & ODS_INACTIVE))
        oldColor = pDC->SetTextColor(::GetSysColor(COLOR_GRAYTEXT));
...
void
CNewMenu::DrawSpecialChar(CDC* pDC, LPCRECT pRect, TCHAR Sign, BOOL bBold)
{
    //  48 Min
    //  49 Max
    //  50 Restore
...
void
CMenuTheme::DrawFrame(CDC* pDC, CRect rectOuter, CRect rectInner, COLORREF crBorder)
{
    CRect Temp;
    rectInner.right -= 1;
    // Border top
...
BOOL
CMenuTheme::DoDrawBorder()
{
    return (m_dwFlags & 1) ? TRUE : FALSE;
}

void
CMenuTheme::DrawSmalBorder(HWND hWnd, HDC hDC)
{
...
void
CMenuTheme::DrawShade(HWND hWnd, HDC hDC, CPoint screen)
{
    if (IsShadowEnabled())
        return;
    ...
    if (NumScreenColors() <= 256) {
        ...
        for (X = 1; X <= 4; X++) {
            ...
            for (Y = 4; Y < 8; Y++) {
                ...
                SetPixel(hDC, winW - X, Y, DarkenColor(6 * X * (Y - 3), c));
            }

            for (Y = 8; Y <= (winH - 5); Y++) {
                c = GetPixel(hDcDsk, xOrg + winW - X, yOrg + Y);
                SetPixel(hDC, winW - X, Y, DarkenColor(SCALE_DARK * X, c));
            }

            for (Y = (winH - 4); Y <= (winH - 1); Y++) {
                c = GetPixel(hDcDsk, xOrg + winW - X, yOrg + Y);
                SetPixel(hDC, winW - X, Y, DarkenColor(6 * X * -(Y - winH), c));
            }
        }
        ...
        for (Y = 1; Y <= 4; Y++) {
            ...
            for (X = 4; X <= 7; X++) {
                ...
                SetPixel(hDC, X, winH - Y, DarkenColor(6 * (X - 3) * Y, c));
            }

            for (X = 8; X <= (winW - 5); X++) {
                c = GetPixel(hDcDsk, xOrg + X, yOrg + winH - Y);
                SetPixel(hDC, X, winH - Y, DarkenColor(SCALE_DARK * Y, c));
            }
        }
    }
...
BOOL
CMenuTheme::OnInitWnd(HWND hWnd)
{
    CNewMenuHook::CMenuHookData* pData = CNewMenuHook::GetMenuHookData(hWnd);
    ASSERT(pData);
...
BOOL
CMenuTheme::OnUnInitWnd(HWND hWnd)
{
    CNewMenuHook::CMenuHookData* pData = CNewMenuHook::GetMenuHookData(hWnd);
    if (pData) {
...
BOOL
CMenuTheme::OnEraseBkgnd(HWND hWnd, HDC hDC)
{
    // CNewMenuHook::CMenuHookData* pData = CNewMenuHook::GetMenuHookData(hWnd);
...
BOOL
CMenuTheme::OnWindowPosChanging(HWND hWnd, LPWINDOWPOS pPos)
{
...
BOOL
CMenuTheme::OnNcCalcSize(HWND hWnd, NCCALCSIZE_PARAMS* pCalc)
{
    UNREFERENCED_PARAMETER(hWnd);
...
BOOL
CMenuTheme::OnCalcFrameRect(HWND hWnd, LPRECT pRect)
{
...
void*
CMenuTheme::SetScreenBitmap(HWND hWnd, HDC hDC)
{
...
BOOL
CMenuTheme::OnDrawBorder(HWND hWnd, HDC hDC, BOOL bOnlyBorder)
{
...
BOOL
CMenuThemeXP::OnDrawBorder(HWND hWnd, HDC hDC, BOOL bOnlyBorder)
{
    CNewMenuHook::CMenuHookData* pData = (CNewMenuHook::CMenuHookData*)SetScreenBitmap(hWnd, hDC);
    ;
    if (DoDrawBorder() && pData) {
...
BOOL
CMenuThemeXP::OnEraseBkgnd(HWND hWnd, HDC hDC)
{
    CNewMenuHook::CMenuHookData* pData = CNewMenuHook::GetMenuHookData(hWnd);
...
CNewMenuHook::CNewMenuHook()
{
    ...
    AddTheme(
      new CMenuTheme(CNewMenu::STYLE_ORIGINAL, CNewMenu::MeasureItem_OldStyle, CNewMenu::DrawItem_OldStyle, CNewMenu::DrawMenuTitle, TRUE));

    AddTheme(new CMenuTheme(
      CNewMenu::STYLE_ORIGINAL_NOBORDER, CNewMenu::MeasureItem_OldStyle, CNewMenu::DrawItem_OldStyle, CNewMenu::DrawMenuTitle));
...
BOOL
CNewMenuHook::AddTheme(CMenuTheme* pTheme)
{
    if (m_pRegisteredThemesList == NULL)
...
CMenuTheme*
CNewMenuHook::RemoveTheme(DWORD dwThemeId)
{
...
CMenuTheme*
CNewMenuHook::FindTheme(DWORD dwThemeId)
{
    if (m_pRegisteredThemesList == NULL)
        return NULL;
...
CNewMenuHook::CMenuHookData*
CNewMenuHook::GetMenuHookData(HWND hWnd)
{
...
void
CNewMenuHook::UnsubClassMenu(HWND hWnd)
{
    ...
    switch (uMsg) {
        case WM_NCPAINT:
            ...
            if (pData->m_bDoSubclass) {
                ...
                if (pData->m_dwData & 8)
                    bCallDefault = FALSE; // do not call default!!!
        ...
        case WM_SHOWWINDOW:
            ...
            if (wParam == NULL)
                UnsubClassMenu(hWnd); // Special handling for NT 2000 and WND 0x10012.
            break;
...
LRESULT CALLBACK
CNewMenuHook::NewMenuHook(int code, WPARAM wParam, LPARAM lParam)
{
    AFX_MANAGE_STATE(AfxGetModuleState());
...
CNewDialog::OnInitDialog()
{
    ...
    if (hMenu != hSysMenu && IsMenu(hMenu)) {
...
void
CNewDialog::OnInitMenuPopup(CMenu* pMenu, UINT nIndex, BOOL bSysMenu)
{
...
BOOL
CNewFrameWnd::LoadFrame(UINT nIDResource, DWORD dwDefaultStyle, CWnd* pParentWnd, CCreateContext* pContext)
{
    // only do this once
...
BOOL
CNewFrameWnd::Create(LPCTSTR lpszClassName,
...
BOOL
CNewMDIFrameWnd::ShowMenu(BOOL bShow)
{
...
BOOL
CNewMDIFrameWnd::LoadFrame(UINT nIDResource, DWORD dwDefaultStyle, CWnd* pParentWnd, CCreateContext* pContext)
{
    // only do this once
    ASSERT_VALID_IDR(nIDResource);
    ASSERT(m_nIDHelp == 0 || m_nIDHelp == nIDResource);
...
BOOL
CNewMDIFrameWnd::Create(LPCTSTR lpszClassName,
                        LPCTSTR lpszWindowName,
                        DWORD dwStyle,
                        const RECT& rect,
                        CWnd* pParentWnd,
