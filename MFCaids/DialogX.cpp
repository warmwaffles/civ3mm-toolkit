...
void CDialogX::SetControlInfo(UINT CtrlId, WORD Anchor, WORD xDivider, WORD xDivPos, WORD yDivider, WORD yDivPos)
{
...
//
void CDialogX::OnSize(UINT nType, int cx, int cy)
{
...
//			WndRect.OffsetRect(0, dy / cInfo.m_yDivider);
	...
	InvalidateRect(CRect(m_xPrevSize - GetSystemMetrics(SM_CXHSCROLL), 
		m_yPrevSize - GetSystemMetrics(SM_CYVSCROLL), cx, cy), m_bRedrawOnResize);
	UpdateWindow();
	m_xPrevSize = cx;
	m_yPrevSize = cy;	
}

void CDialogX::OnGetMinMaxInfo(MINMAXINFO* lpMMI)
{
...
// OnCreate()
//
int CDialogX::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
//
UINT CDialogX::OnNcHitTest(CPoint point)
{
...
void CDialogX::GetDialogProfileEntry(CString &sEntry)
{
