...
HTREEITEM CTreeCtrlX::FindItemID(HTREEITEM hParent, DWORD id)
{
...
BOOL CTreeCtrlX::IsItemBold(HTREEITEM hItem)
{
...
BOOL CTreeCtrlX::IsItemExpanded(HTREEITEM hItem)
{
	return GetItemState(hItem, TVIS_EXPANDED) & TVIS_EXPANDED;
}

void CTreeCtrlX::SetItemExpand(HTREEITEM hItem, BOOL bExpand)
{
	Expand(hItem, bExpand ? TVE_EXPAND : TVE_COLLAPSE);
}

BOOL CTreeCtrlX::IsItemSelected(HTREEITEM hItem)
{
	return GetItemState(hItem, TVIS_SELECTED) & TVIS_SELECTED;
}

void CTreeCtrlX::SetItemBlue(HTREEITEM hItem)
{
	Color_Font cf;
...
void CTreeCtrlX::SetItemRed(HTREEITEM hItem)
{
	Color_Font cf;
...
void CTreeCtrlX::ClearColorFont()
{	
	m_mapColorFont.RemoveAll();
}

CImageList* CTreeCtrlX::CreateImageList(UINT nIDResource, COLORREF crBgd /*= RGB_BACKGROUND*/, int nImageListType /*= TVSIL_NORMAL*/)
{
	CBitmap bmp;
...
int CTreeCtrlX::AddGloomIcon(HICON hIcon)
{
	ICONINFO iconInfo;
...
void CTreeCtrlX::SetImageSize(CSize sz)
{
	m_sizeImage = sz;
}

CSize CTreeCtrlX::GetImageSize()
{
	return m_sizeImage;
}

BOOL CTreeCtrlX::IsDragEnabled()
{
	return m_bDragEnable;
}

void CTreeCtrlX::EnableDrag(BOOL bEnable /*= TRUE*/)
{
...
//	return -1;
//}
BOOL CTreeCtrlX::TransferItem(HTREEITEM hDrag, HTREEITEM hDrop)
{
	if (!GetSafeHwnd())
		return FALSE;
...
void CTreeCtrlX::OnBeginDrag(LPNMHDR pnmhdr, LRESULT *pLResult)
{
	if (!m_bDragEnable)
		return;
...
void CTreeCtrlX::MouseScroll(CPoint point)
{
	CRect client;
	GetClientRect(client);
