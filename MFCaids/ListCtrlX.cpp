...
void CListCtrlX::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	LVITEM item;
...
//	dp.bHotLight        = lpDrawItemStruct->itemState & ODS_HOTLIGHT;
	...
	for (dp.nColumn = 0; GetColumn(dp.nIndex = OrderToIndex(dp.nColumn), &lvc); dp.nColumn++)
	{
		...
		dp.bDrawCell = (GetSelectedCount() <= 1) && (dp.nIndex == m_nColumnSel) &&
			(dp.bSelected || dp.bAlwaysSelected);
		...
		{
			if (dp.nIndex == 0 && dp.nColumn != 0 && !m_bKeepLabelLeft && !dp.bFullRowSelected || dp.nColumn == 0 && (m_bKeepLabelLeft || dp.nIndex == 0))
			{
				...
				if (!dp.bFullRowSelected)
				{
					...
					{
					case LVCFMT_LEFT:
						rcSubItemSel.right = rcSubItemSel.left 
							+ m_nFirstColXOff + nLabelWidth + m_nNextColXOff;
						break;
					case LVCFMT_RIGHT:
						rcSubItemSel.left = rcSubItemSel.right - 
							m_nFirstColXOff - nLabelWidth - m_nNextColXOff;
						break;
					case LVCFMT_CENTER:
						{
...
BOOL CListCtrlX::GetRealSubItemRect(int iItem, int iSubItem, int nArea, CRect &ref)
{
	ASSERT(iSubItem >= 0);
...
//
int CListCtrlX::IndexToOrder(int nIndex)
{
...
//
int CListCtrlX::OrderToIndex(int nOrder)
{
	ASSERT(GetHeaderCtrl());
...
//
BOOL CListCtrlX::SetKeepLabelLeft(BOOL bKeepLeft)
{
	BOOL bSuccess = TRUE;
...
void CListCtrlX::EnableSortIcon(BOOL bEnable, int nSortColumn)
{
	ASSERT(GetHeaderCtrl());
...
//
int CListCtrlX::GetLabelWidth(CDC *pDC, LVITEM *pItem, int nMaxWidth)
{
...
//
BOOL CListCtrlX::GetStateIconRect(int nItem, LPRECT pRect)
{
	CRect rcSubItem;
...
// Draw state icon
//
void CListCtrlX::DrawStateIcon(CDC *pDC, LVITEM *pItem, LPRECT pRect)
{
	...
	if (nImage > 0)
	{
		...
		if (pimglst)
		{
			...
			{
				pimglst->DrawIndirect(pDC, nImage, CPoint(pRect->left, pRect->top),
...
// Draw small icon
//
void CListCtrlX::DrawSmallIcon(CDC *pDC, LVITEM *pItem, LPRECT pRect)
{
	if (pItem->iImage >= 0)
	{
		...
		if (pimglst)
		{
			...
			{
				CSize szImg(__min(pRect->right - pRect->left + 1,
					imgInfo.rcImage.right - imgInfo.rcImage.left),
					__min(pRect->bottom - pRect->top + 1,
					imgInfo.rcImage.bottom - imgInfo.rcImage.top));
...
//
void CListCtrlX::DrawSubItemText(CDC *pDC, LVITEM *pItem, LVCOLUMN *pColumn, LPRECT pRect)
{
	if (*pItem->pszText)
	{
		...
		if (nWidth > 0)
		{
			...
			pDC->DrawText  (strLabel, -1, pRect,
				(pColumn->fmt & LVCFMT_CENTER ? DT_CENTER : pColumn->fmt & LVCFMT_RIGHT ? DT_RIGHT  : DT_LEFT) |
...
//
void CListCtrlX::CreateSortIcons()
{
...
void CListCtrlX::ColorSortColumn(BOOL bEnable)
{
...
//
int CListCtrlX::InsertColumn(int nCol, const LVCOLUMN *pColumn)
{
	if (nCol == 0)
...
//
void CListCtrlX::JustifyFirstColumn(int nFormat)
{
...
int CListCtrlX::InsertColumn(int nCol, LPCTSTR lpszColumnHeading, int nFormat, int nWidth, int nSubItem)
{
	if (nCol == 0)
...
BOOL CListCtrlX::OnEraseBkgnd(CDC* pDC) 
{
	CRect rc;
	GetClientRect(&rc);
...
BOOL CListCtrlX::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
...
//			break;
		...
		case HDN_DIVIDERDBLCLICKA:
			if (m_bKeepLabelLeft && (pNMHdr->iItem  > 0 && IndexToOrder(pNMHdr->iItem) == 0 ||
...
//
void CListCtrlX::InvalidateNonItemArea()
{
...
// Small icon always should be kept left
//
BOOL CListCtrlX::KeepLabelLeft(BOOL bKeepLeft)
{
	BOOL bSuccess = TRUE;
...
void CListCtrlX::SetRowHeight(int nHeight)
{
...
//
int CListCtrlX::HitTest(CPoint pt, UINT *pFlags, int *pColumn)
{
	LVHITTESTINFO info;
	info.pt = pt;
...
void CListCtrlX::OnRButtonDown(UINT nFlags, CPoint point) 
{
	SetFocus();
	int nCol = 0;
	int nRow = HitTest(point, 0, &nCol);
...
void CListCtrlX::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
int CListCtrlX::GetSel(int *nCol)
{
...
void CListCtrlX::EnsureVisibleColumn(int nCol)
{
	if (nCol < 0)
		return;
...
void CListCtrlX::OnEndtrack(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CListCtrlX::OnItemchanged(NMHDR* pNMHDR, LRESULT* pResult) 
{
	HD_NOTIFY *phdn = (HD_NOTIFY *) pNMHDR;
...
CEditX* CListCtrlX::EditSubItem(int nItem, int nCol, LPCTSTR strInitText)
{
...
void CListCtrlX::DestroyEdit()
{
	SAFE_DELETE(m_pEdit);
}

CListCtrlXEdit* CListCtrlX::GetEditCtrl()
{
	return m_pEdit;
}

void CListCtrlX::DrawCellRect(CDC *pDC, CRect &rc, CRect &rcText, BOOL bSelected)
{
	BOOL bDrawn		= FALSE;
...
void CListCtrlX::DrawCellGrid(CDC *pDC, CRect &rc)
{
...
// Draw '...' button
//
void CListCtrlX::DrawBrowse(CDC *pDC, CRect &rc)
{
	CRect rcBtn(rc);
...
// Draw check box
//
void CListCtrlX::DrawCheckBox(CDC *pDC, CRect &rc, DWORD dwStyle)
{
	CRect rcBtn(rc);
...
void CListCtrlX::OnBeginlabeledit(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
BOOL CListCtrlX::SelectColumn(int nCol)
{
	m_nColumnSel = nCol;
	return TRUE;
}

void CListCtrlX::StartCellEdit(int nRow, int nCol)
{
	int nItems = GetItemCount();
	for (int i = 0; i < nItems; i++)
...
void CListCtrlX::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	switch (nChar)
	{
	case VK_RETURN:
...
BOOL CListCtrlX::IsEditable(int nRow, int nCol)
{
	return TRUE;
}

void CListCtrlX::SelectItem(int nItem, int nSubItem)
{
	int nItems = GetItemCount();
...
int CListCtrlX::GetSelColumn()
{
	return m_nColumnSel;
}

int CListCtrlX::FindItem(LPARAM lParam)
{
	LVFINDINFO lvfi;
...
int CListCtrlX::FindItem(CString sFind, int nCol /*=1*/)
{
	if (sFind.IsEmpty())
		return -1;
...
void CListCtrlX::EditStyle(int nRow, int nCol, DWORD &dwStyle)
{
}

UINT CListCtrlX::EditLimit(int nRow, int nCol)
{
	return 0;
}

BOOL CListCtrlX::SetListText(int nRow, int nCol, int dlgCtrlId, CString strText)
{
    CString Text;
    GetWindowText(Text);
...
int CListCtrlX::Get1stSelectedItem()
{
	POSITION pos = GetFirstSelectedItemPosition();
	return (pos != NULL) ? GetNextSelectedItem(pos) : -1;
}

LPCTSTR CListCtrlX::RegKey()
{
	return "";
}

void CListCtrlX::SaveColumns()
{
	CHeaderCtrl *pHeader = GetHeaderCtrl();
	CString strCol;
...
void CListCtrlX::LoadColumns(int nColumns, ...)
{
	CHeaderCtrl *pHeader = GetHeaderCtrl();
	va_list argList;
...
void CListCtrlX::DrawSpin(CDC *pDC, CRect &rc, CRect &rcText, BOOL bSelected)
{
	m_rcBtn = rc;
...
void CListCtrlX::DrawBtn(CDC *pDC, CRect &rc, CRect &rcText, BOOL bSelected)
{
	m_rcBtn = rc;
	m_rcBtn.left = m_rcBtn.right - m_rcBtn.Height();
...
void CListCtrlX::DrawCombo(CDC *pDC, CRect &rc, CRect &rcText, BOOL bSelected)
{
	m_rcBtn = rc;
	m_rcBtn.left = m_rcBtn.right - m_rcBtn.Height();
...
void CListCtrlX::DrawCheck(CDC *pDC, CRect &rc, CRect &rcText, BOOL bSelected)
{
	FillCellRect(pDC, rc, bSelected ? (dp.bDrawCell ? dp.dwHiLiteBkCell : dp.dwHiLiteBk) : dp.dwBkColor);
	DrawCheckBox(pDC, rc, DFCS_FLAT | ((lstrlen(dp.pSubItem->pszText) > 0) ? DFCS_CHECKED : 0));
}

BOOL CListCtrlX::IsButton(int nRow, int nCol)
{
	return FALSE;
}

BOOL CListCtrlX::IsCombo(int nRow, int nCol)
{
	return FALSE;
}

BOOL CListCtrlX::IsCheck(int nRow, int nCol)
{
	return FALSE;
}

BOOL CListCtrlX::IsSpin(int nRow, int nCol)
{
	return FALSE;
}

BOOL CListCtrlX::IsDragEnabled()
{
	return m_bDragEnable;
}

void CListCtrlX::EnableDrag(BOOL bEnable /*= TRUE*/)
{
...
//
BOOL CListCtrlX::IsDraggingAllowed()
{
	return TRUE;
}
...
//
void CListCtrlX::OnBegindrag(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CListCtrlX::MouseScroll(CPoint point)
{
	CRect client, rc;
...
//	return -1;
//}
BOOL CListCtrlX::DropItemOnList()
{
	if (!GetSafeHwnd())
		return FALSE;
...
void CListCtrlX::OnButtonPressed()
{
}

void CListCtrlX::CreateComboList(int nRow, int nCol)
{
	if (m_pCombo == NULL)
...
void CListCtrlX::PopulateCombo(int nRow, int nCol, CListCtrlXCombo *pCombo)
{
	pCombo->ResetContent();
}

void CListCtrlX::OnCaptureChanged(CWnd *pWnd) 
{
...
void CListCtrlX::SetEmptyMsg(LPCTSTR strMsg)
{
	m_strEmptyMsg = strMsg;
}

CImageList* CListCtrlX::SetImageList(CImageList *pImageList, int nImageList)
{
	CImageList *pImgList = NULL;

	switch (nImageList)
	{
	case LVSIL_NORMAL:	pImgList = m_pImgList_Normal;	m_pImgList_Normal = pImageList;	break;
	case LVSIL_SMALL:	pImgList = m_pImgList_Small;	m_pImgList_Small  = pImageList;	break;
	case LVSIL_STATE:	pImgList = m_pImgList_State;	m_pImgList_State  = pImageList;	break;
	}
	...
	if (pImageList != NULL)
	{
		...
		m_ImageList.Create(img_info.rcImage.right - img_info.rcImage.left,
			img_info.rcImage.bottom - img_info.rcImage.top, ILC_COLOR, 0, 0);
...
CImageList* CListCtrlX::GetImageList(int nImageList) const
{
	CImageList *pImgList = NULL;
	switch (nImageList)
	{
	case LVSIL_NORMAL: pImgList = m_pImgList_Normal; break;
	case LVSIL_SMALL:  pImgList = m_pImgList_Small;	 break;
	case LVSIL_STATE:  pImgList = m_pImgList_State;	 break;
	}

	return pImgList;
}

BOOL CListCtrlX::SetItemText(int nItem, int nSubItem, LPCTSTR lpszText)
{
...
void CListCtrlX::FillCellRect(CDC *pDC, CRect &rc, COLORREF col)
{
	pDC->FillSolidRect(rc, col);
}

void CListCtrlX::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
//	CWnd *pWnd = GetFocus();
//	if (pWnd)
//		pWnd = pWnd->GetParent();
...
BOOL CListCtrlX::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
//	if (GetFocus()->GetParent() == this)
//		SetFocus();
...
// added by Alex Zhondin
BOOL CListCtrlX::SortColumn(int nColumn)
{
	if (!IsSorted(nColumn))
		return FALSE;
...
BOOL CListCtrlX::IsSorted(int nCol)
{
	return FALSE;
}

void CListCtrlX::SetItemImage(int nItem, int nImg)
{
	LVITEM item;
...
void CListCtrlX::OnDeltaposSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CListCtrlX::SetSpinRange(int nLower, int nUpper)
{
	if (m_pSpin == NULL)
	{
		m_pSpin = new CSpinButtonCtrl;
...
int CListCtrlXEdit::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CListCtrlXEdit::OnKillFocus(CWnd* pNewWnd) 
{
	if (!pNewWnd)
		return;

	if ((!m_bKillFocus && !m_bCancelEdit) && 
		 !m_pListTable->m_bWarning)
		SetListText();
	...
	if (m_pListTable->m_bWarning)
	{
		...
		if (m_pListTable->m_nError == 1)
			...
			MessageBox(CString((LPCSTR)IDS_ERR_DUPLICATE_NAME), uLoadString(IDS_WARNING), MB_ICONWARNING); // (!) don't use the uLoadStringLocal
...
void CListCtrlXEdit::OnNcDestroy() 
{
	CEditX::OnNcDestroy();
...
void CListCtrlXEdit::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
...
HBRUSH CListCtrlXEdit::CtlColor(CDC* pDC, UINT nCtlColor) 
{
//	pDC->SetTextColor(GetSysColor(COLOR_WINDOWTEXT));
...
UINT CListCtrlXEdit::OnGetDlgCode() 
{
    return CEditX::OnGetDlgCode() | DLGC_WANTARROWS | DLGC_WANTTAB;
}

BOOL CListCtrlXEdit::SetListText()
{
    CString Text;
    GetWindowText(Text);
...
BOOL CListCtrlXEdit::PreTranslateMessage(MSG* pMsg) 
{
    if (pMsg->message == WM_KEYDOWN)
    {
	    if ((pMsg->wParam == VK_RETURN) || (pMsg->wParam == VK_ESCAPE) || GetKeyState(VK_CONTROL) ||
...
void CListCtrlXEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
...
void CListCtrlXEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
...
BOOL CListCtrlXCombo::SetListText()
{
	if (GetCurSel() == LB_ERR)
		return TRUE;
...
void CListCtrlXCombo::OnSelendok() 
{
	SetListText();
	m_pListTable->SetFocus();
}

void CListCtrlXCombo::OnSelendcancel() 
{
	m_pListTable->SetFocus();
}

void CListCtrlXCombo::Init(int iItem, int iSubItem, CString strInitText)
{
	m_nItem       = iItem;
	m_nSubItem    = iSubItem;
	m_strInitText = strInitText;
