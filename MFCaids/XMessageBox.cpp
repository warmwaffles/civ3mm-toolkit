/*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\
	FILE NAME:	XMessageBox.cpp
	VERSION:	1.6

	AUTHOR:		Hans Dietrich
	MODIFIED:	Dreyk
  
	DESCRIPTION: XMessageBox.cpp implements XMessageBox(), a drop-in replacement for 
	...
	HISTORY:
	 Version 1.6 - 2004 March 30 (by Dreyk)
	 - Buttons strings added to the resources to load  button captions from resource

     Version 1.5 - 2004 February 6 (by Dreyk)
	 - Added function: int XMessageBox(HWND hwnd, UINT nIDPrompt, ...
	 - Added function: int XMessageBox(HWND hwnd, LPCTSTR lpszMessage, ...
	 - Added function: int XMessageBox(LPCTSTR lpszMessage, LPCTSTR lpszCaption = NULL, ...
	 - Added function: int XMessageBox(UINT nIDPrompt, ...
	 - Added function: int XMessageBox(LPCTSTR lpszMessage, ...
	 - Implemented DLL support
...
class CXDialogItem
{
  ...
  public:
    ...
    void AddItem(CXDialogTemplate& dialog, Econtroltype cType, UINT nID, CXRect* prect = NULL, LPCTSTR pszCaption = NULL);
};
...
class CXDialogTemplate
{
    // Constructors
  public:
    CXDialogTemplate(HWND hWnd, LPCTSTR lpszMessage, LPCTSTR lpszCaption, UINT nStyle, XMSGBOXPARAMS* pXMB);
  ...
  public:
    int Display();
    void AddItem(CXDialogItem::Econtroltype cType, UINT nID, CXRect* prect = NULL, LPCTSTR pszCaption = NULL);
    void
    AddCheckBox(HDC hdc, int& x, int& y, CXRect& rect, CXRect& mbrect, CXRect& buttonrow, CXRect& checkboxrect, LPCTSTR lpszButtonCaption);
...
int
XMessageBox(HWND hwnd, LPCTSTR lpszMessage, UINT nStyle /*= MB_OK | MB_ICONEXCLAMATION*/, XMSGBOXPARAMS* pXMB /*= NULL*/)
{
    return XMessageBox(hwnd, lpszMessage, "", nStyle, pXMB);
}

int
XMessageBox(HWND hwnd, UINT nIDPrompt, UINT nStyle /*= MB_OK | MB_ICONEXCLAMATION*/, XMSGBOXPARAMS* pXMB /*= NULL*/)
{
    CString string;
    if (!string.LoadString(nIDPrompt)) {
        TRACE1("Error: failed to load XMessage box prompt string 0x%04x.\n", nIDPrompt);
        ASSERT(FALSE);
    }

    return XMessageBox(hwnd, string, "", nStyle, pXMB);
}

int
XMessageBox(LPCTSTR lpszMessage,
            LPCTSTR lpszCaption /*= NULL*/,
            UINT nStyle /*= MB_OK | MB_ICONEXCLAMATION*/,
            XMSGBOXPARAMS* pXMB /*= NULL*/)
{
    HWND hwnd = NULL;
    return XMessageBox(hwnd, lpszMessage, lpszCaption, nStyle, pXMB);
}

int
XMessageBox(LPCTSTR lpszMessage, UINT nStyle /*= MB_OK | MB_ICONEXCLAMATION*/, XMSGBOXPARAMS* pXMB /*= NULL*/)
{
    HWND hwnd = NULL;
    return XMessageBox(hwnd, lpszMessage, "", nStyle, pXMB);
}
...
int
XMessageBox(UINT nIDPrompt, UINT nStyle /*= MB_OK | MB_ICONEXCLAMATION*/, XMSGBOXPARAMS* pXMB /*= NULL*/)
{
    CString string;
    if (!string.LoadString(nIDPrompt)) {
        TRACE1("Error: failed to load XMessage box prompt string 0x%04x.\n", nIDPrompt);
            ...
            XMSGBOXPARAMS* pXMB /*= NULL*/)
{
    ...
    if ((nStyle & MB_DONOTASKAGAIN) || (nStyle & MB_DONOTTELLAGAIN) || (nStyle & MB_DONOTSHOWAGAIN)) {
        ...
        if (xmb.lpszModule && (xmb.lpszModule[0] != _T('\0'))) {
            ...
            if ((dwData & MB_DONOTASKAGAIN) || (dwData & MB_DONOTTELLAGAIN) || (dwData & MB_DONOTSHOWAGAIN)) {
    ...
    CXDialogTemplate dlg(hwnd, lpszMessage, lpszCaption, nStyle, &xmb);
...
// IconProc
LONG CALLBACK
IconProc(HWND hwnd, UINT message, WPARAM, LPARAM)
{
    if (message == WM_PAINT) {
        PAINTSTRUCT ps;
        HDC hdc;
...
CXDialogTemplate::CXDialogTemplate(HWND hWnd, LPCTSTR lpszMessage, LPCTSTR lpszCaption, UINT nStyle, XMSGBOXPARAMS* pXMB)
  : m_hWnd(hWnd)
  , m_lpszMessage(NULL)
  , m_lpszCaption(NULL)
  , m_nStyle(nStyle)
  ...
  , // current button no.
  m_nDefId(1)
  , // button number of default button
  m_nMaxID(FirstControlId)
  , // control id
  m_X(pXMB->x)
  , m_Y(pXMB->y)
  ...
  , m_bRightJustifyButtons(pXMB->dwOptions & XMSGBOXPARAMS::RightJustifyButtons)
{
    ...
    if (lpszMessage) {
        if (HIWORD(lpszMessage) == NULL) {
            ...
            if (::LoadString(hInstanceStrings, LOWORD((DWORD)lpszMessage), m_lpszMessage, MessageSize - 1) == 0)
                m_lpszMessage[0] = _T('\0');
        } else {
    ...
    if (lpszCaption) {
        if (HIWORD(lpszCaption) == NULL) {
            // looks like a resource id
            if (::LoadString(hInstanceStrings, LOWORD((DWORD)lpszCaption), m_lpszCaption, MessageSize - 1) == 0)
                m_lpszCaption[0] = _T('\0');
        } else {
            // looks like a string pointer
    ...
    if (pXMB->nIdCustomButtons) {
        ...
        if (::LoadString(hInstanceStrings, pXMB->nIdCustomButtons, m_szCustomButtons, countof(m_szCustomButtons) - 1) == 0)
    ...
    if (pXMB->nIdReportButtonCaption) {
        // load from resource id
        if (::LoadString(hInstanceStrings, pXMB->nIdReportButtonCaption, szReport, MaxButtonStringSize - 1) == 0)
            szReport[0] = _T('\0');
    }
    ...
    ::DrawText(hdc, GetMessageText(), -1, &msgrect, DT_LEFT | DT_NOPREFIX | DT_WORDBREAK | DT_CALCRECT);
    ...
    // client rect
    CXRect mbrect;
    SetRect(&mbrect, 0, 0, msgrect.Width() + (2 * SpacingSize), msgrect.Height() + (2 * SpacingSize));
...
// AddCheckBox
void
CXDialogTemplate::AddCheckBox(HDC hdc,
                              int& x,
                              int& y,
                              CXRect& rect,
                              CXRect& mbrect,
                              ...
                              LPCTSTR lpszButtonCaption)
{
    ...
    ::DrawText(hdc, lpszButtonCaption, -1, &checkboxrect, DT_LEFT | DT_NOPREFIX | DT_CALCRECT | DT_SINGLELINE);
...
CXDialogTemplate::LoadButtonStrings()
{
    ...
    _tcscpy(szDoNotAskAgain, _T("Apply to all"));
...
void
CXDialogTemplate::LoadButtonStringsFromResources(HINSTANCE hInstance)
{
...
// MsgBoxDlgProc
BOOL CALLBACK
CXDialogTemplate::MsgBoxDlgProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
            ...
            switch (wParam) {
                ...
                case IdExHelp: {
                    ...
                    if (_tcslen(szBuf) > 0) {
                        ...
                        if (cp) {
                            _tcscpy(cp, _T(".hlp"));
                            ::WinHelp(hwnd, szBuf, (Me->m_nHelpId == 0) ? HELP_PARTIALKEY : HELP_CONTEXT, Me->m_nHelpId);
                        }
                    }
                    return FALSE;
                }
                ...
                default:
                    ...
                    if (bFlag && Me->m_lpszModule && (Me->m_lpszModule[0] != _T('\0'))) {
                        TCHAR szPathName[MAX_PATH * 2];
        ...
        case WM_TIMER: // used for timeout
        {
            ...
            if (wParam == 1) // timeout timer
            {
                ...
                if (Me->m_szDefaultButton[0] == _T('\0')) {
                    ...
                    ::GetWindowText(hwndDefButton, Me->m_szDefaultButton, MaxButtonStringSize);
                }

                TCHAR szButtonText[MaxButtonStringSize * 2];
                _stprintf(szButtonText, XMESSAGEBOX_TIMEOUT_TEXT_FORMAT, Me->m_szDefaultButton, Me->m_nTimeoutSeconds);
...
void
CXDialogTemplate::AddItem(CXDialogItem::Econtroltype cType, UINT nID, CXRect* prect, LPCTSTR pszCaption)
{
...
CXDialogTemplate::Display()
{
    ...
    *(WORD*)(pdest + 1) = 0; // use default window class
...
#else
    int nActualChars     = MultiByteToWideChar(CP_ACP, 0, (LPCSTR)szTitle, -1, pchCaption, nTitleLen + 1);
#endif
...
#else
        int nActualChars = MultiByteToWideChar(CP_ACP, 0, (LPCSTR)m_pDlgItemArray[i]->m_szCaption, -1, pchCaption, nChars);
#endif
    ...
    rc = ::DialogBoxIndirectParam(hInstance, (LPCDLGTEMPLATE)pBuffer, m_hWnd, MsgBoxDlgProc, (LPARAM)this);
...
void
CXDialogItem::AddItem(CXDialogTemplate& dialog, Econtroltype ctrltype, UINT nID, CXRect* prect, LPCTSTR lpszCaption)
{
    short hidbu = HIWORD(GetDialogBaseUnits());
    short lodbu = LOWORD(GetDialogBaseUnits());
...
static DWORD
ReadRegistry(LPCTSTR lpszCompanyName, LPCTSTR lpszKey)
{
...
static void
WriteRegistry(LPCTSTR lpszCompanyName, LPCTSTR lpszKey, DWORD dwData)
{
    _ASSERTE((lpszKey != NULL) && (lpszKey[0] != _T('\0')));
    if (!lpszKey || lpszKey[0] == _T('\0'))
        return;
    ...
    DWORD dwResult = 0;
    HKEY hKey      = NULL;
    LONG lRet      = ::RegCreateKeyEx(HKEY_CURRENT_USER, szKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, &dwResult);
