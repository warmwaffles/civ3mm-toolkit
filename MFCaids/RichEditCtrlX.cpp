...
//
void CRichEditCtrlX::SetRTF(CString sRTF)
{
	// Read the text in
	EDITSTREAM es;
	es.dwError     = 0;
...
//
DWORD CALLBACK CRichEditCtrlX::CBStreamIn(DWORD dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{
...
//
DWORD CALLBACK CRichEditCtrlX::CBStreamOut(DWORD dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{
...
BOOL CRichEditCtrlX::IsUnderlined()
{
	CHARFORMAT cf = GetCharFormat();	
...
CHARFORMAT CRichEditCtrlX::GetCharFormat(DWORD dwMask)
{
	CHARFORMAT cf;
...
void CRichEditCtrlX::SetCharStyle(int MASK, int STYLE, int nStart, int nEnd)
{
	CHARFORMAT cf;
	cf.cbSize = sizeof(CHARFORMAT);
	//cf.dwMask = MASK;
...
void CRichEditCtrlX::SetUnderlined()
{
	long start = 0, end = 0;
	GetSel(start, end);
...
void CRichEditCtrlX::SetCenterAlign()
{
...
void CRichEditCtrlX::SetLeftAlign()
{
	PARAFORMAT paraFormat;
	paraFormat.cbSize     = sizeof(PARAFORMAT);
	paraFormat.dwMask     = PFM_ALIGNMENT;    
...
void CRichEditCtrlX::SetRightAlign()
{
	PARAFORMAT paraFormat;
	paraFormat.cbSize     = sizeof(PARAFORMAT);
	paraFormat.dwMask     = PFM_ALIGNMENT;    
...
BOOL CRichEditCtrlX::IsCenteredAlign()
{
...
BOOL CRichEditCtrlX::IsLeftAlign()
{
	PARAFORMAT pf = GetParagraphFormat();
...
BOOL CRichEditCtrlX::IsRightAlign()
{
	PARAFORMAT pf = GetParagraphFormat();
...
PARAFORMAT CRichEditCtrlX::GetParagraphFormat()
{
	PARAFORMAT pf;
...
void CRichEditCtrlX::SetBulleted()
{
...
BOOL CRichEditCtrlX::IsBulleted()
{
	PARAFORMAT pf = GetParagraphFormat();
...
void CRichEditCtrlX::SelectColor()
{
	CColorDialog dlg;
...
void CRichEditCtrlX::SetFontName(CString sFontName)
{
	CHARFORMAT cf = GetCharFormat();
...
void CRichEditCtrlX::SetFontSize(int nPointSize)
{
	CHARFORMAT cf = GetCharFormat();
...
void CRichEditCtrlX::GetSystemFonts(CStringArray &saFontList)
{
	CDC *pDC = GetDC();
...
//
BOOL CALLBACK CRichEditCtrlX::CBEnumFonts(LPLOGFONT lplf, LPTEXTMETRIC lptm, DWORD dwType, LPARAM lpData)
{
...
CString CRichEditCtrlX::GetSelectionFontName()
{
	CHARFORMAT cf = GetCharFormat();
...
long CRichEditCtrlX::GetSelectionFontSize()
{
	CHARFORMAT cf = GetCharFormat();
...
void CRichEditCtrlX::OnContextMenu(CWnd* pWnd, CPoint point) 
{
    ...
    flags = (!len || (nStartChar == 0 && nEndChar ==
        len)) ? MF_GRAYED : 0;
