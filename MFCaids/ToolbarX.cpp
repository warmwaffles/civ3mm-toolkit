...
BOOL CToolBarX::Create(CWnd* pParentWnd,
...
BOOL CToolBarX::CreateEx(CWnd* pParentWnd,
...
//
CWnd* CToolBarX::InsertControl(CRuntimeClass* pClass, LPCTSTR lpszWindowName, CRect& rect, UINT nID, DWORD dwStyle)
{
	CWnd* pCtrl = NULL;
	...
	if (pClass->IsDerivedFrom(RUNTIME_CLASS(CEdit)))
	{
		...
		if (!((CEditX*)pCtrl)->CreateEx(WS_EX_CLIENTEDGE,
                 _T("EDIT"),
                 NULL,
                 WS_CHILD | WS_VISIBLE | dwStyle,
				 rect, this, nID))
...
// In rect parameter, pass only the width and height.
//
CWnd* CToolBarX::InsertControl(CWnd* pCtrl, CRect &rect, UINT nID)
{
	ASSERT_VALID(pCtrl);
...
//
BOOL CToolBarX::AddDropDownButton(UINT nButtonID, UINT nMenuID, BOOL bArrow /*=TRUE*/)
{
	ASSERT_VALID(this);
...
void CToolBarX::AddDropDownButtonInfo(UINT nButtonID, UINT nMenuID)
{
...
CToolBarX::CDropDownButtonInfo* CToolBarX::FindDropDownButtonInfo(UINT nID)
{
...
BOOL CToolBarX::OnBtnDropDown(NMHDR* pNMHDR, LRESULT* pRes)
{
	UNUSED_ALWAYS(pRes);
...
BOOL CToolBarX::DropDownButton(UINT nID, CRect rc)
{
...
// button?".
//
void CToolBarX::OnQueryDelete(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//
void CToolBarX::OnQueryInsert(NMHDR *pNMHDR, LRESULT *pResult)
{
	UNUSED_ALWAYS(pNMHDR);
...
//
void CToolBarX::OnChange(NMHDR *pNMHDR, LRESULT *pResult)
{
	UNUSED_ALWAYS(pNMHDR);
...
//
void CToolBarX::OnBeginAdjust(NMHDR *pNMHDR, LRESULT *pResult)
{
	m_pToolBar = this;
...
//
void CToolBarX::OnCustomHelp(NMHDR *pNMHDR, LRESULT *pResult)
{
	UNUSED_ALWAYS(pNMHDR);
	UNUSED_ALWAYS(pResult);
...
// dialog box.
//
void CToolBarX::OnEndAdjust(NMHDR *pNMHDR, LRESULT *pResult)
{
...
//
void CToolBarX::OnGetButtonInfo(NMHDR *pNMHDR, LRESULT *pResult)
{
...
// toolbar customization dialog box.
//
void CToolBarX::OnReset(NMHDR *pNMHDR, LRESULT *pResult)
{
	// Delete all buttons
    CToolBarCtrl& tbCtrl = GetToolBarCtrl();
...
void CToolBarX::OnSave(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CToolBarX::OnRestore(NMHDR* pNMHDR, LRESULT* pResult)
{
...
int CToolBarX::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CToolBarX::PositionControls()
{
...
// 
CSize CToolBarX::GetButtonSize(TBBUTTON* pData, int iButton, DWORD dwMode)
{
...
CSize CToolBarX::CalcSize(TBBUTTON* pData, int nCount, DWORD dwMode)
{
...
int CToolBarX::WrapToolBar(TBBUTTON* pData, int nCount, int nWidth, DWORD dwMode)
{
	ASSERT(pData != NULL && nCount > 0);
...
void CToolBarX::SizeToolBar(TBBUTTON* pData, int nCount, int nLength, BOOL bVert, DWORD dwMode)
{
	ASSERT(pData != NULL && nCount > 0);
...
CSize CToolBarX::CalcLayout(DWORD dwMode, int nLength)
{
	ASSERT_VALID(this);
...
CSize CToolBarX::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void CToolBarX::GetButtonEx(int nIndex, TBBUTTON* pButton) const
{
...
void CToolBarX::SetButtonEx(int nIndex, TBBUTTON* pButton)
{
...
BOOL CToolBarX::IsVertDocked()
{
	UINT nID = GetParent()->GetDlgCtrlID();
	return ((nID == AFX_IDW_DOCKBAR_LEFT) || (nID == AFX_IDW_DOCKBAR_RIGHT));
}

void CToolBarX::SetTextOptions(ETextOptions eTextOptions, BOOL bUpdate)
{
	ASSERT(::IsWindow(m_hWnd));
...
void CToolBarX::SetIconOptions(EIconOptions eIconOptions, BOOL bUpdate)
{
	ASSERT(::IsWindow(m_hWnd));
...
void CToolBarX::OnSysColorChange() 
{
	m_clrBtnShadow  = ::GetSysColor(COLOR_BTNSHADOW);
	m_clrBtnHilight = ::GetSysColor(COLOR_BTNHILIGHT);
	m_clrBtnFace    = ::GetSysColor(COLOR_BTNFACE);
...
BOOL CToolBarX::ShowContextMenu(CPoint point)
{
	...
	if (pFrame && pFrame->IsKindOf(RUNTIME_CLASS(CFrameWnd)))
	{
		...
		while (pos != NULL)
		{
			...
			if (pBar && pBar->IsKindOf(RUNTIME_CLASS(CToolBar)))
			{
				...
				menu.AppendMenu((pBar->GetStyle() & 
					WS_VISIBLE ? MF_CHECKED : MF_UNCHECKED) | MF_STRING,
					pBar->GetDlgCtrlID(), strName);
			}
		}
...
//	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	menu.AppendMenu(MF_STRING |
		((GetStyle() & CCS_ADJUSTABLE) ? 0 : MF_GRAYED),
		ID_CUSTOMIZE_BAR,
		uLocalLoadString(IDS_CUSTOMIZE));
	{
		AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
		// Show text labels
		popupText.AppendMenu(MF_STRING |
			(m_eTextOptions == toTextLabels) ? MF_CHECKED : 0 |
			IsTextOptionAvailable(toTextLabels) ? 0 : MF_GRAYED,
			ID_TO_TEXTLABELS,
			uLocalLoadString(IDS_TO_TEXTLABELS));
		
		// Selective text on right
		popupText.AppendMenu(MF_STRING |
			(m_eTextOptions == toTextOnRight) ? MF_CHECKED : 0 |
			IsTextOptionAvailable(toTextOnRight) ? 0 : MF_GRAYED,
			ID_TO_TEXTONRIGHT,
			uLocalLoadString(IDS_TO_TEXTONRIGHT));
		
		// No text labels
		popupText.AppendMenu(MF_STRING |
			(m_eTextOptions == toNoTextLabels) ? MF_CHECKED : 0 |
			IsTextOptionAvailable(toNoTextLabels) ? 0 : MF_GRAYED,
			ID_TO_NOTEXTLABELS,
	...
	{
		// Small icons
		popupIcon.AppendMenu(MF_STRING |
			(m_eIconOptions == ioSmallIcons) ? MF_CHECKED : 0 |
			IsIconOptionAvailable(ioSmallIcons) ? 0 : MF_GRAYED,
			ID_IO_SMALLICONS,
			uLocalLoadString(IDS_IO_SMALLICONS));

		// Large icons
		popupIcon.AppendMenu(MF_STRING |
			(m_eIconOptions == ioLargeIcons) ? MF_CHECKED : 0 |
			IsIconOptionAvailable(ioLargeIcons) ? 0 : MF_GRAYED,
			ID_IO_LARGEICONS,
	...
	int nResult = menu.TrackPopupMenu(
...
void CToolBarX::OnContextMenu(CWnd* pWnd, CPoint point) 
{
	ASSERT_VALID(pWnd);
	CPoint pt(point);
...
void CToolBarX::Customize()
{
	if (m_bIsCustomizeEnable)
		GetToolBarCtrl().Customize();
}

void CToolBarX::OnInitCustomize(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
...
void CToolBarX::DrawGripper(CDC* pDC, const CRect& rect)
{
	...
	if ((m_dwStyle & (CBRS_GRIPPER | CBRS_FLOATING)) == CBRS_GRIPPER)
	{
		...
		if (m_dwStyle & CBRS_ORIENT_HORZ)
		{
			...
			pDC->Draw3dRect(
				rect.left + CX_BORDER_GRIPPER,
				rect.top + m_cyTopBorder,
				CX_GRIPPER, 
				rect.Height() - m_cyTopBorder - m_cyBottomBorder,
				m_clrBtnHilight, m_clrBtnShadow);
		}
		else
		{
			pDC->FillSolidRect(
				...
				rect.top,
				rect.Width(),
				CY_GRIPPER + CX_BORDER_GRIPPER + CX_BORDER_GRIPPER,
				m_clrBtnFace);
...
// The default state is set when Reset button of Customize Dialog Box is pressed.
//
BOOL CToolBarX::MarkDefaultState()
{
	CToolBarCtrl & tbCtrl = GetToolBarCtrl();
...
void CToolBarX::UpdateParentBandInfo()
{
    CWnd* pWnd = GetParent();
...
BOOL CToolBarX::GetButtonInfo(UINT nID, TBBUTTON& tbinfo)
{
    if (tbinfo.fsStyle & TBSTYLE_SEP)
        return TRUE;
...
LRESULT CALLBACK CToolBarX::CBTProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ASSERT(m_pToolBar != 0);
...
BOOL CToolBarX::HasButtonTip(UINT nID)
{
...
void CToolBarX::GetButtonText(UINT nID, CString& strText)
{
    CString strFull;
...
void CToolBarX::GetButtonTip(UINT nID, CString& strTip)
{
    GetButtonText(nID, strTip);
}

BOOL CToolBarX::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
...
void CToolBarX::OnGetInfoTip(NMHDR* pNMHDR, LRESULT* pResult)
{
...
//
BOOL CToolBarX::IsTextOptionAvailable(ETextOptions eTextOptions) const
{
...
//
BOOL CToolBarX::IsIconOptionAvailable(EIconOptions eIconOptions) const
{
...
//
void CToolBarX::DisableIconOptions(CByteArray &arDisabledIconOptions)
{
...
//
void CToolBarX::SetNoTextOnRight(CDWordArray &arID)
{
...
//
BOOL CToolBarX::HasButtonText(UINT nID)
{
...
//
BOOL CToolBarX::LoadToolBar(UINT nIDResource, COLORREF clrMask /*=RGB_BACKGROUND*/)
{
...
//
BOOL CToolBarX::LoadToolBar(LPCTSTR lpszResourceName)
{
	ASSERT_VALID(this);
...
// The bitmap should contain one image for each toolbar button.
//
BOOL CToolBarX::LoadBitmap(LPCTSTR lpszResourceName)
{
...
BOOL CToolBarX::LoadBitmapEx(LPCTSTR lpszResourceName)
{
	if (CToolBar::LoadBitmap(lpszResourceName))
	{
		m_lpszResourceName = lpszResourceName;
...
int CToolBarX::AddGloomIcon(HICON hIcon, int nIndex /*=-1*/)
{
  ICONINFO iconInfo;
...
int CToolBarX::AddGrayIcon(HICON hIcon, int nIndex /*=-1*/)
{
  ICONINFO iconInfo;
  ZeroMemory(&iconInfo, sizeof(iconInfo));
  if (!GetIconInfo(hIcon, &iconInfo))
...
BOOL CToolBarX::MakeDisabledImgList(HBITMAP hbmp, CSize sizeImage)
{
	CDC myDC;
	myDC.CreateCompatibleDC(0);
...
BOOL CToolBarX::MakeNormalImgList(HBITMAP hbmp, CSize sizeImage)
{
	CDC myDC;
	myDC.CreateCompatibleDC(0);
...
// within the bitmap.
//
void CToolBarX::SetButtons(int nNumButtons, TBBUTTONSETUP* lpButtons, ETextOptions eTextOptionsDef /*=toNoTextLabels*/)
{
    // Delete old buttons
    CToolBarCtrl& tbCtrl = GetToolBarCtrl();
    while (tbCtrl.DeleteButton(0));
...
// Also you can set default icon options and background (transparent) color.
//
BOOL CToolBarX::SetBitmap(UINT nIDResource, EIconOptions eIconOptionsDef /*=ioSmallIcons*/, COLORREF clrMask /*=RGB_BACKGROUND*/)
{
...
//
BOOL CToolBarX::SetBitmap(HBITMAP hbmImageWell)
{
...
//
void CToolBarX::HideCtrlsOnVertical(BOOL bHide)
{
	m_bHideCtrlsOnVertical = bHide;
}

void CToolBarX::OnBarStyleChange(DWORD dwOldStyle, DWORD dwNewStyle)
{
...
//
BOOL CToolBarX::EnableCustomize(BOOL bEnable)
{
