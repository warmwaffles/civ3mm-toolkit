...
int
CComboZoom::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
BOOL
CComboZoom::PreTranslateMessage(MSG* pMsg)
{
    if (pMsg->message == WM_KEYDOWN) {
        ...
        if ((((UINT)pMsg->wParam < '0') || ((UINT)pMsg->wParam > '9')) && ((UINT)pMsg->wParam != '\b') && ((UINT)pMsg->wParam != '%')) {
            return TRUE;
        }
    }
...
void
CComboZoom::SetParentWnd(CWnd* pWnd)
{
    m_pWnd = pWnd;
}

void
CComboZoom::InitComboZoom(int nZoomFactor /*= 0*/)
{
    ResetContent();
