...
CFadeWnd::CFadeWnd(CWnd* pParentWnd /* = NULL */,
                   BOOL bFade /* = TRUE */,
...
void
CFadeWnd::AutoFade(CWnd* pParentWnd /* = NULL */, COLORREF clrLight /* = RGB(255,255,255) */, COLORREF clrDark /* = RGB(  0,  0,  0) */)
{
    if (pParentWnd) {
...
BOOL
CFadeWnd::Create(CWnd* pParentWnd, BOOL bAutoDel /* = FALSE */, CRect rc /* = CRect(0,0,0,0) */)
{
    if (!pParentWnd || !IsWindow(pParentWnd->GetSafeHwnd()))
        return FALSE;
    ...
    if (!CWnd::CreateEx(0, WC_FADEWND, szText, WS_POPUP | WS_CHILD | WS_VISIBLE, rc, pParentWnd, NULL, NULL)) {
        if (bAutoDel)
            delete this;
    ...
    if (m_bFade) {
        ...
        for (int i = 0; i < 100; i += nStep) {
            dwStartCount = timeGetTime(); // tick counter in milliseconds
            ...
            dwEndCount = timeGetTime(); // tick counter in milliseconds
        ...
        m_hNewBitmap = FadeBitmap(m_hBitmap, (double)100);
...
void
CFadeWnd::CreateGradient(COLORREF clrLight /* = RGB(255,255,255) */, COLORREF clrDark /* = RGB(  0,  0,  0) */)
{
    for (int i = 0; i < 256; i++) {
        m_clrGradient[i] = RGB((GetRValue(clrLight) * i + GetRValue(clrDark) * (255 - i)) >> 8,
...
BOOL
CFadeWnd::PreCreateWindow(CREATESTRUCT& cs)
{
...
HBITMAP
CFadeWnd::CopyScreenToBitmap(LPRECT lpRect)
{
...
#define COLORREF2DIBRGB(clr) ((((clr) << 16) & 0x00ff0000) | ((clr)&0x0000ff00) | (((clr) >> 16) & 0x000000ff))
...
HBITMAP
CFadeWnd::FadeBitmap(HBITMAP hBmp, double dfTrans, HDC hBmpDC)
{
    ...
    if (hBmp) {
        ...
        if (hBufferDC) {
            ...
            if (hBmpDC && hBmp == (HBITMAP)GetCurrentObject(hBmpDC, OBJ_BITMAP)) {
            ...
            if (hDirectDC) {
                ...
                if (hDirectBitmap) {
                    ...
                    for (int i = 0; i < nSize; i++) {
                        ...
                        ptPixels[i] = RGB((GetBValue(Col) * iAlpha + iSrcB * (255 - iAlpha)) >> 8,
...
void
CFadeWnd::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void
CFadeWnd::OnRButtonDown(UINT nFlags, CPoint point)
{
...
void
CFadeWnd::OnKillFocus(CWnd* pNewWnd)
{
