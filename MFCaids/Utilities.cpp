...
//
CString uLocalLoadString(UINT id)
{
...
//
CString uLocalFormatString(UINT id, ...)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//
CString uLocalGetShort(UINT id)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
// }
//
DWORD uGetDllVersion(LPCTSTR lpszDllName)
{
    HINSTANCE hinstDll;
    DWORD dwVersion = 0;
...
//
void uBrowseFolder(HWND hParentWnd, CString *sFolder, CString sTitle, UINT ulFlags /*=0*/)
{
	CWaitCursor wait;
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//
BOOL uCreateNewFolder(CString sPath)
{
...
//
BOOL uIsDirExist(LPCTSTR lpPath)
{
...
//
BOOL uIsFileExist(LPCTSTR lpPath)
{
	CFileStatus st;
...
//
void uGetUserNameAndHost(CString& sUserName, CString& sComputerName, CString& sIPaddress)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
//
void uAppendCtrlDots(CWnd *pWnd, UINT nCtrlID)
{
	CString sTemp;
...
//
void uMakeShortString(CDC *pDC, CString &sText, int nColumnLen)
{
	LPCTSTR psz3Dots = _T("...");
...
//
BOOL uCheckIncorrectChars(CString &str, BOOL bReplaceBlanks /*= FALSE*/)
{
...
//
void uCheckLineFeed(CString &str)
{
	str.Replace("\n", "\r\n");
}
...
//
CString uGetPathName(CString sPath)
{
...
//
CString uGetFileName(CString sPath)
{
...
//
CString uGetFileExt(CString sPath)
{
	int pos = sPath.ReverseFind('.');
	if (pos == -1)
		return "";
...
//
CString uSHGetFolderPath(int csidl)
{
	HMODULE hMod = NULL;
...
//
void uGenerateNewName(CString &sName)
{
...
//
BOOL uCheckName(CString &sName, int nMaxLen /*=-1*/)
{
	CString sTmp;
...
//
BOOL uReadFile(LPCTSTR lpszFileName, CString &sContent, UINT nOpenFlags)
{
	char*   pszBuf = NULL;
	int     nLen;
...
//
BOOL uWriteFile(LPCTSTR lpszFileName, CString sContent, UINT nOpenFlags)
{
	try
	{
		CFile fl(lpszFileName, nOpenFlags); // open file
...
//
CString uGuid2Str(GUID *pGuid)
{
	CString str;
	str.Format("%.8X-%.4X-%.4X-%.2X%.2X-%.2X%.2X%.2X%.2X%.2X%.2X", pGuid->Data1, pGuid->Data2, pGuid->Data3,
		pGuid->Data4[0], pGuid->Data4[1], pGuid->Data4[2], pGuid->Data4[3], pGuid->Data4[4], pGuid->Data4[5],
...
//
int uFindNoCase(CString sTxt, CString sSub, int nStart /* = 0*/)
{
	sTxt.MakeUpper();
	sSub.MakeUpper();
...
//
void CFileX::ProcessPath(CString sPath)
{
...
//
BOOL CFileX::FileExists(CString sFileName)
{
...
//
DWORD CFileX::GetFileSize(CString sFileName)
{
	DWORD result = 0;
	try
	{			
...
// Remove the file
//
void CFileX::Remove(CString sFileName)
{
	try
	{
...
// Rename the file
//
void CFileX::Rename(CString sOldFileName, CString sNewFileName)
{
...
//
void CFileX::MakeNormal(CString sFileName)
{
...
//
BOOL CFileX::CreateBAKFile(CString sFileName)
{
	char path_buffer[_MAX_PATH];
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
...
// Rename the file
//
CString	CFileX::ChangeFileName(CString sWholeFileName, CString sNewFileName)
{
	char path_buffer[_MAX_PATH];
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
...
//
CString	CFileX::ChangeFileExt(CString sWholeFileName, CString sNewFileExt)
{
	char path_buffer[_MAX_PATH];
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
...
// Get folder path.
//
CString	CFileX::GetAsFolder(CString sName)
{
	char path_buffer[_MAX_PATH];
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
...
//
CString	CFileX::CreateTempFileName(CString sPath, CString sPrefix)
{
	CString result;
...
// Get folder name
//
CString	CFileX::GetFolder(CString sName)
{
	char path_buffer[_MAX_PATH];
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
...
//
CString	CFileX::GetFileName(CString sName)
{
	char path_buffer[_MAX_PATH];
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
...
//
CString	CFileX::GetShortFileName(CString sName)
{
	char path_buffer[_MAX_PATH];
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
...
//
BOOL CFileX::FolderExists(CString sFolderName)
{
...
//
CString CFileX::CreateTempFolder(CString sRoot)
{
...
// Remove all files
//
void CFileX::RemoveFiles(CString sFileName)
{
	CFileFind finder;
...
//
void CFileX::RemoveFolder(CString sFolderName)
{
...
COLORREF MapToSysColor(COLORREF color, BOOL bUseRGBQUAD)
{
	struct COLORMAP
	{
...
HBITMAP LoadSysColorBitmapSize(HINSTANCE hInst, HRSRC hRsrc, CSize sz)
{
	HGLOBAL hglb;
...
BOOL uMakeShortPath(CString &sPath)
{
...
CString uGetTempFileName(LPCSTR lpPrefixString /*= NULL*/)
{
...
CString uBin2Str(LPBYTE &lpData, DWORD nDataLen)
{
	CString sRes, sTmp;
...
DWORD uStr2Bin(CString sSrc, LPBYTE &lpData)
{
	sSrc.TrimLeft();
...
COLORREF uColorFadeRed(COLORREF col, int nFadeValue)
{
	int r = col & 0x0000ff;
...
COLORREF uColorFadeGreen(COLORREF col, int nFadeValue)
{
	int r = col & 0x0000ff;
	int g = (col & 0x00ff00) >> 8;
	int b = (col & 0xff0000) >> 16;
...
COLORREF uColorFadeBlue(COLORREF col, int nFadeValue)
{
	int r = col & 0x0000ff;
	int g = (col & 0x00ff00) >> 8;
	int b = (col & 0xff0000) >> 16;
...
//
BOOL uIsOpend(LPCSTR lpFilePath)
{
	OFSTRUCT reOpenBuff;
...
//
DWORD uDeleteFolder(CString sPath)
{
	DWORD dwErr = 0;
	CFileFind ff;
...
// Get file's version.
//
CString uGetFileVersion(CString sPath)
{
	...
	sQuery.Format (_T ("\\StringFileInfo\\%04x%04x\\ProductVersion"), ProductInfo.Translation.languageId, ProductInfo.Translation.characterSet);
...
// 2 - 2d > 1st
//
int uCompareVersions(CString sVer1, CString sVer2)
{
...
BOOL uCopyFile(CString sExistingFileName, CString sNewFileName)
{
