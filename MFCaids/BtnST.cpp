...
BOOL
CButtonST::PreTranslateMessage(MSG* pMsg)
{
    InitToolTip();
...
HBRUSH
CButtonST::CtlColor(CDC* pDC, UINT nCtlColor)
{
...
LRESULT
CButtonST::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
...
LRESULT
CButtonST::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
...
LRESULT
CButtonST::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
    ASSERT(m_bIsCheckBox);
    return GetCheck();
} // End of OnGetCheck
...
#ifdef BTNST_USE_NEWMENU
LRESULT
CButtonST::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu)
{
    LRESULT lResult;
...
#ifdef BTNST_USE_NEWMENU
void
CButtonST::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
    BOOL bSetFlag = FALSE;
...
void
CButtonST::OnKillFocus(CWnd* pNewWnd)
{
...
void
CButtonST::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
...
BOOL
CButtonST::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
...
LRESULT
CButtonST::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
    CancelHover();
    return 0;
} // End of OnMouseLeave
...
#ifdef BTNST_USE_NEWMENU
            CNewMenu* psub = (CNewMenu*)m_menuPopup.GetSubMenu(0);
            if (m_csCallbacks.hWnd)
                ::SendMessage(m_csCallbacks.hWnd, m_csCallbacks.nMessage, (WPARAM)psub, m_csCallbacks.lParam);
            DWORD dwRetValue = psub->TrackPopupMenu(
              TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD, rWnd.left, rWnd.bottom, this, NULL);
#else
...
HBITMAP
CButtonST::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
    HBITMAP hMask    = NULL;
    HDC hdcSrc       = NULL;
    HDC hdcDest      = NULL;
...
//
void
CButtonST::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
    CRect rBtn;
...
void
CButtonST::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
    BYTE byIndex = 0;
    ...
    // Ole'!
    pDC->DrawState(rImage.TopLeft(), rImage.Size(), m_csIcons[byIndex].hIcon, (bIsDisabled ? DSS_DISABLED : DSS_NORMAL), (CBrush*)NULL);
} // End of DrawTheIcon

void
CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
    HDC hdcBmpMem     = NULL;
    ...
    if (bIsDisabled && m_bShowDisabledBitmap) {
        ...
        ::BitBlt(
          pDC->m_hDC, rImage.left, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, hdcBmpMem, 0, 0, SRCPAINT);
    } // else
...
void
CButtonST::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
...
//
HBITMAP
CButtonST::CreateGrayscaleBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
...
//
HBITMAP
CButtonST::CreateDarkerBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
    HBITMAP hGrayBitmap = NULL;
    HDC hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
    HBITMAP hOldBmp1 = NULL, hOldBmp2 = NULL;
...
//
HICON
CButtonST::CreateGrayscaleIcon(HICON hIcon)
{
...
//
HICON
CButtonST::CreateDarkerIcon(HICON hIcon)
{
    HICON hGrayIcon = NULL;
    HDC hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
    BITMAP bmp;
...
COLORREF
CButtonST::DarkenColor(COLORREF crColor, double dFactor)
{
...
//
DWORD
CButtonST::SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut, int nCxDesiredOut, int nCyDesiredOut)
{
    HICON hIconIn           = NULL;
    HICON hIconOut          = NULL;
...
//
DWORD
CButtonST::SetIcon(int nIconIn, int nIconOut)
{
...
//
DWORD
CButtonST::SetIcon(HICON hIconIn, HICON hIconOut)
{
    BOOL bRetValue;
    ICONINFO ii;
...
//
DWORD
CButtonST::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
//
DWORD
CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
    int nRetValue = 0;
    BITMAP csBitmapSize;
...
//
DWORD
CButtonST::SetFlat(BOOL bFlat, BOOL bRepaint)
{
    m_bIsFlat = bFlat;
    if (bRepaint)
        Invalidate();
...
//
DWORD
CButtonST::SetAlign(BYTE byAlign, BOOL bRepaint)
{
    switch (byAlign) {
        case ST_ALIGN_HORIZ:
        case ST_ALIGN_HORIZ_RIGHT:
...
//
DWORD
CButtonST::SetPressedStyle(BYTE byStyle, BOOL bRepaint)
{
    switch (byStyle) {
...
//
DWORD
CButtonST::SetCheck(int nCheck, BOOL bRepaint)
{
    if (m_bIsCheckBox) {
        if (nCheck == 0)
            m_nCheck = 0;
...
//
DWORD
CButtonST::SetDefaultColors(BOOL bRepaint)
{
...
//
DWORD
CButtonST::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//
DWORD
CButtonST::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
    if (byColorIndex >= BTNST_MAX_COLORS)
        return BTNST_INVALIDINDEX;
...
//
DWORD
CButtonST::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
    BYTE byRed      = 0;
    BYTE byGreen    = 0;
    BYTE byBlue     = 0;
...
//
DWORD
CButtonST::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//
DWORD
CButtonST::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
    HINSTANCE hInstResource = NULL;
...
//
DWORD
CButtonST::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//
DWORD
CButtonST::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
//
void
CButtonST::SetTooltipText(int nText, BOOL bActivate)
{
    CString sText;
...
//
void
CButtonST::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void
CButtonST::ActivateTooltip(BOOL bActivate)
{
...
//
DWORD
CButtonST::EnableBalloonTooltip()
{
...
//
void
CButtonST::DrawTransparent(BOOL bRepaint)
{
...
//
DWORD
CButtonST::SetURL(LPCTSTR lpszURL)
{
...
#ifndef BTNST_USE_NEWMENU
DWORD
CButtonST::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
    HINSTANCE hInstResource = NULL;
...
// This function associates a menu to the button.
// The menu will be displayed clicking the button.
// The menu will be handled by the CNewMenu class.
//
// Parameters:
//		[IN]	nMenu
//				ID number of the menu resource.
...
#ifdef BTNST_USE_NEWMENU
DWORD
CButtonST::SetMenu(UINT nMenu,
                   HWND hParentWnd,
                   BOOL bWinXPStyle,
                   UINT nToolbarID,
                   CSize sizeToolbarIcon,
...
//
DWORD
CButtonST::SetMenuCallback(HWND hWnd, UINT nMessage, LPARAM lParam)
{
...
//
void
CButtonST::SizeToContent()
{
    if (m_csIcons[0].hIcon) {
        m_ptImageOrg.x = 0;
        m_ptImageOrg.y = 0;
        SetWindowPos(NULL, -1, -1, m_csIcons[0].dwWidth, m_csIcons[0].dwHeight, SWP_NOMOVE | SWP_NOZORDER | SWP_NOREDRAW | SWP_NOACTIVATE);
    } // if
    else if (m_csBitmaps[0].hBitmap) {
        m_ptImageOrg.x = 0;
        m_ptImageOrg.y = 0;
        SetWindowPos(
          NULL, -1, -1, m_csBitmaps[0].dwWidth, m_csBitmaps[0].dwHeight, SWP_NOMOVE | SWP_NOZORDER | SWP_NOREDRAW | SWP_NOACTIVATE);
    } // if
...
#ifdef BTNST_USE_SOUND
DWORD
CButtonST::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
...
//
DWORD
CButtonST::OnDrawBackground(CDC* pDC, CRect* pRect)
{
    COLORREF crColor;
...
//
DWORD
CButtonST::OnDrawBorder(CDC* pDC, CRect* pRect)
{
    // Draw pressed button
    if (m_bIsPressed) {
        if (m_bIsFlat) {
