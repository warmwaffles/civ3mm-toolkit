...
void
BCMenuData::SetAnsiString(LPCSTR szAnsiString)
{
    USES_CONVERSION;
...
void
BCMenuData::SetWideString(const wchar_t* szWideString)
{
    delete[] m_szMenuText; //Need not check for NULL because ANSI X3J16 allows "delete NULL"
...
BOOL
BCMenu::GetSelectDisableMode(void)
{
    if (IsLunaMenuStyle())
...
void
BCMenu::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
    ASSERT(lpDIS != NULL);
...
void
BCMenu::DrawItem_Win9xNT2000(LPDRAWITEMSTRUCT lpDIS)
{
    ASSERT(lpDIS != NULL);
    CDC* pDC = CDC::FromHandle(lpDIS->hDC);
    CRect rect;
            ...
            if (checkflag && checkmaps) {
                ...
                if (!selectedflag) {
                    ...
                    if (disable_old_style)
                        DitherBlt(
                          lpDIS->hDC, rect.left + 2, rect.top + 2 + dy, m_iconX, m_iconY, (HBITMAP)(bitmapstandard), 0, 0, m_clrBack);
                    else
                        DitherBlt2(pDC, rect.left + 2, rect.top + 2 + dy, m_iconX, m_iconY, bitmapstandard, 0, 0, m_clrBack);
...
COLORREF
BCMenu::LightenColor(COLORREF col, double factor)
{
...
COLORREF
BCMenu::DarkenColor(COLORREF col, double factor)
{
    if (factor > 0.0 && factor <= 1.0) {
        BYTE red, green, blue, lightred, lightgreen, lightblue;
        red        = GetRValue(col);
...
void
BCMenu::DrawItem_WinXP(LPDRAWITEMSTRUCT lpDIS)
{
    ASSERT(lpDIS != NULL);
    CDC* pDC = CDC::FromHandle(lpDIS->hDC);
    ...
    if (state & MF_SEPARATOR) {
        ...
        if (checkflag || standardflag || selectedflag || disableflag) {
            ...
            if (checkflag && checkmaps) {
                ...
                if (!selectedflag) {
                    ...
                    if (disable_old_style)
                        DitherBlt(
                          lpDIS->hDC, rect.left + dx, rect.top + dy, m_iconX, m_iconY, (HBITMAP)(bitmapstandard), 0, 0, transparentcol);
                    else
                        DitherBlt2(pDC, rect.left + dx, rect.top + dy, m_iconX, m_iconY, bitmapstandard, 0, 0, transparentcol);
                    if (state & ODS_SELECTED)
                        pDC->Draw3dRect(rect, crSelect, crSelect);
                    bitmapstandard.DeleteObject();
                }
        ...
        if (nIconNormal < 0 && state & ODS_CHECKED && !checkflag) {
            ...
            if (state & ODS_CHECKED || info.hbmpUnchecked) {
                ...
                DrawXPCheckmark(pDC, rect2, state & ODS_CHECKED ? info.hbmpChecked : info.hbmpUnchecked, crSelect);
            }
        }
...
BOOL
BCMenu::GetBitmapFromImageList(CDC* pDC, CImageList* imglist, int nIndex, CBitmap& bmp)
{
...
void
BCMenu::MeasureItem(LPMEASUREITEMSTRUCT lpMIS)
{
    ...
    if (state & MF_SEPARATOR) {
        ...
        if (Win32s != g_Shell)
            ...
            size.cy =
              DrawText(pDC->m_hDC, (LPCTSTR)lpstrText, wcslen(lpstrText), &rect, DT_SINGLELINE | DT_LEFT | DT_VCENTER | DT_CALCRECT);
...
void
BCMenu::SetIconSize(int width, int height)
{
    m_iconX = width;
    m_iconY = height;
}
...
BOOL
BCMenu::AppendODMenuA(LPCSTR lpstrText, UINT nFlags, UINT nID, int nIconNormal)
{
    USES_CONVERSION;
...
BOOL
BCMenu::AppendODMenuW(wchar_t* lpstrText, UINT nFlags, UINT nID, int nIconNormal)
{
...
BOOL
BCMenu::AppendODMenuA(LPCSTR lpstrText, UINT nFlags, UINT nID, CImageList* il, int xoffset)
{
    USES_CONVERSION;
...
BOOL
BCMenu::AppendODMenuW(wchar_t* lpstrText, UINT nFlags, UINT nID, CImageList* il, int xoffset)
{
    // Add the MF_OWNERDRAW flag if not specified:
    if (!nID) {
        if (nFlags & MF_BYPOSITION)
...
BOOL
BCMenu::InsertODMenuA(UINT nPosition, LPCSTR lpstrText, UINT nFlags, UINT nID, int nIconNormal)
{
    USES_CONVERSION;
...
BOOL
BCMenu::InsertODMenuW(UINT nPosition, wchar_t* lpstrText, UINT nFlags, UINT nID, int nIconNormal)
{
    if (!(nFlags & MF_BYPOSITION)) {
        int iPosition = 0;
...
BOOL
BCMenu::InsertODMenuA(UINT nPosition, LPCSTR lpstrText, UINT nFlags, UINT nID, CImageList* il, int xoffset)
{
    USES_CONVERSION;
...
BOOL
BCMenu::InsertODMenuW(UINT nPosition, wchar_t* lpstrText, UINT nFlags, UINT nID, CImageList* il, int xoffset)
{
    if (!(nFlags & MF_BYPOSITION)) {
        int iPosition = 0;
        BCMenu* pMenu = FindMenuOption(nPosition, iPosition);
...
BOOL
BCMenu::ModifyODMenuA(const char* lpstrText, UINT nID, int nIconNormal)
{
    USES_CONVERSION;
...
BOOL
BCMenu::ModifyODMenuW(wchar_t* lpstrText, UINT nID, int nIconNormal)
{
    int nLoc;
    BCMenuData* mdata;
...
BOOL
BCMenu::ModifyODMenuA(const char* lpstrText, UINT nID, CImageList* il, int xoffset)
{
    USES_CONVERSION;
...
BOOL
BCMenu::ModifyODMenuW(wchar_t* lpstrText, UINT nID, CImageList* il, int xoffset)
{
    int nLoc;
    BCMenuData* mdata;
    CArray<BCMenu*, BCMenu*> bcsubs;
...
BOOL
BCMenu::ModifyODMenuA(const char* lpstrText, UINT nID, CBitmap* bmp)
{
    USES_CONVERSION;
...
BOOL
BCMenu::ModifyODMenuW(wchar_t* lpstrText, UINT nID, CBitmap* bmp)
{
    if (bmp) {
        CImageList temp;
...
BOOL
BCMenu::ModifyODMenuA(const char* lpstrText, UINT nID, COLORREF fill, COLORREF border, int hatchstyle, CSize* pSize)
{
    USES_CONVERSION;
...
BOOL
BCMenu::ModifyODMenuW(wchar_t* lpstrText, UINT nID, COLORREF fill, COLORREF border, int hatchstyle, CSize* pSize)
{
    CWnd* pWnd = AfxGetMainWnd(); // Get main window
    CDC* pDC   = pWnd->GetDC();   // Get device context
    SIZE sz;
...
BOOL
BCMenu::ModifyODMenuA(const char* lpstrText, const char* OptionText, int nIconNormal)
{
    USES_CONVERSION;
...
BOOL
BCMenu::ModifyODMenuW(wchar_t* lpstrText, wchar_t* OptionText, int nIconNormal)
{
    BCMenuData* mdata;
...
BCMenuData*
BCMenu::NewODMenu(UINT pos, UINT nFlags, UINT nID, CString string)
{
    BCMenuData* mdata;
...
BOOL
BCMenu::LoadToolbars(const UINT* arID, int n)
{
    ASSERT(arID);
...
BOOL
BCMenu::LoadToolbar(UINT nToolBar)
{
    UINT nID, nStyle;
...
BOOL
BCMenu::LoadFromToolBar(UINT nID, UINT nToolBar, int& xoffset)
{
    int xset, offset;
    UINT nStyle;
    BOOL returnflag = FALSE;
...
BCMenu*
BCMenu::FindAnotherMenuOption(int nId, int& nLoc, CArray<BCMenu*, BCMenu*>& bcsubs, CArray<int, int&>& bclocs)
{
    int i, numsubs, j;
...
BCMenuData*
BCMenu::FindMenuOption(wchar_t* lpstrText)
{
    int i, j;
    BCMenu* psubmenu;
...
BOOL
BCMenu::LoadMenu(int nResource)
{
    return (BCMenu::LoadMenu(MAKEINTRESOURCE(nResource)));
};

BOOL
BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
    ASSERT_VALID(this);
...
void
BCMenu::RemoveTopLevelOwnerDraw(void)
{
    CString str;
    int i = 0, j = 0;
...
void
BCMenu::LoadCheckmarkBitmap(int unselect, int select)
{
...
//[18.06.99 rj]
BOOL
BCMenu::GetMenuText(UINT id, CString& string, UINT nFlags /*= MF_BYPOSITION*/)
{
    BOOL returnflag = FALSE;
...
void
BCMenu::DrawRadioDot(CDC* pDC, int x, int y, COLORREF color)
{
...
void
BCMenu::DrawCheckMark(CDC* pDC, int x, int y, COLORREF color)
{
    CPen m_penBack;
...
void
BCMenu::InitializeMenuList(int value)
{
    for (int i                  = 0; i <= m_MenuList.GetUpperBound(); ++i)
...
LRESULT
BCMenu::FindKeyboardShortcut(UINT nChar, UINT nFlags, CMenu* pMenu)
{
#ifdef _CPPRTTI
...
void
BCMenu::DitherBlt(HDC hdcDest, int nXDest, int nYDest, int nWidth, int nHeight, HBITMAP hbm, int nXSrc, int nYSrc, COLORREF bgcolor)
{
    ASSERT(hdcDest && hbm);
...
void
BCMenu::GetFadedBitmap(CBitmap& bmp)
{
    CDC ddc;
    COLORREF bgcol, col;
    BITMAP BitMap;
...
void
BCMenu::GetShadowBitmap(CBitmap& bmp)
{
    CDC ddc;
...
BOOL
BCMenu::AddBitmapToImageList(CImageList* bmplist, UINT nResourceID)
{
    BOOL bReturn = FALSE;
...
void
BCMenu::SetBitmapBackground(COLORREF color)
{
...
void
BCMenu::UnSetBitmapBackground(void)
{
    m_bitmapBackgroundFlag = FALSE;
}
...
void
BCMenu::AddFromToolBar(CToolBar* pToolBar, int nResourceID)
{
...
BOOL
BCMenu::Draw3DCheckmark(CDC* dc, const CRect& rc, BOOL bSelected, HBITMAP hbmCheck)
{
    CRect rcDest = rc;
    CBrush brush;
    COLORREF col;
...
BOOL
BCMenu::DrawXPCheckmark(CDC* dc, const CRect& rc, HBITMAP hbmCheck, COLORREF& colorout)
{
    CBrush brushin;
...
void
BCMenu::DitherBlt2(CDC* drawdc, int nXDest, int nYDest, int nWidth, int nHeight, CBitmap& bmp, int nXSrc, int nYSrc, COLORREF bgcolor)
{
...
void
BCMenu::SetDisableOldStyle(void)
{
    disable_old_style = TRUE;
}

void
BCMenu::UnSetDisableOldStyle(void)
{
    disable_old_style = FALSE;
}

BOOL
BCMenu::GetDisableOldStyle(void)
{
    return (disable_old_style);
}

WORD
BCMenu::NumBitmapColors(LPBITMAPINFOHEADER lpBitmap)
{
...
HBITMAP
BCMenu::LoadSysColorBitmap(int nResourceId)
{
...
BOOL
BCMenu::RemoveMenu(UINT uiId, UINT nFlags)
{
    if (MF_BYPOSITION & nFlags) {
...
BOOL
BCMenu::DeleteMenu(UINT uiId, UINT nFlags)
{
    if (MF_BYPOSITION & nFlags) {
        UINT uint = GetMenuState(uiId, MF_BYPOSITION);
        if (uint & MF_SEPARATOR && !(uint & MF_POPUP)) {
...
BOOL
BCMenu::AppendMenuA(UINT nFlags, UINT nIDNewItem, const char* lpszNewItem, int nIconNormal)
{
    USES_CONVERSION;
...
BOOL
BCMenu::AppendMenuW(UINT nFlags, UINT nIDNewItem, wchar_t* lpszNewItem, int nIconNormal)
{
    return AppendODMenuW(lpszNewItem, nFlags, nIDNewItem, nIconNormal);
}

BOOL
BCMenu::AppendMenuA(UINT nFlags, UINT nIDNewItem, const char* lpszNewItem, CImageList* il, int xoffset)
{
    USES_CONVERSION;
...
BOOL
BCMenu::AppendMenuW(UINT nFlags, UINT nIDNewItem, wchar_t* lpszNewItem, CImageList* il, int xoffset)
{
    return AppendODMenuW(lpszNewItem, nFlags, nIDNewItem, il, xoffset);
}

BOOL
BCMenu::AppendMenuA(UINT nFlags, UINT nIDNewItem, const char* lpszNewItem, CBitmap* bmp)
{
    USES_CONVERSION;
...
BOOL
BCMenu::AppendMenuW(UINT nFlags, UINT nIDNewItem, wchar_t* lpszNewItem, CBitmap* bmp)
{
    if (bmp) {
        CImageList temp;
        temp.Create(m_iconX, m_iconY, ILC_COLORDDB | ILC_MASK, 1, 1);
...
BOOL
BCMenu::InsertMenuA(UINT nPosition, UINT nFlags, UINT nIDNewItem, const char* lpszNewItem, int nIconNormal)
{
    USES_CONVERSION;
...
BOOL
BCMenu::InsertMenuW(UINT nPosition, UINT nFlags, UINT nIDNewItem, wchar_t* lpszNewItem, int nIconNormal)
{
    return InsertODMenuW(nPosition, lpszNewItem, nFlags, nIDNewItem, nIconNormal);
}

BOOL
BCMenu::InsertMenuA(UINT nPosition, UINT nFlags, UINT nIDNewItem, const char* lpszNewItem, CImageList* il, int xoffset)
{
    USES_CONVERSION;
...
BOOL
BCMenu::InsertMenuW(UINT nPosition, UINT nFlags, UINT nIDNewItem, wchar_t* lpszNewItem, CImageList* il, int xoffset)
{
    return InsertODMenuW(nPosition, lpszNewItem, nFlags, nIDNewItem, il, xoffset);
}

BOOL
BCMenu::InsertMenuA(UINT nPosition, UINT nFlags, UINT nIDNewItem, const char* lpszNewItem, CBitmap* bmp)
{
    USES_CONVERSION;
...
BOOL
BCMenu::InsertMenuW(UINT nPosition, UINT nFlags, UINT nIDNewItem, wchar_t* lpszNewItem, CBitmap* bmp)
{
    if (bmp) {
        CImageList temp;
        temp.Create(m_iconX, m_iconY, ILC_COLORDDB | ILC_MASK, 1, 1);
...
BOOL
BCMenu::ImageListDuplicate(CImageList* il, int xoffset, CImageList* newlist)
{
...
CMenu*
BCMenu::GetSubMenu(int nPos)
{
    return CMenu::GetSubMenu(nPos);
}

CMenu*
BCMenu::GetSubMenu(LPCTSTR lpszSubMenuName)
{
...
int
BCMenu::GetMenuPosition(char* pText)
{
    USES_CONVERSION;
...
int
BCMenu::GetMenuPosition(wchar_t* pText)
{
    int i, j;
    BCMenu* psubmenu;
    for (i = 0; i < (int)(GetMenuItemCount()); ++i) {
...
int
BCMenu::RemoveMenu(char* pText, BC_Seperator sPos)
{
    USES_CONVERSION;
...
int
BCMenu::RemoveMenu(wchar_t* pText, BC_Seperator sPos)
{
...
int
BCMenu::DeleteMenu(char* pText, BC_Seperator sPos)
{
    USES_CONVERSION;
...
int
BCMenu::DeleteMenu(wchar_t* pText, BC_Seperator sPos)
{
    int nPos = GetMenuPosition(pText);
    if (nPos != -1) {
...
// Larry Antram
BOOL
BCMenu::SetMenuText(UINT id, CString string, UINT nFlags /*= MF_BYPOSITION*/)
{
    BOOL returnflag = FALSE;
...
// courtesy of Warren Stevens
void
BCMenu::ColorBitmap(CDC* pDC, CBitmap& bmp, CSize bitmap_size, CSize icon_size, COLORREF fill, COLORREF border, int hatchstyle)
{
    CDC bmpdc;
    COLORREF m_newclrBack;
...
BOOL
BCMenu::IsWindowsClassicTheme(void)
{
    ...
    lRetCode          = RegOpenKeyEx(HKEY_CURRENT_USER, _T("Software\\Microsoft\\Plus!\\Themes\\Current"), 0, KEY_READ, &hKey);
    if (lRetCode == ERROR_SUCCESS) {
        size     = _MAX_PATH;
        type     = REG_SZ;
        lRetCode = ::RegQueryValueEx(hKey, NULL, NULL, &type, (unsigned char*)Buf, &size);
        if (lRetCode == ERROR_SUCCESS) {
...
int
BCMenu::GlobalImageListOffset(int nID)
{
...
BOOL
BCMenu::CanDraw3DImageList(int offset)
{
    BOOL retflag   = FALSE;
    int numcurrent = m_AllImagesID.GetSize();
...
int
BCMenu::AddToGlobalImageList(CImageList* il, int xoffset, int nID)
{
    int loc               = -1;
...
BOOL
BCMenuToolBar::LoadToolBar(LPCTSTR lpszResourceName)
{
    ASSERT_VALID(this);
    ASSERT(lpszResourceName != NULL);
...
BOOL
BCMenuToolBar::LoadBitmap(LPCTSTR lpszResourceName)
{
    ASSERT_VALID(this);
    ASSERT(lpszResourceName != NULL);
