...
//
void CMenuManager::DestroyAccel()
{
...
//
void CMenuManager::Install(CFrameWnd* pWnd)
{
	ASSERT_VALID(pWnd);
	m_pFrame = pWnd;
...
//
BOOL CMenuManager::AddToolbar(UINT nIdBase, UINT nIdNormal, UINT nIdHot)
{
	// load bitmap
	...
	if (!AddToImageList(&m_ilHot, nIdHot != NULL ? nIdHot : 
	(nIdNormal != NULL ? nIdNormal : nIdBase), nBtnCount))
		return FALSE;
...
//	* add each button ID to button map
//
BOOL CMenuManager::LoadToolbar(UINT nIDToolbar)
{
	// destroy everything
	Destroy();
...
//
LRESULT CMenuManager::WindowProc(UINT msg, WPARAM wp, LPARAM lp)
{
	switch(msg) 
	{
...
//
CFont* CMenuManager::GetMenuFont()
{
	if (!(HFONT)m_fontMenu) 
	{
...
//
BOOL CMenuManager::OnMeasureItem(LPMEASUREITEMSTRUCT lpms)
{
	ASSERT(lpms);
...
//
BOOL CMenuManager::OnDrawItem(LPDRAWITEMSTRUCT lpds)
{
	ASSERT(lpds);
...
//
void CMenuManager::DrawMenuText(CDC& dc, CRect rc, CString text, COLORREF color)
{
	CString left = text;
	CString right;
...
//
BOOL CMenuManager::Draw3DCheckmark(CDC& dc, const CRect& rc, BOOL bSelected, HBITMAP hbmCheck)
{
...
//
void CMenuManager::OnInitMenuPopup(CMenu* pMenu,
...
//
void CMenuManager::LoadAccel(HACCEL hAccel)
{
	DestroyAccel();
	int nAccel;
...
//
void CMenuManager::ConvertMenu(CMenu* pMenu,
...
//
LRESULT CMenuManager::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu)
{
	ASSERT_VALID(pMenu);
...
//
void CMenuManager::OnMenuSelect(UINT nItemID, UINT nFlags, HMENU hSysMenu)
{
...
//
BOOL CMenuManager::AppendAccelName(CString& sItemName, UINT nID)
{
	...
	for (ACCEL* pa = GetAccel(nID); pa; pa -= pa->cmd) 
	{
		...
		if (pa->fVirt & FVIRTKEY) 
		{
			switch (pa->key)
			{
			case VK_INSERT:   sItemName += "INS";      break;
			case VK_DELETE:   sItemName += "DEL";      break;
			case VK_ESCAPE:   sItemName += "ESC";      break;
			case VK_SPACE:    sItemName += "SPACE";    break;
			case VK_PRIOR:    sItemName += "PRIOR";    break;
			case VK_NEXT:     sItemName += "NEXT";     break;
			case VK_END:      sItemName += "END";      break;
			case VK_HOME:     sItemName += "HOME";     break;
			case VK_LEFT:     sItemName += "LEFT";     break;
			case VK_UP:       sItemName += "UP";       break;
			case VK_RIGHT:    sItemName += "RIGHT";    break;
			case VK_DOWN:     sItemName += "DOWN";     break;
			case VK_SELECT:   sItemName += "SELECT";   break;
			case VK_PRINT:    sItemName += "PRINT";    break;
			case VK_EXECUTE:  sItemName += "EXECUTE";  break;
			case VK_SNAPSHOT: sItemName += "SNAPSHOT"; break;
			case VK_HELP:     sItemName += "HELP";     break;
			default:
				{
					TCHAR keyname[64];
...
// sItemName is menu item name, which will have the accelerator appended.
//
BOOL CMenuManager::RemoveAccelName(CString& sItemName)
{
	int iTabPos = sItemName.Find('\t');
	if (iTabPos > 0)
	{
...
//
void CMenuManager::FixMFCDotBitmap()
{
...
//
HBITMAP CMenuManager::GetMFCDotBitmap()
{
...
//
BOOL CMenuManager::SetNormalImageList(UINT id, DWORD rgbBg)
{
	CBitmap    bmp;
...
//
BOOL CMenuManager::SetHotImageList(UINT id, DWORD rgbBg)
{
	CBitmap    bmp;
...
//
BOOL CMenuManager::SetDisabledImageList(UINT id, DWORD rgbBg)
{
	CBitmap    bmp;
...
//
BOOL CMenuManager::AddToImageList(CImageList *pImg, UINT nId, INT nBtnCount)
{
	// load bitmap
	CBitmap bmp;
...
//
void CMenuManager::SetBackgroundColor(COLORREF bgdColor)
{
	m_bgdRGB = bgdColor;
}
...
//
void CMenuManager::ShowButtons(BOOL bShow)
{
...
//
void CMenuManager::ShowDisabledButtonsInColor(BOOL bShow)
{
...
//
void CMenuManager::ShowAutoAccel(BOOL bShow)
{
	m_bAutoAccel = bShow;
}
