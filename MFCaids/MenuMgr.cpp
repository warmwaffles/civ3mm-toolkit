/*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\
    ...
    FILE NAME:	    MenuMgr.cpp
	CREATED:	    22.08.2002

	AUTHOR:		    Paul DiLascia (Microsoft Systems Journal)
	MODIFIED:	    Dreyk
	LAST REVERSION: 08.01.2003

    VERSION:        1.0
	DESCRIPTION:    CMenuManager implements "cool" menus with buttons in the menu 

    To use:
    * Call Install to install it in CMainFrame
    * Call LoadToolbar or (and) AddToolbar to load toolbars
...
BOOL CMenuManager::bTRACE = FALSE;
...
IMPLEMENT_DYNAMIC(CMenuManager, CSubclassWnd)

CMenuManager::CMenuManager()
{
    m_bgdRGB   = RGB(255, 0, 255);         // initial background color
    ...
    m_bUseDrawState         = TRUE;        // use DrawState by default
    m_bColorDisabled        = FALSE;       // use color for disabled buttons
    FixMFCDotBitmap();
    m_pFrame = NULL;
}
...
//
void
CMenuManager::Install(CFrameWnd* pWnd)
{
    ASSERT_VALID(pWnd);
    m_pFrame = pWnd;
...
/////////////////////////////////////////////////////////////////////////
// Load one toolbar and add it to the existents.
//
//  * add toolbar bitmap to image list
//	* add each button ID to button map
//  * set normal, hot and disable image lists
//  NOTE: if nIdNormal and (or) nIdHot is NULL,
//  it will be replaced with nIdBase
//
BOOL
CMenuManager::AddToolbar(UINT nIdBase, UINT nIdNormal, UINT nIdHot)
{
    // load bitmap
    HBITMAP hbmToolbar = PxLib::LoadSysColorBitmap(nIdBase);
    ...
    // load toolbar
    LPTSTR lpResName = MAKEINTRESOURCE(nIdBase);
    HINSTANCE hInst;
    HRSRC hRsrc;
    TOOLBARDATA* ptbd;

    if ((hInst = AfxFindResourceHandle(lpResName, RT_TOOLBAR)) == NULL || (hRsrc = FindResource(hInst, lpResName, RT_TOOLBAR)) == NULL ||
    ...
    m_szImg.cx = ptbd->wWidth;
    m_szImg.cy = ptbd->wHeight;

    if (m_szBitmap.cx == 0) {
        // First toolbar: initialized bitmap/button sizes and create image list.
        m_szBitmap = m_szImg;
        m_szButton = m_szImg + CSize(CXBUTTONMARGIN << 1, CYBUTTONMARGIN << 1);

        VERIFY(m_ilNormal.Create(m_szImg.cx, m_szImg.cy, ILC_MASK | ILC_COLOR24, 0, 10));
        VERIFY(m_ilHot.Create(m_szImg.cx, m_szImg.cy, ILC_MASK | ILC_COLOR24, 0, 10));
        VERIFY(m_ilDisabled.Create(m_szImg.cx, m_szImg.cy, ILC_MASK | ILC_COLOR24, 0, 10));
    } else if (m_szBitmap != m_szImg) {
    ...
    //
    int iNextImage = m_ilNormal.GetImageCount();
    int nBtnCount  = ptbd->wItemCount;

    if (!AddToImageList(&m_ilDisabled, nIdBase, nBtnCount))
        return FALSE;

    if (!AddToImageList(&m_ilNormal, nIdNormal != NULL ? nIdNormal : nIdBase, nBtnCount))
        return FALSE;

    if (!AddToImageList(&m_ilHot, nIdHot != NULL ? nIdHot : (nIdNormal != NULL ? nIdNormal : nIdBase), nBtnCount))
        return FALSE;
    ...
    m_arToolbarID.Add(nIdBase); // remember toolbar ID for Refresh
...
//
BOOL
CMenuManager::LoadToolbar(UINT nIDToolbar)
{
    // destroy everything
    Destroy();
    ...
    m_ilNormal.Add(&bmToolbar, m_bgdRGB); //GetSysColor(COLOR_3DFACE));

    m_ilHot.Create(&m_ilNormal);      // create hot image list
    m_ilDisabled.Create(&m_ilNormal); // create disabled image list
...
//
LRESULT
CMenuManager::WindowProc(UINT msg, WPARAM wp, LPARAM lp)
{
    switch (msg) {
...
*/
        ...
        case WM_MENUCHAR:
            LRESULT lr = OnMenuChar((TCHAR)LOWORD(wp), (UINT)HIWORD(wp), CMenu::FromHandle((HMENU)lp));
            if (lr != 0)
                return lr;
            break;
    }
...
//
BOOL
CMenuManager::OnMeasureItem(LPMEASUREITEMSTRUCT lpms)
{
    ASSERT(lpms);
    ...
    if (pmd->fType & MFT_SEPARATOR) {
        ...
        CMTRACE(_T("OnMeasureItem for '%s':\tw=%d h=%d\n"), (LPCTSTR)pmd->text, lpms->itemWidth, lpms->itemHeight);
    }
...
//
BOOL
CMenuManager::OnDrawItem(LPDRAWITEMSTRUCT lpds)
{
    ASSERT(lpds);
    ...
    {
        CMTRACE(_T("OnDrawItem for '%s':\tw=%d h=%d\n"), (LPCTSTR)pmd->text, rcItem.Width(), rcItem.Height());
        ...
        if (iButton >= 0) {
            ...
            if (!bDisabled) {
                ...
                PxLib::FillRect(dc, rcButn, GetSysColor((bChecked && !bSelected) ? COLOR_3DLIGHT : COLOR_MENU));
                ...
                // draw the button!
                if (bSelected)
                    m_ilHot.Draw(&dc, iButton, p, ILD_TRANSPARENT);
                else
                    m_ilNormal.Draw(&dc, iButton, p, ILD_TRANSPARENT);

            } else if (m_bUseDrawState) {
                // use DrawState to draw disabled button: must convert to icon
                HICON hIcon = m_ilDisabled.ExtractIcon(iButton);
                ASSERT(hIcon);
                ...
                PxLib::DrawEmbossed(dc, m_ilNormal, iButton, p, m_bColorDisabled);
            }
        } else {
            // no button: look for custom checked/unchecked bitmaps
            CMenuItemInformation info;
            info.fMask = MIIM_CHECKMARKS;
            GetMenuItemInfo((HMENU)lpds->hwndItem, lpds->itemID, MF_BYCOMMAND, &info);

            if (bChecked || info.hbmpUnchecked) {
                bHaveButn = Draw3DCheckmark(dc, rcButn, bSelected, bChecked ? info.hbmpChecked : info.hbmpUnchecked);
            }
        }
        ...
        if (bDisabled && (!bSelected || colorText == colorBG)) {
            ...
            //
            DrawMenuText(dc, rcText + CPoint(1, 1), pmd->text, GetSysColor(COLOR_3DHILIGHT));
        }
...
//
void
CMenuManager::DrawMenuText(CDC& dc, CRect rc, CString text, COLORREF color)
{
    CString left = text;
    CString right;
...
//
BOOL
CMenuManager::Draw3DCheckmark(CDC& dc, const CRect& rc, BOOL bSelected, HBITMAP hbmCheck)
{
    ...
    dc.BitBlt(rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), &memdc, p.x, p.y, SRCCOPY);
...
//
void
CMenuManager::OnInitMenuPopup(CMenu* pMenu, UINT nIndex, BOOL bSysMenu)
{
    if (m_bAutoAccel) {
        HACCEL hAccel = NULL;
...
//
void
CMenuManager::LoadAccel(HACCEL hAccel)
{
    DestroyAccel();
    int nAccel;
...
//
void
CMenuManager::ConvertMenu(CMenu* pMenu, UINT nIndex, BOOL bSysMenu, BOOL bShowButtons)
{
    ASSERT_VALID(pMenu);
    ...
    for (UINT i = 0; i < nItem; i++) {
        ...
        if (pmd && !pmd->IsMyItemData()) {
            CMTRACE(_T("CMenuManager: ignoring foreign owner-draw item\n"));
            continue; // owner-draw menu item isn't mine--leave it alone
        }

        if (bSysMenu && info.wID >= 0xF000) {
            CMTRACE(_T("CMenuManager: ignoring sys menu item\n"));
        ...
        if (bShowButtons) {
            ...
            if (m_pAccel && m_bAutoAccel)
                ...
                RemoveAccelName(pmd->text);

        } else // no buttons -- I'm converting to strings
        {
            // if ownerdraw:
            if (info.fType & MFT_OWNERDRAW) {
            ...
            if (m_pAccel && m_bAutoAccel) {
                if (AppendAccelName(sItemName, info.wID))
                    info.fMask |= MIIM_TYPE; //  change item type (string)
            } else if (RemoveAccelName(sItemName))
                info.fMask |= MIIM_TYPE; //  change item type (string)
...
//
LRESULT
CMenuManager::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu)
{
    ASSERT_VALID(pMenu);
...
//
void
CMenuManager::OnMenuSelect(UINT nItemID, UINT nFlags, HMENU hSysMenu)
{
    if (hSysMenu == NULL && nFlags == 0xFFFF) {
        // Windows has closed the menu: restore all menus to original state
        while (!m_menuList.IsEmpty()) {
            ConvertMenu(CMenu::FromHandle((HMENU)m_menuList.RemoveHead()), 0, FALSE, FALSE);
        }
    }
}
...
//
BOOL
CMenuManager::AppendAccelName(CString& sItemName, UINT nID)
{
...
/////////////////////////////////////////////////////////////////////////
// Remove the name of accelerator for given command ID to menu string.
// sItemName is menu item name, which will have the accelerator appended.
//
BOOL
CMenuManager::RemoveAccelName(CString& sItemName)
{
    int iTabPos = sItemName.Find('\t');
    if (iTabPos > 0) {
        sItemName = sItemName.Left(iTabPos);
...
/////////////////////////////////////////////////////////////////////////
// Set new image list for non-selected menu options.
//
BOOL
CMenuManager::SetNormalImageList(UINT id, DWORD rgbBg)
{
    CBitmap bmp;

    if (bmp.LoadBitmap(id)) // load bitmap for image list
    {
        if (rgbBg == NULL)
            rgbBg = m_bgdRGB;

        int nCount = m_ilNormal.GetImageCount(); // get count of images
        CImageList img;

        img.Create(m_szImg.cx, m_szImg.cy, ILC_MASK | ILC_COLOR24, 0, 10);
        img.Add(&bmp, rgbBg);
        bmp.DeleteObject();

        if (img.GetImageCount() == nCount) {
            m_ilNormal.DeleteImageList();
            m_ilNormal.Create(&img); // create a new image list for buttons
        } else
            return FALSE;
...
/////////////////////////////////////////////////////////////////////////
// Set new image list for selected menu options.
//
BOOL
CMenuManager::SetHotImageList(UINT id, DWORD rgbBg)
{
    CBitmap bmp;
    ...
    if (bmp.LoadBitmap(id)) // load bitmap for image list
    {
        ...
        int nCount = m_ilHot.GetImageCount(); // get count of images
        CImageList img;
        ...
        if (img.GetImageCount() == nCount) {
            m_ilHot.DeleteImageList();
            m_ilHot.Create(&img); // create a new image list for buttons
        } else
            return FALSE;
...
/////////////////////////////////////////////////////////////////////////
// Set new image list for for disabled menu options.
//
BOOL
CMenuManager::SetDisabledImageList(UINT id, DWORD rgbBg)
{
    CBitmap bmp;
    ...
    if (bmp.LoadBitmap(id)) // load bitmap for image list
    {
        ...
        int nCount = m_ilDisabled.GetImageCount(); // get count of images
        CImageList img;
        ...
        if (img.GetImageCount() == nCount) {
            m_ilDisabled.DeleteImageList();
            m_ilDisabled.Create(&img); // create a new image list for buttons
        } else
            return FALSE;
...
/////////////////////////////////////////////////////////////////////////
// Add new bitmaps to the given image list.
//
BOOL
CMenuManager::AddToImageList(CImageList* pImg, UINT nId, INT nBtnCount)
{
    // load bitmap
    CBitmap bmp;

    if (!bmp.LoadBitmap(nId)) {
        TRACE(_T("*** Can't load bitmap %d for toolbar!\n"), nId);
        return FALSE;
    }

    pImg->Add(&bmp, m_bgdRGB);

    return TRUE;
}

/////////////////////////////////////////////////////////////////////////
// Set new background (transparent) color.
//
void
CMenuManager::SetBackgroundColor(COLORREF bgdColor)
{
    m_bgdRGB = bgdColor;
}

/////////////////////////////////////////////////////////////////////////
// Show or not menu buttons.
//
void
CMenuManager::ShowButtons(BOOL bShow)
{
    m_bShowButtons = bShow;
}

/////////////////////////////////////////////////////////////////////////
// Draw disabled buttons in color or as usual.
// NOTE: We can draw disabled buttons in color
// only if m_bUseDrawState = FALSE
//
void
CMenuManager::ShowDisabledButtonsInColor(BOOL bShow)
{
    m_bColorDisabled = bShow;
    m_bUseDrawState  = !bShow;
}

/////////////////////////////////////////////////////////////////////////
// Generate auto accelerators.
//
void
CMenuManager::ShowAutoAccel(BOOL bShow)
{
    m_bAutoAccel = bShow;
}
...
//
HBITMAP
PxLib::LoadSysColorBitmap(LPCTSTR lpResName, BOOL bMono)
{
...
//
void
PxLib::FillRect(CDC& dc, const CRect& rc, COLORREF color)
{
    CBrush brush(color);
...
//
void
PxLib::DrawEmbossed(CDC& dc, CImageList& il, int i, CPoint p, BOOL bColor)
{
    IMAGEINFO info;
...
//
BOOL
CSubclassWnd::HookWindow(CWnd* pWnd)
{
    if (pWnd) {
        // Hook the window
...
//
LRESULT
CSubclassWnd::WindowProc(UINT msg, WPARAM wp, LPARAM lp)
{
    ASSERT(m_pOldWndProc);
    return m_pNext ? m_pNext->WindowProc(msg, wp, lp) : ::CallWindowProc(m_pOldWndProc, m_pWndHooked->m_hWnd, msg, wp, lp);
}
...
//
void
CSubclassWndMap::Add(HWND hwnd, CSubclassWnd* pSubclassWnd)
{
    ASSERT(hwnd && ::IsWindow(hwnd));
...
//
void
CSubclassWndMap::Remove(CSubclassWnd* pUnHook)
{
...
//
void
CSubclassWndMap::RemoveAll(HWND hwnd)
{
...
//
CSubclassWnd*
CSubclassWndMap::Lookup(HWND hwnd)
{
