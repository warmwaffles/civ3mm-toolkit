...
//
void
CGraphXImage::LoadFile(LPCTSTR sPath)
{
    CloseFile();
    m_sPath = sPath;
...
//
void
CGraphXImage::LoadFileSafely(LPCTSTR sPath)
{
    CloseFile();
    m_sPath = sPath;
...
//
void
CGraphXImage::Render(CDC* pDC, CRect& rcDst, UINT nMode /*= PaintOriginal*/)
{
    if (!IsLoaded())
        return;
    ...
    if (nMode == PaintProportion) {
        ...
        } else if (m_hbmp.Height > nOrgHeight) {
            rcPlay.right  = m_hbmp.Width * nOrgHeight / m_hbmp.Height;
            rcPlay.bottom = nOrgHeight;
        } else if (m_hbmp.Width < nOrgWidth) {
            if (m_hbmp.Height * nOrgWidth / m_hbmp.Width > nOrgHeight) {
                rcPlay.right  = m_hbmp.Width * nOrgHeight / m_hbmp.Height;
                rcPlay.bottom = nOrgHeight;
            } else {
        ...
        rcDst = rect;
    } else if (nMode == PaintProportion2) {
        INT nOrgWidth  = rcDst.Width();
        INT nOrgHeight = rcDst.Height();
        CRect rcPlay(0, 0, m_hbmp.Width, m_hbmp.Height);
...
//
void
CGraphXImage::DrawClipRect(CDC* pDC, CRect rcDst, CRect rcClip, UINT nMode /*= PaintOriginal*/)
{
    if (!IsLoaded())
        return;
...
//
void
CGraphXImage::GetFrame(CDC* pDC, CRect& rc)
{
    if (IsLoaded()) {
        rc.left = rc.top = 0;
...
//
BOOL
CGraphXImage::SetColorMode(UINT nBitsPerPixel)
{
    if (!IsLoaded())
        return FALSE;
    ...
    try {
        ...
        else if (nBitsPerPixel == 1) // black & white
            GRAPHX::ColorResBitmap(&m_hbmp, &m_hbmp, 1, CRF_NODITHERING | CRF_FIXEDPALETTE, NULL, NULL, 0, NULL, NULL);
        else if (nBitsPerPixel == 8) // 256 colors
            GRAPHX::ColorResBitmap(&m_hbmp, &m_hbmp, 8, CRF_NODITHERING | CRF_FIXEDPALETTE, NULL, NULL, 0, NULL, NULL);
        else if (nBitsPerPixel == 24) // true color
            GRAPHX::ColorResBitmap(&m_hbmp, &m_hbmp, 24, CRF_BYTEORDERBGR, NULL, NULL, 0, NULL, NULL);
        else
...
//
BOOL
CGraphXImage::ResizeImage(UINT cx, UINT cy)
{
    if (!IsLoaded())
        return FALSE;
...
//
BOOL
CGraphXImage::SaveChangesToFile()
{
    if (!IsLoaded())
        return FALSE;
...
//
BOOL
CGraphXImage::ConvertTo(UINT nFileType, LPCTSTR sNewExt /*= NULL*/)
{
    if (!IsLoaded())
        return FALSE;
