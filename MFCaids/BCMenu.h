...
class MFCAIDS_API BCMenu : public CMenu
{
  ...
  protected:
    ...
    void DitherBlt(HDC hdcDest, int nXDest, int nYDest, int nWidth, int nHeight, HBITMAP hbm, int nXSrc, int nYSrc, COLORREF bgcolor);
    void DitherBlt2(CDC* drawdc, int nXDest, int nYDest, int nWidth, int nHeight, CBitmap& bmp, int nXSrc, int nYSrc, COLORREF bgcolor);
...
class MFCAIDS_API BCMenuToolBar : public CToolBar
{
  public:
    BCMenuToolBar()
      : CToolBar()
    ...
    void GetIconSize(int& iconx, int& icony)
    {
        iconx = m_iconX;
        icony = m_iconY;
    }
...
class MFCAIDS_API BCMenuMemDC : public CDC
{
  public:
    // constructor sets up the memory DC
    BCMenuMemDC(CDC* pDC, LPCRECT lpSrcRect)
      : CDC()
    {
        ASSERT(pDC != NULL);
