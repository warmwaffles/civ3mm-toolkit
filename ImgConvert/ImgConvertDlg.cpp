...
CAboutDlg::CAboutDlg() : CDialogX(CAboutDlg::IDD)
{
...
void CImgConvertDlg::OnChangeHeight() 
{
...
void CImgConvertDlg::OnDeltaposHeightSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CImgConvertDlg::OnChangeWidth()
{
...
void CImgConvertDlg::OnDeltaposWidthSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
	OnChangeWidth();
	*pResult = 0;
...
void CImgConvertDlg::UpdateControlsState()
{
	UpdateData();
	...
	m_btnOK.EnableWindow(
		(m_bUseSize || m_bUseFileType) || 
		(m_bUseBitsPerPixel || m_bUseSuffix) ||
		m_bUseFrm);
}

void CImgConvertDlg::OnSelendokFileType() 
{
	UpdateControlsState();
}

void CImgConvertDlg::OnSelendokBitsPerPixel() 
{
	UpdateControlsState();
}

void CImgConvertDlg::OnChanged() 
{
	UpdateControlsState();
}
...
*/			
			...
			if ((m_bUseBitsPerPixel && m_bCivPalette) && nBitsPerPixel == cm_256Colors)
			{
				...
				{
					GRAPHX::ColorResBitmap(&img.m_hbmp, &img.m_hbmp, cm_256Colors,
...
*/
				...
				if (img.m_fi.Format == FILE_PCX)
				{
					...
					GRAPHX::ColorResBitmap(&img.m_hbmp, &img.m_hbmp, cm_256Colors,
						CRF_USERPALETTE, 
						pPalS.pal, NULL, 0, NULL, NULL);
				}
				else
				{
					GRAPHX::ColorResBitmap(&img.m_hbmp, &img.m_hbmp, cm_256Colors,
						CRF_USERPALETTE, 
						pPalD.pal, NULL, 0, NULL, NULL);
				}
...
void CImgConvertDlg::OnCivPal() 
{
	UpdateControlsState();
}

void CImgConvertDlg::OnFrmBrowseTargetPath() 
{
	CFileDialogX dlg(TRUE, NULL, NULL,
		OFN_ENABLESIZING | OFN_EXPLORER | 
		OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST,
...
void CImgConvertDlg::OnFrmTypeChanged() 
{
	UpdateControlsState();
}

void CImgConvertDlg::OnUseFrm() 
{
	UpdateControlsState();
}

void CImgConvertDlg::OnChangeFrmCx() 
{
	UpdateData();
}

void CImgConvertDlg::OnChangeFrmCy() 
{	
	UpdateData();
}

void CImgConvertDlg::OnChangeImgCx() 
{
	UpdateData();
}

void CImgConvertDlg::OnChangeImgCy() 
{
	UpdateData();
}
