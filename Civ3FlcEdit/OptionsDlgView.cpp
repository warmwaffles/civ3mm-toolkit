...
void COptionsDlgView::DoDataExchange(CDataExchange* pDX)
{
...
BOOL COptionsDlgView::OnInitDialog() 
{
...
void COptionsDlgView::Apply()
{
	...
	if (m_nMenuStyle == 0 && m_bMenuBorder)
	...
	MAIN_FRAME->SendMessageToDescendants(
		WM_BX_SETTINGCHANGE,
		NULL,
		(LPARAM)nFlags, TRUE);
	...
	{
		pParent->SendMessageToDescendants(
			WM_BX_SETTINGCHANGE,
			NULL,
			(LPARAM)nFlags);
	}
...
//
void COptionsDlgView::SetDefault()
{
	m_bToolBar         = TRUE;
	m_bStatusBar       = TRUE;
...
void COptionsDlgView::OnChanged() 
{
	OnChangeData();
}

void COptionsDlgView::OnChangedStyle()
{
	UpdateData();
	m_wndGloom.EnableWindow(m_nMenuStyle == 0);
...
void COptionsDlgView::OnChangedVisualEffects()
{
	UpdateData();
	m_wndShowIcon.EnableWindow(m_bBtnVisualEff);
