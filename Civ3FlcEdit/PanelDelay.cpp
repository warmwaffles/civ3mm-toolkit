...
void CPanelDelay::DoDataExchange(CDataExchange* pDX)
{
...
void CPanelDelay::SetView(CAnimationView *pView)
{
	m_pView = pView;
...
void CPanelDelay::OnReleasedcaptureTimerSlider(NMHDR* pNMHDR, LRESULT* pResult) 
{
	if (IsAttached())
	{
...
void CPanelDelay::OnRadioTimerDef() 
{
	if (!IsAttached())
		return;
...
void CPanelDelay::OnRadioTimerCustom() 
{
	if (!IsAttached())
		return;
...
BOOL CPanelDelay::IsAttached()
{
	return (m_pView != NULL && m_pView->GetSafeHwnd());
}

void CPanelDelay::UpdateControlsState()
{
	if (m_pView->IsLoaded())
	{
...
void CPanelDelay::OnChangeDelay() 
{
...
void CPanelDelay::OnDeltaposSpinDelay(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
BOOL CPanelDelay::PreTranslateMessage(MSG* pMsg) 
{
