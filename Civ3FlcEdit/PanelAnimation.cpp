...
void
CPanelAnimation::DoDataExchange(CDataExchange* pDX)
{
...
void
CPanelAnimation::OnReleasedcaptureFramesSlider(NMHDR* pNMHDR, LRESULT* pResult)
{
    if (IsAttached())
...
BOOL
CPanelAnimation::IsAttached()
{
    return (m_pView != NULL && m_pView->GetSafeHwnd());
}

void
CPanelAnimation::SetView(CAnimationView* pView)
{
    m_pView = pView;
...
void
CPanelAnimation::UpdateControlsState()
{
    if (m_pView->IsLoaded()) {
...
BOOL
CPanelAnimation::PreTranslateMessage(MSG* pMsg)
{
