...
HRESULT CScreen::Init(HWND hWnd)
{
	m_hWnd		= hWnd;
	m_lpDD		= NULL;
	m_lpClipper	= NULL;
...
*/
HRESULT CScreen::CreateWindowedMode()
{
	return CreateWindowedMode(WND_W, WND_CY);
}

HRESULT CScreen::CreateWindowedMode(int nWidth, int nHeight)
{
//	HDC hDC;
	DDSURFACEDESC ddsd;
	LPDIRECTDRAWSURFACE lpdds;
...
HRESULT CScreen::Fill(DWORD fillColor)
{
	if (!m_pBackSurf)
		return S_OK;
