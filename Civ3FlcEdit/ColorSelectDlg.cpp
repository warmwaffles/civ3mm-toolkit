...
void
CColorSelectDlg::DoDataExchange(CDataExchange* pDX)
{
...
void
CColorSelectDlg::OnCancel()
{
    CDialogX::OnCancel();
}

void
CColorSelectDlg::UpdateControlsState()
{
    UpdateData();
...
void
CColorSelectDlg::ChangeVariables()
{
    UpdateData();
...
CColorSelectDlg::OnPaint()
{
    ...
    if (m_wndPreview.GetSafeHwnd()) {
        ...
        if (IsLoaded()) {
            ...
            GRAPHX::PaintDC(dc.m_hDC,
                            &m_hbmp,
                            /*CRect(0, 0, m_hbmp.Width, m_hbmp.Height)*/ NULL,
                            NULL,
                            CRect(rcDest.left, rcDest.top, rcDest.left + m_hbmp.Width, rcDest.top + m_hbmp.Height) /*rcDest*/,
                            rc,
                            SRCCOPY);
...
BOOL
CColorSelectDlg::PreTranslateMessage(MSG* pMsg)
{
...
//
LONG
CColorSelectDlg::OnSelEndOK(UINT lParam, LONG wParam)
{
...
//
LONG
CColorSelectDlg::OnSelEndCancel(UINT lParam, LONG wParam)
{
    OnCancel();
    return TRUE;
}
...
//
LONG
CColorSelectDlg::OnSelChange(UINT lParam, LONG wParam)
{
    COLORREF col = (COLORREF)lParam;
    if (m_crDest != col) {
        m_crDest = col;
...
void
CColorSelectDlg::UpdateColorInfo()
{
    // Current color
...
void
CColorSelectDlg::ApplyColorChanges()
{
...
//
HBRUSH
CColorSelectDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void
CColorSelectDlg::OnSize(UINT nType, int cx, int cy)
{
...
void
CColorSelectDlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void
CColorSelectDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
    // Get the minimum and maximum scroll-bar positions.
    int minpos;
    int maxpos;
...
void
CColorSelectDlg::OnMouseMove(UINT nFlags, CPoint point)
{
    if (IsLoaded()) {
        CRect rc;
        m_wndPreview.GetWindowRect(rc);
...
void
CColorSelectDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
    if (IsLoaded()) {
        CRect rc;
        m_wndPreview.GetWindowRect(rc);
...
BOOL
CColorSelectDlg::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
...
//
void
CColorSelectDlg::DrawZoomedPreview()
{
    CDC* pDC = GetDC();
