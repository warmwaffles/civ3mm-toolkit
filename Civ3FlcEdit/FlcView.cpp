...
void
CFlcView::OnInitialUpdate()
{
    CAnimationView::OnInitialUpdate();
}

void
CFlcView::ShowAnimInfo()
{
...
void
CFlcView::OnUpdateFileSave(CCmdUI* pCmdUI)
{
    pCmdUI->Enable(IsLoaded());
}

void
CFlcView::OnUpdateFileSaveAs(CCmdUI* pCmdUI)
{
    pCmdUI->Enable(IsLoaded());
}
...
void
CFlcView::OnUpdateExport(CCmdUI* pCmdUI)
{
    pCmdUI->Enable(IsLoaded());
}
...
//
BOOL
CFlcView::LoadAnimation(LPCTSTR lpszPathName, BOOL bColorTable /*=TRUE*/)
{
...
*/
                ...
                switch (chunkh.Type) {
                    ...
                    case COLOR_256: {
                        ...
                        if (bColorTable) {
                            ...
                            m_colTransparent =
                              RGB(m_rgbPalDef.pal[255].rgbBlue, m_rgbPalDef.pal[255].rgbGreen, m_rgbPalDef.pal[255].rgbRed);
                        } else {
                            // Skip color chunk
    ...
    if (m_civ3Header.anim_length == 0) {
        ...
        SetWindowPos(NULL, 0, 0, m_flcHeader.width + WND_ACX, m_flcHeader.height + WND_ACY, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
    } else {
        UpdateBackground();
        m_bLeaderHead = FALSE;
...
//
UINT
CFlcView::WriteCOLOR_256(FILE* fp)
{
    UINT size;
...
//
UINT
CFlcView::ReadCOLOR_256(BYTE* pBuffer, FILE* fp, RGBQUADPAL& rgbPal)
{
    UINT read;
    short packets, i, j, idx;
    BYTE skip, change;
...
//
UINT
CFlcView::ReadDELTA_FLC(BYTE* pBuffer, FILE* fp)
{
    UINT read, pos = 0;
    short lines, l, j, i;
    short b, color;
...
//
UINT
CFlcView::ReadBLACK(BYTE* pBuffer, FILE* fp)
{
    ASSERT(FALSE);
...
//
UINT
CFlcView::ReadBYTE_RUN(BYTE* pBuffer, FILE* fp)
{
...
//
UINT
CFlcView::ReadFLI_COPY(BYTE* pBuffer, FILE* fp)
{
    UINT read;
...
void
CFlcView::OnTimer(UINT nIDEvent)
{
...
void
CFlcView::OnUpdateOffset(CCmdUI* pCmdUI)
{
...
//
BOOL
CFlcView::SaveFLC(LPCTSTR lpszPathName)
{
    if (m_arColorChunkPos.GetSize() == 0)
        return FALSE;
...
void
CFlcView::OnUpdateShrinkFrmCount(CCmdUI* pCmdUI)
{
    pCmdUI->Enable(FALSE);
}
