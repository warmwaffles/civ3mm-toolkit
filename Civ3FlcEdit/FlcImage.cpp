...
BOOL
WriteC3F(CString sPath, C3FSTRUCT c3f)
{
    try {
        /////////////////////////////////////////////////
        // Make C3F file
...
BOOL
ReadC3F(CString sPath, C3FSTRUCT* c3f)
{
    try {
        CFile fl;
...
void
CFlcImage::AttachFlcViewer(CFlcView* pFlc)
{
    ASSERT(pFlc);
    m_pFlc = pFlc;
}
...
CFlcImage::FlcToStoryboard()
{
    ...
    if (bResizeEnable) {
        if ((nResizeLeft != 0 || nResizeRight != nFrmWidth) || (nResizeTop != 0 || nResizeBottom != nFrmHeight))
            bResizeBorders = TRUE;
    ...
    try {
        ...
        GRAPHX::CreateBitmap(&hTempBmp, TYPE_CONV, nFrmWidthCur, nFrmHeightCur, m_constBits, ORDER_BGR, NULL, TOP_LEFT, NULL, 0);
...
*/
            ...
            BitBlt(hTempDC, 0, 0, nFrmWidthCur, nFrmHeightCur, hSurfDC, 0, 0, SRCCOPY);
            ...
            if (bSizeChanged) // Change frame sizes
            {
                ...
                {
                    BitBlt(hDC, nXDst + nIndentX, nYDst + nIndentY, nFrmWidth, nFrmHeight, hTempDC, nXSrc, nYSrc, SRCCOPY);
                }
            } else // Copy frame without sizes changing
            {
                BitBlt(hDC, nXDst, nYDst, nFrmWidth, nFrmHeight, hTempDC, 0, 0, SRCCOPY);
            }
...
// clock - finish
        ...
        GRAPHX::SaveBitmap(m_sPath, &m_hbmp, nFileType, dlg.m_nBitsPerPixel == BitsPerPixel_8 ? BITS_8 : BITS_24);
...
BOOL
CFlcImage::CreateFile(LPCSTR sPath)
{
    CFile fl;
...
BOOL
CFlcImage::CreateStoryboard(UINT nWidth, UINT nHeight, BYTE nFrmWidth, BYTE nFrmHeight, BYTE nDirCount, WORD nFrmCount, BYTE nBitsPerPixel)
{
    if ((nWidth < 1 || nHeight < 1) || (nFrmWidth < 1 || nFrmHeight < 1) || (nDirCount < 1 || nFrmCount < 1)) {
    ...
    try {
        ...
        GRAPHX::CreateBitmap(&m_hbmp, TYPE_CONV, nWidth, nHeight, nBitsPerPixel, ORDER_BGR, NULL, TOP_LEFT, NULL, 0);
...
BOOL
CFlcImage::OpenStoryboard(CString sPath, C3FSTRUCT& c3f)
{
    CloseFile();
    ...
    try {
        ...
        if (m_fi.Format == FILE_PCX) {
            ...
            if (GRAPHX::LoadFile((LPTSTR)(LPCTSTR)sTempFile,
                                 &m_hbmp,
                                 m_fi.BitsPerPixel /*m_constBits*/,
                                 ORDER_BGR,
                                 LOADFILE_ALLOCATE | LOADFILE_STORE,
...
BOOL
ImportPalette(RGBQUADPAL& palette)
{
...
BOOL
ExportPalette(RGBQUADPAL& palette)
{
    CFileDialogX dlg(FALSE, NULL, NULL, OFN_OVERWRITEPROMPT | OFN_HIDEREADONLY, CString((LPCTSTR)IDS_FILTER_PAL_EXP));
...
BOOL
ImportPaletteFromFile(CString sPathName, RGBQUADPAL& palette)
{
...
//
void
RemovePaletteFile(CString sPathName, EPaletteFileType type)
{
...
//
BOOL
ExportPaletteToFile(CString sPathName, RGBQUADPAL& palette, EPaletteFileType type, LPCTSTR sExtName /*= NULL*/)
{
    ...
    try {
        ...
        switch (type) {
            ...
            case PalMS: {
                ...
                // write header
                BYTE header[24] = { 0x52, 0x49, 0x46, 0x46, 0x10, 0x04, 0x00, 0x00, 0x50, 0x41, 0x4C, 0x20,
                                    0x64, 0x61, 0x74, 0x61, 0x04, 0x04, 0x00, 0x00, 0x00, 0x03, 0x00, 0x01 };
            ...
            case PalJASC: {
                ...
                for (idx = 0; idx < COLOR8; idx++) {
                    sRGB.Format("%d %d %d\n", palette.pal[idx].rgbRed, palette.pal[idx].rgbGreen, palette.pal[idx].rgbBlue);
...
BOOL
CFlcImage::CopyToFile(CAnimationView* pAnim)
{
    try {
        CloseFile();
...
//
void
CFlcImage::ReplaceLast32Colors(ELastPalType nLastPalType, LPCSTR sLastPalPath)
{
    switch (nLastPalType) {
        case PalHardcoded:
...
BOOL
CFlcImage::SaveToFile(CAnimationView* pAnim)
{
    if (!IsLoaded())
        return FALSE;
    ...
    try {
        CFileImgDlg dlg(FALSE,
                        NULL,
                        sFileName,
                        OFN_OVERWRITEPROMPT | OFN_ENABLESIZING | OFN_ENABLETEMPLATE | OFN_EXPLORER | OFN_LONGNAMES | OFN_PATHMUSTEXIST,
        ...
        // Save bitmap
        GRAPHX::SaveBitmap(m_sPath, &m_hbmp, dlg.m_nFileType, dlg.m_nBitsPerPixel);
...
BOOL
CFlcImage::SaveFrameToFile(CAnimationView* pAnim)
{
    try {
        ...
        GRAPHX::CreateBitmap(&m_hbmp, TYPE_CONV, nWidth, nHeight, BITS_24, ORDER_BGR, NULL, TOP_LEFT, NULL, 0);
        ...
        // Copy whole picture to the temp HDC
        BitBlt(hDC, 0, 0, nWidth, nHeight, hSurfDC, pAnim->m_xPos, pAnim->m_yPos, SRCCOPY);
...
BYTE
GetPaletteIndex(RGBQUADPAL& rgbPal, COLORREF& color)
{
    BYTE idx = 0;
    ...
    for (idx; idx < IDX_MAX; idx++) {
        if (rgbPal.pal[idx].rgbRed == r) {
            if (rgbPal.pal[idx].rgbGreen == g && rgbPal.pal[idx].rgbBlue == b)
                return idx;
        }
    }
