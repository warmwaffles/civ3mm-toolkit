...
//
void CMainFrame::LoadFromRegistry()
{
...
*/
CToolBarX* CMainFrame::GetToolBar()
{
	return &m_wndToolBar;
}
...
//
void CMainFrame::OnDrawColorButton(NMHDR *pNotifyStruct, LRESULT *result)
{
		...
		for (int i = 0; i < n; ++i)
		{
			...
			if (bi.idCommand == ID_BGD_SELECT)
			{
				...
				if (m_wndToolBar.GetToolBarCtrl().IsButtonEnabled(id))
				{
					...
					if (m_wndToolBar.GetToolBarCtrl().IsButtonPressed(id) ||
						m_wndToolBar.GetToolBarCtrl().IsButtonChecked(id))
					{
...
CAnimationView* CMainFrame::GetAnimationView()
{
	return (CAnimationView*)GetActiveView(); //!!!
}

void CMainFrame::ShowDialogBar(BOOL bShow)
{
...
void CMainFrame::OnNewStoryboard() 
{
...
void CMainFrame::OnUpdateFALSE(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(FALSE);
}

void CMainFrame::OnUpdateFALSEandUNCHECK(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(FALSE);
	pCmdUI->SetCheck(0);
}

void CMainFrame::OnOpenINI() 
{
	MAIN_APP->GetDocManager()->OpenFromINI();
}

void CMainFrame::OnOpenMulti() 
{
...
//
void CMainFrame::OnImporC3f() 
{
	CImportToC3F dlg;
	dlg.DoModal();
}

void CMainFrame::OnColorReplace()
{
//	RGBQUADPAL m_rgbPal;
