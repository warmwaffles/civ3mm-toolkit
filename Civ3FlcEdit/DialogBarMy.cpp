...
void CDialogBarMy::OnUpdateCmdUI(class CFrameWnd *pTarget, int bDisableIfNoHndler)
{
    UpdateDialogControls(pTarget, bDisableIfNoHndler);
}

BOOL CDialogBarMy::Create(CWnd* pParentWnd, CDialog *pDialog, CString &pTitle, UINT nID, DWORD dwStyle) 
{
	...
	::SetWindowPos(m_pDialog->m_hWnd, NULL, 
		0, CX_TOP_INDENT, 0, 0,
...
CSize CDialogBarMy::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    CRect rc;
...
CSize CDialogBarMy::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void CDialogBarMy::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos) 
{
	...
	if (m_pDialog->GetSafeHwnd())
	{
		...
		if (IsFloating())
			::SetWindowPos(m_pDialog->m_hWnd, NULL, 
			0, CY_BORDER, rc.Width(), rc.Height() - CY_BORDER,
			SWP_NOZORDER | SWP_NOACTIVATE);
		else
			::SetWindowPos(m_pDialog->m_hWnd, NULL, 
			0, CX_TOP_INDENT, rc.Width(), rc.Height() - CX_TOP_INDENT,
			SWP_NOZORDER | SWP_NOACTIVATE);
	}
}
...
// Mouse Handling
//
void CDialogBarMy::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (m_bPressedClose)
	{
		ReleaseCapture();
...
void CDialogBarMy::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_bPressedClose)
	{
		CRect rc;
...
void CDialogBarMy::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
...
void CDialogBarMy::OnNcLButtonUp(UINT nHitTest, CPoint point) 
{
	if (!IsFloating())
	{
...
UINT CDialogBarMy::OnNcHitTest(CPoint point) 
{
    if (IsFloating())
...
void CDialogBarMy::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CDialogBarMy::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
BOOL CDialogBarMy::IsHorz() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
        m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);
}

CPoint& CDialogBarMy::ClientToWnd(CPoint& point)
{
...
void CDialogBarMy::DrawGripper(CDC* pDC)
{
...
void CDialogBarMy::OnNcLButtonDblClk(UINT nHitTest, CPoint point) 
{
...
void CDialogBarMy::OnCloseBar()
{
	GetDockingFrame()->ShowControlBar(this, FALSE, FALSE);
}

LPCTSTR CDialogBarMy::RegKey()
{
	return "Dlg Bar";
}

void CDialogBarMy::OnDestroy() 
{
	CString sReg = RegKey();
...
void CDialogBarMy::ToggleDocking()
{
	m_pDockContext->ToggleDocking();
}

void CDialogBarMy::EnableDocking(DWORD dwStyle)
{
...
void CDialogBarMy::OnSize(UINT nType, int cx, int cy) 
{
	...
	if (m_pDialog->GetSafeHwnd())
	{
		if (IsFloating())
			::SetWindowPos(m_pDialog->m_hWnd, NULL, 
			0, CY_BORDER, cx, cy - CY_BORDER,
			SWP_NOZORDER | SWP_NOACTIVATE);
		else
			::SetWindowPos(m_pDialog->m_hWnd, NULL, 
			0, CX_TOP_INDENT, cx, cy - CX_TOP_INDENT,
			SWP_NOZORDER | SWP_NOACTIVATE);
	}
}
