...
void
CPreviewDlg::DoDataExchange(CDataExchange* pDX)
{
...
CPreviewDlg::OnInitDialog()
{
    ...
    try {
        ...
        if (m_pSurface) {
            GRAPHX::CreateBitmap(&m_hbmp, TYPE_CONV, m_nFrmWidthCur, m_nFrmHeightCur, 24, ORDER_BGR, NULL, TOP_LEFT, NULL, 0);
            ...
            BitBlt(hDC, 0, 0, m_nFrmWidthCur, m_nFrmHeightCur, hSurfDC, 0, 0, SRCCOPY);
            ...
            GRAPHX::CreateBitmap(&m_hbmp, TYPE_CONV, WND_W, WND_H, 24, ORDER_BGR, NULL, TOP_LEFT, NULL, 0);
        ...
        if (IsLoaded()) {
            ...
            if (m_bFrmResize)
                GRAPHX::PaintDC(dc.m_hDC,
                                &m_hbmp,
                                CRect(0, 0, m_nFrmWidthCur, m_nFrmHeightCur),
                                NULL,
                                rcDest,
                                rc,
                                SRCCOPY); //Render(&dc, rcDest, CGraphXImage::PaintProportion2);
            else {
...
BOOL
CPreviewDlg::PreTranslateMessage(MSG* pMsg)
{
...
void
CPreviewDlg::ShowPreviewWindow(BOOL bShow)
{
    ShowWindow(bShow ? SW_SHOW : SW_HIDE);
}

void
CPreviewDlg::OnTimer(UINT nIDEvent)
{
...
void
CPreviewDlg::SetParam(PREVIEW_PARAM* pParam)
{
    m_Mutex.Lock();
    m_pParam = pParam;
    m_Mutex.Unlock();
