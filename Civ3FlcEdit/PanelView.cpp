...
void
CPanelView::DoDataExchange(CDataExchange* pDX)
{
...
void
CPanelView::SetView(CAnimationView* pView)
{
    m_pView = pView;
...
void
CPanelView::OnSelendokComboPalettes()
{
    if (!IsAttached())
        return;
...
BOOL
CPanelView::IsAttached()
{
    return (m_pView != NULL && m_pView->GetSafeHwnd());
}

void
CPanelView::UpdateControlsState()
{
    if (m_pView->IsLoaded()) {
...
BOOL
CPanelView::PreTranslateMessage(MSG* pMsg)
{
