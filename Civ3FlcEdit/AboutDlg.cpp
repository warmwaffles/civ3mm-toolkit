...
//	CString sRegisteredCompanyName;
	...
	// Get copyright
	sQuery.Format (_T ("\\StringFileInfo\\%04x%04x\\LegalCopyRight"), ProductInfo.Translation.languageId,  ProductInfo.Translation.characterSet);
	::VerQueryValue (pProductInfo, (LPTSTR)(LPCTSTR)sQuery, (LPVOID*)&lpPtr,&uiVerLength);
...
BOOL CAboutDlg::PreTranslateMessage(MSG* pMsg) 
{
	if (pMsg->message == WM_KEYDOWN)
    {
		...
		if (!m_bThanksShowed)
		{
			switch (pMsg->wParam)
			{
			case 'T': m_sThanks  = 'T'; break;
			case 'H': m_sThanks += 'H'; break;
			case 'A': m_sThanks += 'A'; break;
			case 'N': m_sThanks += 'N'; break;
			case 'K': m_sThanks += 'K'; break;
			case 'S': m_sThanks += 'S';
...
// Set RTF text from resources to the rich edit control
//
void CAboutDlg::ShowTextResource(LPCTSTR lpName)
{
	HINSTANCE hAppInstance = AfxGetResourceHandle();
