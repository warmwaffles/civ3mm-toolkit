...
static const CString S_ACTION_INI[20] =
{
	"BLANK",
	"DEFAULT",
	"RUN",
...
static const CString S_ACTION_NAMES[20] =
{
	"Blank",
	"Default",
	"Run",
...
BOOL CDocManagerEx::CheckDocUnique()
{
...
CFlcDoc* CDocManagerEx::GetFlcDoc()
{
...
CDocTemplate* CDocManagerEx::GetDocTemplateFlc()
{
...
CC3fDoc* CDocManagerEx::GetC3fDoc()
{
...
CDocTemplate* CDocManagerEx::GetDocTemplateC3f()
{
	POSITION pos = GetFirstDocTemplatePosition();
	if (pos == NULL)
	{
...
{
	CFileFlcDlg dlg(TRUE, NULL, NULL,
...
BOOL CDocManagerEx::OnFileSaveAs()
{
	MAIN_FRAME->SetFocus();
...
void CDocManagerEx::RegisterShellFileTypes(BOOL bCompat)
{
	   ...
	   CDocTemplate::regFileTypeId) && !strFileTypeId.IsEmpty())
	{
		...
		if (!strFilterExt.IsEmpty())
		{
			...
			lSize = _MAX_PATH * 2;
			lResult = ::RegQueryValue(HKEY_CLASSES_ROOT, strFilterExt,
		...
		hKey = NULL;
		lResult = RegOpenKey(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts\\.flc",
			&hKey);
		...
		!strFileTypeId.IsEmpty())
	{
		...
		hKey = NULL;
		lResult = RegOpenKey(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts\\.c3f",
			&hKey);
		if (ERROR_SUCCESS == lResult)
		{
			RegDeleteValue(hKey, "Application");
...
void CDocManagerEx::OpenFromFolder()
{
	CWinApp* pApp = AfxGetApp();
	...
	while (bWorking)
	{
		...
		if (!finder.IsDirectory())
		{
			...
			if (sTmp.CompareNoCase(FLC_EXT) == 0 ||
				sTmp.CompareNoCase(C3F_EXT) == 0)
			{
