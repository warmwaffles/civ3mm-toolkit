...
void CShrinkFrmCountDlg::DoDataExchange(CDataExchange* pDX)
{
...
BOOL CShrinkFrmCountDlg::OnInitDialog() 
{
...
void CShrinkFrmCountDlg::OnDestroy() 
{
	SavePosition(this);
	CDialog::OnDestroy();	
}

void CShrinkFrmCountDlg::OnOK() 
{
	if (!ShrinkFrmCount())
		return;
...
void CShrinkFrmCountDlg::OnChangeFrmCount() 
{
...
void CShrinkFrmCountDlg::OnDeltaposFrmCountSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
BOOL CShrinkFrmCountDlg::ShrinkFrmCount()
{
...
//		GRAPHX::SizeBitmap(&m_hbmp, c3f.Width, c3f.Height);
		...
		GRAPHX::SaveBitmap(m_sPath, &m_hbmp,
			c3f.FileType == 0 ? FILE_PCX : FILE_BMP,
			m_fi.BitsPerPixel);
