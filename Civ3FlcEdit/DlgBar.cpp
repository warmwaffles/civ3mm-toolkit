...
void
CDlgBar::DoDataExchange(CDataExchange* pDX)
{
...
BOOL
CDlgBar::PreTranslateMessage(MSG* pMsg)
{
...
*/
    ...
    pButton->Setup(IDI_PANEL_DIRECT, BX_ALIGN_RIGHT, "Direction panel", IDC_HAND, FALSE, RGB_TXT_IN);
    ...
    pButton = m_wndRollup.GetPageInfo(nIndex)->pwndButton;
    pButton->Setup(IDI_PANEL_DELAY, BX_ALIGN_RIGHT, "Delay panel", IDC_HAND, FALSE, RGB_TXT_IN);
    ...
    pButton = m_wndRollup.GetPageInfo(nIndex)->pwndButton;
    pButton->Setup(IDI_PANEL_ANIM, BX_ALIGN_RIGHT, "Animation panel", IDC_HAND, FALSE, RGB_TXT_IN);
    ...
    pButton = m_wndRollup.GetPageInfo(nIndex)->pwndButton;
    pButton->Setup(IDI_PANEL_VIEW, BX_ALIGN_RIGHT, "View panel", IDC_HAND, FALSE, RGB_TXT_IN);
    ...
    pButton = m_wndRollup.GetPageInfo(nIndex)->pwndButton;
    pButton->Setup(IDI_PANEL_SOUND, BX_ALIGN_RIGHT, "Sound panel", IDC_HAND, FALSE, RGB_TXT_IN);
    ...
    pButton = m_wndRollup.GetPageInfo(nIndex)->pwndButton;
    pButton->Setup(IDI_PANEL_EXTRA, BX_ALIGN_RIGHT, "Extra panel", IDC_HAND, FALSE, RGB_TXT_IN);
...
void
CDlgBar::SetView(CAnimationView* pView)
{
    m_pView = pView;
...
void
CDlgBar::OnSize(UINT nType, int cx, int cy)
{
    CDialog::OnSize(nType, cx, cy);

    if (m_wndRollup.GetSafeHwnd()) {
        ::SetWindowPos(m_wndRollup.m_hWnd, NULL, 2, 5, cx - 5, cy - 5, SWP_NOZORDER | SWP_NOACTIVATE);
    }
}
