...
CFileFlcDlg::CFileFlcDlg(BOOL bOpenFileDialog,
                         LPCTSTR lpszDefExt,
...
void
CFileFlcDlg::OnSize(UINT nType, int cx, int cy)
{
...
CFileFlcDlg::OnFileNameOK()
{
    if (m_bFlc && m_bPreview) {
        ...
        if (pFi->Format != FILE_FLC && (pFi->Format != FILE_BMP && pFi->Format != FILE_PCX)) {
...
void
CFileFlcDlg::LoadFileForPreview()
{
    try {
