...
void CFlcInfoExt::DoDataExchange(CDataExchange* pDX)
{
...
BOOL CFlcInfoExt::OnInitDialog() 
{
	...
	m_Table.InsertColumn(COL_TYPE,
		uLoadString(IDS_COL_TYPE),
		LVCFMT_LEFT,
		150);

	m_Table.InsertColumn(COL_NOTE,
		uLoadString(IDS_COL_NOTE),
		LVCFMT_LEFT,
		100);
	
	m_Table.InsertColumn(COL_POS,
		uLoadString(IDS_COL_POS),
		LVCFMT_LEFT,
		100);

	m_Table.InsertColumn(COL_SIZE,
		uLoadString(IDS_COL_SIZE),
		LVCFMT_LEFT,
		100);
	...
	for (int i = 0; i < m_pParent->m_pFlc->m_arFrames.GetSize(); i++)
	{
		...
		*/
		for (j = 0; j < frm.numch; j++)
		{
			...
			{
			case ChDELTA_FLC:	sType = "DELTA_FLC";	nImg = imgDelta;	break;
			case ChBYTE_RUN:	sType = "BYTE_RUN";		nImg = imgRun;		break;
			case ChCOLOR_256:	sType = "COLOR_256";	nImg = imgColor;	break;
			case ChBLACK:		sType = "BLACK";		nImg = imgBlack;	break;
			case ChFLI_COPY:	sType = "FLI_COPY";		nImg = imgCopy;		break;
			default:			sType = "unknown";		nImg = imgErr;
			}
