...
BOOL CColorTable::Create(CPoint p, RGBQUADPAL* pPal, CWnd* pParentWnd, UINT nID /*=0*/)
{
...
//	m_hCursor = AfxGetApp()->LoadCursor(IDC_COLOR_PICKER);
    ...
    m_WindowRect.SetRect(p.x, p.y, 
		p.x + NUM_COL * m_nBoxSize + 3 * m_nMargin,
		p.y  + NUM_ROW * m_nBoxSize + 3 * m_nMargin);

	if (!CWnd::CreateEx(WS_EX_CLIENTEDGE, szClassName, _T(""), WS_VISIBLE | WS_CHILD, 
                        m_WindowRect.left, m_WindowRect.top, m_WindowRect.Width(), m_WindowRect.Height(),
	...
	m_BoxesRect = CRect(
		CPoint(0, 0), 
		CSize(NUM_COL * m_nBoxSize, NUM_ROW * m_nBoxSize)
		);
...
BOOL CColorTable::PreTranslateMessage(MSG* pMsg) 
{
...
//
void CColorTable::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    int row = GetRow(m_nCurrent),
        col = GetColumn(m_nCurrent);
...
void CColorTable::OnRButtonUp(UINT nFlags, CPoint point) 
{
...
int CColorTable::GetIndex(int row, int col) const
{ 
...
int CColorTable::GetRow(int nIndex) const               
{ 
...
int CColorTable::GetColumn(int nIndex) const            
{ 
    if (nIndex < 0 || nIndex >= COLOR8)
        return INVALID_COLOUR;
...
//
BOOL CColorTable::GetCellRect(int nIndex, const LPRECT& rect)
{
    if (nIndex < 0 || nIndex >= COLOR8)
        return FALSE;
...
void CColorTable::ChangeSelection(int nIndex)
{
...
void CColorTable::DrawCell(CDC* pDC, int nIndex)
{
...
BOOL CColorTable::OnQueryNewPalette() 
{
    Invalidate(FALSE);    
    return CWnd::OnQueryNewPalette();
}

void CColorTable::OnPaletteChanged(CWnd* pFocusWnd) 
{
...
void CColorTable::OnActivateApp(BOOL bActive, HTASK hTask) 
{
...
void CColorTable::ChangeSelectedColor(COLORREF color)
{
...
BOOL CColorTable::SetPalette(RGBQUADPAL *pPal)
{
	ASSERT(pPal);
	if (!pPal)
		return FALSE;
...
void CColorTable::ChangeSelectedIdx(int nIndex)
{
	CClientDC dc(this);        // device context for drawing
