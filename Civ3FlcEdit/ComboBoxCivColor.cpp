...
void
CComboBoxCivColor::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    if ((lpDrawItemStruct->itemAction | ODA_SELECT) && (state & ODS_SELECTED)) {
    ...
    if ((state & ODS_DISABLED) == 0) {
        ...
        ::FrameRect(dc.m_hDC, &rcColor, ::GetSysColorBrush(COLOR_WINDOWTEXT));
...
void
CComboBoxCivColor::SetItemColor(int nIndex, COLORREF rcColor)
{
...
COLORREF
CComboBoxCivColor::GetItemColor(int nIndex)
{
    if (nIndex < 0 || nIndex > MAX_COLORS_NUMBER)
...
void
CComboBoxCivColor::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
