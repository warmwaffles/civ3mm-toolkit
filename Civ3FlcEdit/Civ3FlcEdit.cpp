...
//
static BOOL
CompareFilenameSuffix(LPCTSTR lpszPathName, LPCTSTR lpszSuffix)
{
    int len    = strlen(lpszPathName);
    int suflen = strlen(lpszSuffix);

    return (suflen > 0 && len > suflen && strcmpi(&lpszPathName[len - suflen], lpszSuffix) == 0);
}
...
FlcFileFunc(LPCTSTR lpszPathName)
{
...
//
static BOOL CALLBACK
C3fFileFunc(LPCTSTR lpszPathName)
{
...
CCiv3FlcEditApp::CCiv3FlcEditApp()
  : m_mruFileMgr(this)
{
...
//  the specific initialization routines you do not need.
    ...
    CDocTemplateFlc* pDocTemplateFlc = new CDocTemplateFlc(IDR_FLCTYPE,
    ...
    CDocTemplateC3f* pDocTemplateC3f = new CDocTemplateC3f(IDR_C3FTYPE,
...
// Setup style for all ButtonX in our app
    ...
    if (m_bCheckFileType) {
        if (!IsFileTypeRegistered(GetDocManager()->GetDocTemplateFlc()) || !IsFileTypeRegistered(GetDocManager()->GetDocTemplateC3f())) {
...
//
void
CCiv3FlcEditApp::AddToRecentFileList(LPCTSTR lpszPathName)
{
...
BOOL
CCiv3FlcEditApp::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
{
...
//
void
CCiv3FlcEditApp::SetAppParameters()
{
...
//
BOOL
CCiv3FlcEditApp::IsFileTypeRegistered(CDocTemplate* pDocTemplate)
{
    ...
    if (pDocTemplate->GetDocString(strFileTypeId, CDocTemplate::regFileTypeId) && !strFileTypeId.IsEmpty()) {
        ...
        // path\shell\open\command = path filename
        strRegKey.Format(_T("%s\\shell\\open\\%s"), (LPCTSTR)strFileTypeId, (LPCTSTR) _T("command"));
...
//
void
CCiv3FlcEditApp::SetFileTypeAssociations(BOOL bRegister /*= TRUE*/)
{
    if (bRegister)
...
CDocManagerEx*
CCiv3FlcEditApp::GetDocManager()
{
    return (CDocManagerEx*)m_pDocManager;
}

void
CCiv3FlcEditApp::OnWindowCloseall()
{
...
//
BOOL
CCiv3FlcEditApp::OpenWithExisting(CCommandLineInfo& cmdInfo)
{
...
BOOL
CCiv3FlcEditApp::PreTranslateMessage(MSG* pMsg)
{
...
CAnimationView*
CCiv3FlcEditApp::GetAnimationView()
{
