...
void
CColorBCHSDlg::DoDataExchange(CDataExchange* pDX)
{
...
CColorBCHSDlg::OnInitDialog()
{
    ...
    if (!m_wndSliderB.Create(WS_VISIBLE | WS_CHILD | TBS_HORZ | TBS_BOTTOM | TBS_TOOLTIPS | TBS_AUTOTICKS, rect, this, IDC_SLIDER_B)) {
        ASSERT(FALSE);
    }
    ...
    if (!m_wndSliderC.Create(WS_VISIBLE | WS_CHILD | TBS_HORZ | TBS_BOTTOM | TBS_TOOLTIPS | TBS_AUTOTICKS, rect, this, IDC_SLIDER_C)) {
        ASSERT(FALSE);
    }
    ...
    if (!m_wndSliderH.Create(WS_VISIBLE | WS_CHILD | TBS_HORZ | TBS_BOTTOM | TBS_TOOLTIPS | TBS_AUTOTICKS, rect, this, IDC_SLIDER_H)) {
        ASSERT(FALSE);
    }
    ...
    if (!m_wndSliderS.Create(WS_VISIBLE | WS_CHILD | TBS_HORZ | TBS_BOTTOM | TBS_TOOLTIPS | TBS_AUTOTICKS, rect, this, IDC_SLIDER_S)) {
        ASSERT(FALSE);
    }
...
BOOL
CColorBCHSDlg::PreTranslateMessage(MSG* pMsg)
{
...
void
CColorBCHSDlg::UpdateControlsState()
{
    UpdateData();
    UpdatePreview();
}
...
void
CColorBCHSDlg::OnDeltaposSpinBright(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void
CColorBCHSDlg::OnChangeContrast()
{
...
void
CColorBCHSDlg::OnDeltaposSpinContrast(NMHDR* pNMHDR, LRESULT* pResult)
{
    NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
    OnChangeContrast();
    *pResult = 0;
...
void
CColorBCHSDlg::OnDeltaposSpinHue(NMHDR* pNMHDR, LRESULT* pResult)
{
    NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
    OnChangeHue();
    *pResult = 0;
...
void
CColorBCHSDlg::OnChangeSaturation()
{
...
void
CColorBCHSDlg::OnDeltaposSpinSaturation(NMHDR* pNMHDR, LRESULT* pResult)
{
    NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
    OnChangeSaturation();
    *pResult = 0;
...
void
CColorBCHSDlg::OnReleasedcaptureSliderB(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void
CColorBCHSDlg::OnReleasedcaptureSliderC(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void
CColorBCHSDlg::OnReleasedcaptureSliderH(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void
CColorBCHSDlg::OnReleasedcaptureSliderS(NMHDR* pNMHDR, LRESULT* pResult)
{
...
//
void
CColorBCHSDlg::ChangeSaturation()
{
    try {
        GRAPHX::CopyBitmap(&m_imgPrev.m_hbmp, &m_img.m_hbmp);
...
void
CColorBCHSDlg::SetupBitmap()
{
    try {
        m_img.CloseFile();
        m_imgPrev.CloseFile();

        GRAPHX::CreateBitmap(&m_img.m_hbmp, TYPE_CONV, 256, 1, 24, ORDER_BGR, NULL, TOP_LEFT, NULL, 0);
        ...
        for (int idx = 0; idx < COLOR8; idx++) {
            ...
            color = RGB(pPal->rgbRed, pPal->rgbGreen, pPal->rgbBlue);
