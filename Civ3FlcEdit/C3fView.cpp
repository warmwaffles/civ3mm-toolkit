...
void
CC3fView::ShowAnimInfo()
{
    CString sInfo = uFormatString(IDS_C3F_INFO,
    ...
    InfoShowDialog(sInfo, m_sPathName, 0, uLoadString(IDS_C3F_INFO_CAP));
}
...
void
CC3fView::OnUpdateExport(CCmdUI* pCmdUI)
{
...
//
BOOL
CC3fView::LoadAnimation(LPCTSTR lpszPathName, BOOL bColorTable /*=TRUE*/)
{
    //	CFadeWnd wndFade(MAIN_FRAME, FALSE);
...
*/			}
    ...
    return FALSE;
        ...
        if (hbmp.BitsPerPixel == BITS_24) {
            ...
            GRAPHX::CreateBitmap(&hTmpBbmp, TYPE_CONV, m_c3f.Width, m_c3f.Height, m_constBits, ORDER_BGR, NULL, TOP_LEFT, NULL, 0);
            ...
            BitBlt(pTempDC->m_hDC, 0, 0, m_c3f.Width, m_c3f.Height, pDC->m_hDC, 0, 0, SRCCOPY);
...
*/
    ...
    if (m_c3f.LeaderHead == TRUE) {
        ...
        SetWindowPos(NULL, 0, 0, m_c3f.FrmWidth + WND_ACX, m_c3f.FrmHeight + WND_ACY, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
    } else {
        UpdateBackground();
        m_bLeaderHead = FALSE;
...
BOOL
CC3fView::StoryboardToFlc(CString sPathName)
{
    if (!IsLoaded())
        return FALSE;
...
*/
        ...
        CFile fl;
        if (!fl.Open(sPathName, CFile::modeCreate | CFile::modeRead | CFile::modeWrite | CFile::typeBinary | CFile::shareDenyWrite)) {
...
//
UINT
CC3fView::WriteCOLOR_256(BYTE* pBuffer)
{
    UINT pos = 0;
...
//
UINT
CC3fView::WriteBYTE_RUN(BYTE* pBuffer, BYTE nDir)
{
    ...
    for (BYTE i = 0; i < m_c3f.FrmHeight; i++) {
        ...
        while (nLinePos < m_c3f.FrmWidth) {
            ...
            {
                while (pixelData[nPos] == idx && nLinePos < m_c3f.FrmWidth && nRes < FLI_MAX_RUN) {
                    nLinePos++;
                    nPos++;
                    nRes++;
                }
                ...
                n = 0;
                while (pixelData[nPos + 1] != idx && nLinePos < m_c3f.FrmWidth && nRes < FLI_MAX_RUN) {
                    nLinePos++;
                    nPos++;
                    nRes++;
...
UINT
CC3fView::WriteDELTA_FLC(BYTE* pBuffer, BYTE nDir, WORD nFrm)
{
    UINT pos        = 0;     // current buffer position
    UINT nPos       = 0;     // current pixel data position
    char nRes       = 0;     // resulting value
    ...
    for (BYTE nCurLine = 0; nCurLine < m_c3f.FrmHeight; nCurLine++) {
        ...
        while (nLinePos < m_c3f.FrmWidth) {
            ...
            if (pixelCur[nPos] != pixelPrev[nPos]) {
                ...
                while (pixelCur[nPos] != pixelPrev[nPos] && nLinePos < m_c3f.FrmWidth) {
                    // in case the next two pixels have the same
                    // indexes as current pair of pixels...
                    if (pixelCur[nPos] == idx1 && pixelCur[nPos + 1] == idx2) {
...
//
UINT
CC3fView::WriteFLI_COPY(BYTE* pBuffer, WORD nFrm)
{
...
void
CC3fView::OnTimer(UINT nIDEvent)
{
...
void
CC3fView::OnUpdateOffset(CCmdUI* pCmdUI)
{
...
void
CC3fView::OnUpdateFileSave(CCmdUI* pCmdUI)
{
    pCmdUI->Enable(IsLoaded());
}
...
//
BOOL
CC3fView::SaveC3F(LPCTSTR lpszPathName)
{
...
CC3fView::OnFileSaveAs()
{
    ...
    CFileDialogX dlg(FALSE,
                     C3F_EXT,
                     pDoc->GetTitle(),
                     OFN_ENABLESIZING | OFN_EXPLORER | OFN_OVERWRITEPROMPT | OFN_HIDEREADONLY | OFN_LONGNAMES | OFN_PATHMUSTEXIST,
...
void
CC3fView::OnUpdateFileSaveAs(CCmdUI* pCmdUI)
{
    pCmdUI->Enable(IsLoaded());
}

void
CC3fView::OnUpdateShrinkFrmCount(CCmdUI* pCmdUI)
{
    pCmdUI->Enable(IsLoaded() && m_c3f.FrmCount > 1);
}

void
CC3fView::OnShrinkFrmCount()
{
