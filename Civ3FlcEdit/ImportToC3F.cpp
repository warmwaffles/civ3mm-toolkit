...
void CImportToC3F::DoDataExchange(CDataExchange* pDX)
{
...
BOOL CImportToC3F::OnInitDialog() 
{
...
//
void CImportToC3F::UpdateControlsState()
{
	UpdateData();
...
//
void CImportToC3F::OnBrowseTargetPath() 
{
	try
	{
		...
		m_sTargetPath = sPath;
		m_sInfo = uFormatString(IDS_IMG_INFO,
			m_fi.Width,
			m_fi.Height,
			m_fi.BitsPerPixel);
...
//
void CImportToC3F::OnBrowseOutputPath() 
{
	CFileDialogX dlg(FALSE, C3F_EXT, CFileX::GetShortFileName(m_sTargetPath),
		OFN_ENABLESIZING | OFN_EXPLORER |
		OFN_LONGNAMES | OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT,
...
BOOL CImportToC3F::PreTranslateMessage(MSG* pMsg) 
{
...
//
void CImportToC3F::OnBrowsePalPath() 
{
	CFileDialogX dlg(TRUE, NULL, NULL,
...
void CImportToC3F::OnPalChanged() 
{
	UpdateControlsState();
}
...
//		dc.Draw3dRect(rc, ::GetSysColor(COLOR_BTNSHADOW), ::GetSysColor(COLOR_BTNHILIGHT));
		...
		if (m_bPreview && IsLoaded())
		{
			...
			GRAPHX::PaintDC(dc.m_hDC, &m_hbmp, CRect(m_nGrid, m_nGrid, m_nFrmWidth+m_nGrid, m_nFrmHeight+m_nGrid), NULL, rcDest, rc, SRCCOPY);
...
//
void CImportToC3F::UpdatePreview()
{
	if (m_wndPreview.GetSafeHwnd())
	{
		CRect rc;
...
void CImportToC3F::OnChangeFrmWidth() 
{
...
void CImportToC3F::OnDeltaposFrmWidthSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CImportToC3F::OnChangeFrmHeight() 
{
...
void CImportToC3F::OnDeltaposFrmHeightSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
	OnChangeFrmHeight();
...
void CImportToC3F::OnChangeFrmLeft() 
{
...
void CImportToC3F::OnDeltaposFrmLeftSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
	OnChangeFrmLeft();
...
void CImportToC3F::OnChangeFrmTop() 
{
...
void CImportToC3F::OnDeltaposFrmTopSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
	OnChangeFrmTop();
...
//
void CImportToC3F::OnShowLines() 
{
	UpdateData();
	UpdatePreview();
}

void CImportToC3F::OnShowGrid() 
{
	UpdateData();
	UpdatePreview();
}
...
// Change timer delay
//
void CImportToC3F::OnChangeDelay() 
{
...
// Change timer delay
//
void CImportToC3F::OnDeltaposSpinDelay(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
	OnChangeDelay();
...
void CImportToC3F::OnReleasedcaptureTimerSlider(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
//
void CImportToC3F::OnPreviewCheck() 
{
	UpdateData();
	UpdatePreview();
}

void CImportToC3F::OnOK() 
{
...
//			GRAPHX::GetBitmapColors(&m_hbmp, 0, dwColors, imgDest.m_rgbPal);//palette.pal);
		...
		if (nDirCount == 1 && 
			(m_nFrmWidth == 200 && m_nFrmHeight == 240))
		{
		...
		if (m_fi.Format == FILE_BMP)
		{
			...
			if (GRAPHX::LoadFile((LPTSTR)(LPCTSTR)sTempFile, &m_hbmp, BITS_24, ORDER_BGR, LOADFILE_ALLOCATE | LOADFILE_STORE, NULL, NULL, NULL, NULL) != SUCCESS)
			{
				AfxMessageBox(uFormatString(IDS_ERR_SB_LOAD, m_sPath), MB_ICONERROR);
				return FALSE;
			}
		...
		if (m_hbmp.BitsPerPixel == BITS_8)
		{
			GRAPHX::ColorResBitmap(&m_hbmp, &m_hbmp, BITS_24,
				CRF_BYTEORDERBGR, 
		...
		GRAPHX::CreateBitmap(&hTempBmp, TYPE_CONV, c3f.FrmWidth, c3f.FrmHeight, 
...
//			GRAPHX::SaveBitmap(imgDest.m_sPath, &imgDest.m_hbmp, c3f.FileType, BITS_8);
			...
			GRAPHX::FreeBitmap(&hTempBmp);
			GRAPHX::CreateBitmap(&hTempBmp, TYPE_CONV, c3f.Width, c3f.Height, 
				BITS_8, ORDER_BGR, NULL, TOP_LEFT, NULL, 0);
			...
			BitBlt(hTempDC, 0, 0, c3f.Width, c3f.Height, 
				hDC, 0, 0, SRCCOPY);
...
void CImportToC3F::OnSaveSettings() 
{
	UpdateData();
}

void CImportToC3F::OnBitsPerPixel() 
{
	UpdateData();
}

void CImportToC3F::OnGrid() 
{
	UpdateData();
	m_nGrid = m_bGrid ? 1 : 0;
	UpdatePreview();
