...
void CColorMoveDlg::DoDataExchange(CDataExchange* pDX)
{
...
BOOL CColorMoveDlg::OnInitDialog() 
{
	if (!m_pPalExtD)
	{
		EndDialog(IDCANCEL);
...
void CColorMoveDlg::OnCancel() 
{
	CDialog::OnCancel();
}

void CColorMoveDlg::OnChangeRange() 
{
	UpdateData();
...
void CColorMoveDlg::OnMoveRight() 
{
...
void CColorMoveDlg::OnMove() 
{
	MoveTo(m_nFrom, m_nTo, m_nNumber);
}

void CColorMoveDlg::OnChangeFirstSrc() 
{
	UpdateData();
	if (m_nTo > IDX_MAX)
	{
...
void CColorMoveDlg::OnChangeFirstDest() 
{
	UpdateData();
	if (m_nFrom > IDX_MAX)
	{
...
void CColorMoveDlg::OnAllPalette() 
{
	UpdateControlsState();
...
void CColorMoveDlg::UpdateControlsState()
{
	UpdateData();
...
void CColorMoveDlg::ChangeVariables()
{
	UpdateData();
...
void CColorMoveDlg::MoveTo(UINT nFrom, UINT nTo, UINT nNumber)
{
	if (nNumber == 0)
		return;
...
BOOL CColorMoveDlg::PreTranslateMessage(MSG* pMsg) 
{
